...
describe('debug', () => {
    test.concurrent('fix order of operations', async () => {
        const { sql } = await debugSql(openai, 'relation "departments" does not exist', codeBlock `
    ...
    test.concurrent('fix typos', async () => {
        const { sql, solution } = await debugSql(openai, 'syntax error at or near "fromm"', codeBlock `
...
describe('title', () => {
    test('title matches content', async () => {
        const { title, description } = await titleSql(openai, codeBlock `
        create table employees (
          id bigint primary key generated always as identity,
          name text,
          email text,
