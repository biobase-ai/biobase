...
import { assertAndRenderColumn, assertAndUnwrapNode, assertDefined, assertEachSideOfExpression, assertEitherSideOfExpression, assertNodeType, getPolicies, getPolicyInfo, parseConstant, renderFields, renderJsonExpression, renderTargets, unwrapNode, } from '../../test/sql-util';
...
describe('rls chat', () => {
    test.concurrent('defaults to authenticated role', async () => {
        const responseStream = await chatRlsPolicy(openai, [
            {
                role: 'user',
    ...
    test.concurrent('wraps every function in select', async () => {
        ...
        withMetadata({ sql }, () => {
            ...
            const usingExpression = assertAndUnwrapNode(usingNode, 'A_Expr', 'Expected USING to contain an expression');
            assertEitherSideOfExpression(usingExpression, (node) => {
                ...
                const selectStatement = assertAndUnwrapNode(subQuery.subselect, 'SelectStmt', 'Expected subselect to contain a SELECT statement');
                assertDefined(selectStatement.targetList, 'Expected SELECT statement to have a target list');
                const [target] = selectStatement.targetList.map((node) => assertAndUnwrapNode(node, 'ResTarget', 'Expected every select target to be a ResTarget'));
    ...
    test.concurrent('discourages restrictive policies', async () => {
        ...
        const responseText = await collectStream(responseStream);
        await expect(responseText).toMatchCriteria('Discourages restrictive policies and provides reasons why');
    });
    test.concurrent('user id is on joined table and joins are minimized', async () => {
        ...
        withMetadata({ sql }, () => {
            ...
            if (sublink.subLinkType === 'ANY_SUBLINK') {
                ...
                const columnName = assertAndRenderColumn(sublink.testexpr, 'Expected sublink test expression to be a column');
                expect(columnName).toBe('library_id');
            }
            // Validate sub-query
            assertDefined(sublink.subselect, 'Expected sublink to have a subselect');
            const selectStatement = assertAndUnwrapNode(sublink.subselect, 'SelectStmt', 'Expected sublink subselect to be a SELECT statement');
            assertDefined(selectStatement.targetList, 'Expected SELECT statement to have a target list');
            if (sublink.subLinkType === 'ANY_SUBLINK') {
                const columns = renderTargets(selectStatement.targetList, (node) => assertAndRenderColumn(node, 'Expected target list to contain columns'));
            ...
            const fromRangeVar = assertAndUnwrapNode(fromNode, 'RangeVar', 'Expected FROM clause to contain a RangeVar');
            ...
            const whereClause = assertAndUnwrapNode(selectStatement.whereClause, 'A_Expr', 'Expected WHERE clause to be an expression');
            assertEachSideOfExpression(whereClause, (node) => {
                const columnName = assertAndRenderColumn(node, 'Expected one side of WHERE clause to have a column');
                expect(columnName).toBe('user_id');
            }, (node) => {
                const sublink = assertAndUnwrapNode(node, 'SubLink', 'Expected one side of WHERE clause to contain a sub-query selecting auth.uid()');
                assertDefined(sublink.subselect, 'Expected sublink to contain a subselect');
                const selectStatement = assertAndUnwrapNode(sublink.subselect, 'SelectStmt', 'Expected subselect to contain a SELECT statement');
                assertDefined(selectStatement.targetList, 'Expected SELECT statement to contain a target list');
                const [functionCall] = renderTargets(selectStatement.targetList, (node) => {
                    const funcCall = assertAndUnwrapNode(node, 'FuncCall', 'Expected SELECT statement to contain a function call');
    ...
    test.concurrent('mfa', async () => {
        ...
        withMetadata({ sql }, () => {
            ...
            const expression = assertAndUnwrapNode(policy.with_check, 'A_Expr', 'Expected WITH CHECK to be an expression');
            assertEachSideOfExpression(expression, (node) => {
                const constValue = assertAndUnwrapNode(node, 'A_Const', 'Expected one side of expression to be a constant');
                const stringValue = parseConstant(constValue);
                expect(stringValue).toBe('aal2');
            }, (node) => {
                const sublink = assertAndUnwrapNode(node, 'SubLink', 'Expected one side of expression to be a sub-query');
                assertDefined(sublink.subselect, 'Expected sublink to have a subselect');
                const selectStatement = assertAndUnwrapNode(sublink.subselect, 'SelectStmt', 'Expected sub-query to be a SELECT statement');
                assertDefined(selectStatement.targetList, 'Expected SELECT statement to have a target list');
                const [jsonTarget] = renderTargets(selectStatement.targetList, (node) => {
                    const expression = assertAndUnwrapNode(node, 'A_Expr', 'Expected SELECT target list to contain a JSON expression');
