...
export async function clippy(openai, supabaseClient, messages) {
    ...
    try {
        await Promise.all(contextMessages.map((message) => openai.moderations.create({ input: message.content })));
    }
    catch (error) {
