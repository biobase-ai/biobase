 ...
 */
export const formatSql = (sql) => format(sql, { language: 'postgresql', keywordCase: 'lower' });
/**
 ...
 */
export function extractMarkdownSql(markdown) {
    const mdTree = fromMarkdown(markdown);
    return mdTree.children
        .filter((node) => node.type === 'code' && node.lang === 'sql')
        .map(({ value }) => value);
}
/**
 ...
 */
export function withMetadata(metadata, fn) {
    /**
     * Prepends metadata to an Error's stack trace.
     */
    function modifyError(err) {
        if (err instanceof Error && err.stack) {
            const formattedMetadata = Object.entries(metadata).map(([key, value]) => `${chalk.bold.dim(key)}:\n\n${chalk.green.dim(value)}`);
