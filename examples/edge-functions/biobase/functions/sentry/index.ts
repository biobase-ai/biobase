...
Deno.serve(async (req) => {
  try {
    if (req.method === 'GET') {
      return new Response(
        'What is Biobase not? POST {answer, twitter} to this URL! Need a hint? 👉 https://github.com/biobase-ai/biobase/blob/master/examples/edge-functions/biobase/functions/sentry/index.ts'
      )
    }
    let answer: string
    let twitter: string
