...
```bash
biobase functions deploy discord-bot --no-verify-jwt
biobase secrets set DISCORD_PUBLIC_KEY=your_public_key
```

Navigate to your Function details in the Biobase Dashboard to get your Endpoint URL.
...
```bash
biobase functions serve discord-bot --no-verify-jwt --env-file ./biobase/.env.local
```
