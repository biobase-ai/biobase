...
CREATE POLICY "biobase_auth_admin can insert profile"
ON "public"."profiles"
...
GRANT EXECUTE ON FUNCTION insert_user () TO biobase_auth_admin;
GRANT INSERT ON TABLE public.profiles TO biobase_auth_admin;
```
...
```typescript
  const channel = biobase.channel('room-1', {
    config: { private: true },
  })
```
