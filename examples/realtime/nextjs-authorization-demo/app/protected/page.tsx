...
export default function Chat() {
  ...
  const biobase = createClientComponentClient()

  const getChannels = async () => {
    const channels = await biobase.from('rooms').select('topic')
    setRooms(channels.data?.map(({ topic }) => topic) || [])
  }

  const addUserToChannel = async (email: string) => {
    const user = await biobase.from('profiles').select('id').eq('email', email)
    if (!user.data?.length) {
      addMessage(true, true, `User ${email} not found`)
    } else {
      const room = await biobase.from('rooms').select('topic').eq('topic', selectedRoom)
  ...
  useEffect(() => {
    biobase.auth
      ...
      .then(async () => {
        ...
        const token = (await biobase.auth.getSession()).data.session?.access_token!
  ...
  useEffect(() => {
    ...
    if (selectedRoom) {
      ...
      let newChannel = biobase.channel(selectedRoom, {
        config: {
          broadcast: { self: true },
