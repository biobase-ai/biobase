...
import 'package:biobase_auth_ui/biobase_auth_ui.dart';
...
class _CanvasPageState extends State<CanvasPage> {
  ...
  /// Biobase realtime channel to communicate to other clients
  ...
  Future<void> _initialize() async {
    ...
    final profileMap = await biobase
        .from('profiles')
        .select('username')
        .eq('id', biobase.auth.currentUser!.id)
        .maybeSingle();
    ...
    final initialData = await biobase
        .from('canvas_objects')
        .select()
  ...
  void onPanEnd(DragEndDetails _) async {
    ...
    // Save whatever was drawn to Biobase DB
    if (drawnObjectId == null) {
      return;
    }

    await biobase.from('canvas_objects').upsert({
      'id': drawnObjectId,
