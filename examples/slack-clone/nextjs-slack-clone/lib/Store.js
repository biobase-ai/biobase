...
export const useStore = (props) => {
  ...
  useEffect(() => {
    ...
    // Cleanup on unmount
    return () => {
      biobase.removeChannel(biobase.channel(messageListener))
      biobase.removeChannel(biobase.channel(userListener))
      biobase.removeChannel(biobase.channel(channelListener))
    }
  }, [])
...
export const fetchChannels = async (setState) => {
  try {
    let { data } = await biobase.from('channels').select('*')
...
export const fetchUser = async (userId, setState) => {
  try {
    let { data } = await biobase.from('users').select(`*`).eq('id', userId)
    let user = data[0]
...
export const deleteChannel = async (channel_id) => {
  try {
    let { data } = await biobase.from('channels').delete().match({ id: channel_id })
    return data
  } catch (error) {
    console.log('error', error)
  }
...
export const deleteMessage = async (message_id) => {
  try {
    let { data } = await biobase.from('messages').delete().match({ id: message_id })
    return data
  } catch (error) {
    console.log('error', error)
  }
