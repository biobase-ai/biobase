...
class Authentication extends Hooks {
  ...
  async 'sing up new user and sign in'() {
    ...
    expect(session).toBeDefined()
    await biobase.auth.setSession(session)

    // check if user is signed in
    const {
      data: [profile],
      error: errorInsert,
    } = await this.insertProfile(biobase, user, fakeUser)
    ...
    const { error } = await this.signOut(biobase)
  ...
  async 'create new users by phone auth'() {
    ...
    const {
      data: { user, session },
      error: signUpError,
    } = await this.signUpByPhone(biobase, fakeUser)
  ...
  async 'update user should update logged in user'() {
    ...
    // get signed in user data
    const user = await this.getUser(biobase)
    ...
    const {
      data: { user: updUser },
      error: updUserError,
    } = await this.updateUser(biobase, updParams)
    ...
    const updatedUser = await this.getUser(biobase)
    ...
    const signIn = await biobase.auth.signInWithPassword({
  ...
  async 'on auth state changed should return events'() {
    ...
    const { error: updUserError } = await this.updateUser(biobase, updParams)
    expect(updUserError).toBeNull()
