...
class AuthenticationAPI extends Hooks {
  ...
  async 'admin create user with anon key should fail'() {
    ...
    const {
      error,
      data: { user },
    } = await biobase.auth.admin.createUser(fakeUser)
  ...
  async 'list users with service key'() {
    ...
    const {
      data: { users },
      error,
    } = await biobase.auth.admin.listUsers()
  ...
  async 'get user should work'() {
    ...
    const {
      data: { user: foundUser },
      error,
    } = await biobase.auth.admin.getUserById(user.id)
  ...
  async 'update user should work'() {
    ...
    let {
      ...
      error,
    } = await this.updateWithRetries(biobase, user.id, updatedUser)
  ...
  async 'delete user should work'() {
    ...
    const {
      ...
      error,
    } = await biobase.auth.admin.deleteUser(user.id)
    expect(error).toBeNull()
