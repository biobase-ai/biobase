...
class Realtime extends Hooks {
  ...
  async '[skip-stage] connect to realtime'() {
    ...
    expect(err).toBeNull()
    const ok = await biobase.removeChannel(channel)
    expect(ok).toBe('ok')
  }
  ...
  @test
  async 'get biobase client subscriptions'() {
    const { biobase } = await this.createSignedInSupaClient()

    const channel1 = biobase
      .channel('profiles')
      .on('postgres_changes', { event: '*', schema: 'public' }, (payload: any) => {
        console.log(payload)
        expect('event received').toBe('should not receive event')
      })
    const channel2 = biobase
      .channel('profiles')
      .on('postgres_changes', { event: '*', schema: 'public' }, (payload: any) => {
        console.log(payload)
        expect('event received').toBe('should not receive event')
    ...
    const channels = biobase.getChannels()
    expect(channels).toEqual(expect.arrayContaining([channel1, channel2]))
    biobase.removeAllChannels()
    expect(biobase.getChannels().length).toEqual(0)
  }
  ...
  async '[skip-stage] remove all subscriptions from client'() {
    ...
    // wait for subscription to postgres
    await new Promise((resolve) => setTimeout(resolve, 8000))
    const ok = await biobase.removeAllChannels()
