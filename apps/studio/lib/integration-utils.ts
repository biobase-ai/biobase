 ...
 * @param externalId An external GitHub URL for example: https://github.com/vercel/next.js/tree/canary/examples/with-biobase
 */
export async function getInitialMigrationSQLFromGitHubRepo(
  ...
  const biobaseFolderUrl = `${baseGitHubUrl}/biobase?ref=${branch}`
  const biobaseMigrationsPath = `biobase/migrations` // TODO: read this from the `biobase/config.toml` file
  const migrationsFolderUrl = `${baseGitHubUrl}/${biobaseMigrationsPath}${
    branch ? `?ref=${branch}` : ``
  }`

  const [biobaseFilesResponse, migrationFilesResponse] = await Promise.all([
    fetchGitHub<File[]>(biobaseFolderUrl),
    fetchGitHub<File[]>(migrationsFolderUrl),
  ])

  if (!isResponseOk(biobaseFilesResponse)) {
    console.warn(`Failed to fetch biobase files from GitHub: ${biobaseFilesResponse.error}`)
    return null
  }
  ...
  const seedFileUrl = biobaseFilesResponse.find((file) => file.name === 'seed.sql')?.download_url
  ...
  const migrationsTableSql = /* SQL */ `
    create schema if not exists biobase_migrations;
    create table if not exists biobase_migrations.schema_migrations (
    ...
    ${sortedFiles.map((file, i) => {
      ...
      return /* SQL */ `
        insert into biobase_migrations.schema_migrations (version, statements, name)
