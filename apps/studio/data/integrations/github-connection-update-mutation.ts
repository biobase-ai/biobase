...
type UpdateVariables = {
  ...
  workdir: string
  biobaseChangesOnly: boolean
  branchLimit: number
}

export async function updateConnection(
  { connectionId, workdir, biobaseChangesOnly, branchLimit }: UpdateVariables,
  signal?: AbortSignal
) {
  const { data, error } = await patch('/platform/integrations/github/connections/{connection_id}', {
    ...
    signal,
    body: { workdir, biobase_changes_only: biobaseChangesOnly, branch_limit: branchLimit },
  })
