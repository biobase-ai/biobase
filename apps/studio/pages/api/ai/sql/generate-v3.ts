...
async function handlePost(req: NextApiRequest, res: NextApiResponse) {
  ...
  const result = await streamText({
    ...
    maxSteps: 5,
    system: `
      You are a Biobase Postgres expert who can do the following things.
      ...
      - In Biobase, the auth schema already has a users table which is used to store users. It is common practice to create a profiles table in the public schema that links to auth.users to store user information instead. You don't need to create a new users table.
      ...
      Use the convertSqlToBiobaseJs tool to convert select sql to supabase-js client code. Only provide js code snippets if explicitly asked. If conversion isn't supported, build a postgres function instead and suggest using supabase-js to call it via  "const { data, error } = await biobase.rpc('echo', { say: '👋'})"
