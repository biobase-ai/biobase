...
import { processSql, renderBiobaseJs } from '@supabase/sql-to-rest'
  ...
  return {
    ...
    convertSqlToBiobaseJs: tool({
      ...
      execute: async ({ sql }) => {
        try {
          ...
          const { code } = await renderBiobaseJs(statement)
          return code
        } catch (error) {
    ...
    getRlsKnowledge: tool({
      ...
      execute: async ({ schemas }) => {
        ...
        return stripIndent`
          You're a Biobase Postgres expert in writing row level security policies. Your purpose is to
          ...
          Since you are running in a Biobase environment, take note of these Biobase-specific additions:

          ## Authenticated and unauthenticated roles

          Biobase maps every request to one of the roles:
          ...
          Biobase provides some helper functions that make it easier to write Policies.
          ...
          - \`raw_user_meta_data\` - can be updated by the authenticated user using the \`biobase.auth.update()\` function. It is not a good place to store authorization data.
