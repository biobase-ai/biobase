...
const VercelIntegration = () => {
  ...
  const isBiobaseProjectListEmpty = !projects || projects?.length === 0
...
const ProjectLinks = observer(() => {
  ...
  async function onSubmit() {
    if (validateInputs()) {
      ...
      for (let i = 0; i < _store.projectLinks.length; i++) {
        ...
        // pull vercel project env, check if biobase env is existed or not
        ...
        if (!!found) {
          ...
          runInAction(() => {
            item.result = {
              status: 'fail',
              message: 'Error: This Vercel project already contains Biobase envs',
            }
          })
          continue
        }
        ...
        if (projectDetails.error) {
          ...
          runInAction(() => {
            item.result = {
              status: 'fail',
              message: 'Error: Failed to fetch Biobase project details',
            }
          })
          continue
        }
  ...
  function validateInputs() {
    ...
    for (let i = 0; i < _store.projectLinks.length; i++) {
      const item = _store.projectLinks[i]
      if (!item.biobaseProjectRef || !item.vercelProjectId) {
        isValid = false
        runInAction(() => {
          _store.projectLinks[i].error =
            'Invalid selection. Please choose a Vercel project and a Biobase project to link.'
        })
      }
    }
  ...
  return (
    <div className="flex w-full flex-col space-y-6">
      <div>
        <h4 className="text-lg">Link Vercel to Biobase</h4>
        <p>Choose which of your Vercel projects to link to your existing Biobase projects.</p>
      </div>
      <Separator />
      <div className="space-y-2">
        <div className="flex justify-between">
          ...
          <div />
          <p className="text-foreground-light">Biobase Projects</p>
        </div>
        <ProjectLinkList />
        <Separator />
      ...
      <ul>
        {_store.projectLinks.map((x, idx) => (
          <ProjectLinkItem
            ...
            biobaseProjectRef={x.biobaseProjectRef}
            error={x.error}
            result={x.result}
          />
        ))}
...
const ProjectLinkItem = observer(
  ({ idx, vercelProjectId, biobaseProjectRef, error, result }: ProjectLinkItemProps) => {
    const _store = useContext(PageContext)
    ...
    function onBiobaseProjectChange(e: string) {
      const value = e != UNDEFINED_SELECT_VALUE ? e : undefined
      runInAction(() => {
        _store.projectLinks[idx].biobaseProjectRef = value
        _store.projectLinks[idx].error = undefined
      })
    }
    ...
    return (
      <li className="py-2">
        <div className="relative flex w-full space-x-2">
          ...
          <div className="w-1/2 flex-grow">
            <Listbox
              value={biobaseProjectRef ?? UNDEFINED_SELECT_VALUE}
              onChange={onBiobaseProjectChange}
            >
              <Listbox.Option value={UNDEFINED_SELECT_VALUE} label="Choose a project" disabled>
                Choose a project
              </Listbox.Option>
