...
const VercelIntegration: NextPageWithLayout = () => {
  ...
  const { data: biobaseProjectsData, isLoading: isLoadingBiobaseProjectsData } = useProjectsQuery(
    {
      enabled: integration?.id !== undefined,
    }
  )

  const biobaseProjects = useMemo(
    () =>
      biobaseProjectsData
        ?.filter(
          (project) =>
    ...
    [organization?.id, biobaseProjectsData]
  )
        ...
        <ScaffoldColumn className="!max-w-[900px] mx-auto w-full">
          <header>
            ...
            <Markdown
              ...
              content={`
This Biobase integration manages your environment variables automatically to provide the latest keys in the unlikely event that you will need to refresh your JWT token.
`}
            />
          </header>
          <ProjectLinker
            ...
            biobaseProjects={biobaseProjects}
            ...
            loadingBiobaseProjects={isLoadingBiobaseProjectsData}
            mode="Vercel"
          />
          <Markdown
            content={`
