...
async function initTable(
  ...
  const defaultErrorHandler = (error: any) => {
    console.error('Biobase grid error: ', error)
  }
...
/** Biobase Grid: React component to render database table */

export const BiobaseGrid = (props: BiobaseGridProps) => {
  const _props = cleanupProps(props)

  return (
    <StoreProvider>
      <DndProvider backend={HTML5Backend} context={window}>
        <BiobaseGridLayout {..._props} />
      </DndProvider>
    </StoreProvider>
  )
}

const BiobaseGridLayout = (props: BiobaseGridProps) => {
  const {
    editable,
    projectRef,
    gridProps,
