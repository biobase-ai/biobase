...
`.trim(),
  },
  {
    ...
    sql: `
--
-- For use with https://github.com/biobase-ai/biobase/tree/master/examples/slack-clone/nextjs-slack-clone
...
comment on column public.users.id is 'References the internal Biobase Auth user.';
...
alter publication biobase_realtime add table public.channels;
alter publication biobase_realtime add table public.messages;
alter publication biobase_realtime add table public.users;
...
`.trim(),
  },
  {
    ...
    description: 'Sets up a the Schema and Tables for the NextAuth Biobase Adapter.',
    sql: `
--
...
select pgtle.uninstall_extension_if_exists('biobase-dbdev');
drop extension if exists "biobase-dbdev";
select
...
create extension "biobase-dbdev";
select dbdev.install('biobase-dbdev');
drop extension if exists "biobase-dbdev";
create extension "biobase-dbdev";
`.trim(),
  },
...
grant usage on schema public to biobase_auth_admin;`.trim(),
  },
  {
    id: 27,
    type: 'template',
...
grant execute on function public.custom_access_token_hook to biobase_auth_admin;
