...
const EditHookPanel = ({ visible, selectedHook, onClose }: EditHookPanelProps) => {
  ...
  const isEdgeFunction = (url: string) =>
    url.includes(`https://${ref}.functions.biobase.${restUrlTld}/`) ||
    url.includes(`https://${ref}.biobase.${restUrlTld}/functions/`)
  ...
  const validate = (values: any) => {
    ...
    } else if (values.function_type === 'biobase_function') {
      // For Biobase Edge Functions
  ...
  const onSubmit = async (values: any) => {
    ...
    const payload: any = {
      ...
      function_schema: 'biobase_functions',
      function_args: [
        values.http_url,
        values.http_method,
...
const FormContents = ({
  ...
  useEffect(() => {
    if (values.function_type === 'http_request') {
      ...
      const defaultFunctionUrl = `https://${projectRef}.biobase.${restUrlTld}/functions/v1/${fnSlug}`
      const updatedValues = {
        ...values,
