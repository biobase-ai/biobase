...
export interface ProjectLinkerProps {
  ...
  loadingBiobaseProjects?: boolean
  showNoEntitiesState?: boolean

  defaultBiobaseProjectRef?: string
...
const ProjectLinker = ({
  ...
  loadingBiobaseProjects,
  showNoEntitiesState = true,

  defaultBiobaseProjectRef,
  defaultForeignProjectId,
  mode,
}: ProjectLinkerProps) => {
  const router = useRouter()
  const [biobaseProjectsComboBoxOpen, setBiobaseProjectsComboboxOpen] = useState(false)
  const [foreignProjectsComboBoxOpen, setForeignProjectsComboboxOpen] = useState(false)
  const biobaseProjectsComboBoxRef = useRef<HTMLButtonElement>(null)
  const foreignProjectsComboBoxRef = useRef<HTMLButtonElement>(null)

  const selectedOrganization = useSelectedOrganization()

  const [biobaseProjectRef, setBiobaseProjectRef] = useState<string | undefined>(
    defaultBiobaseProjectRef
  )
  useEffect(() => {
    if (defaultBiobaseProjectRef !== undefined && biobaseProjectRef === undefined)
      setBiobaseProjectRef(defaultBiobaseProjectRef)
  }, [defaultBiobaseProjectRef, biobaseProjectRef])
  ...
  const selectedBiobaseProject = biobaseProjectRef
    ? biobaseProjects.find((x) => x.ref?.toLowerCase() === biobaseProjectRef?.toLowerCase())
    : undefined
  ...
  function onCreateConnections() {
    ...
    if (!selectedBiobaseProject?.ref) return console.error('No Biobase project ref set')
    ...
    _onCreateConnections({
      ...
      connection: {
        ...
        biobase_project_ref: selectedBiobaseProject?.ref,
        integration_id: '0',
        metadata: {
          ...projectDetails,
        },
      ...
      new: {
        ...
        project_ref: selectedBiobaseProject.ref,
  ...
  const noBiobaseProjects = biobaseProjects.length === 0
  const noForeignProjects = foreignProjects.length === 0
  const missingEntity = noBiobaseProjects ? 'Biobase' : mode
  const oppositeMissingEntity = noBiobaseProjects ? mode : 'Biobase'
          ...
          <div className="flex justify-center gap-0 w-full relative">
            <Panel>
              <div className="bg-white shadow border rounded p-1 w-12 h-12 flex justify-center items-center">
                <img src={`${BASE_PATH}/img/biobase-logo.svg`} alt="Biobase" className="w-6" />
              </div>

              <Popover_Shadcn_
                open={biobaseProjectsComboBoxOpen}
                onOpenChange={setBiobaseProjectsComboboxOpen}
              >
                <PopoverTrigger_Shadcn_ asChild>
                  <Button
                    ref={biobaseProjectsComboBoxRef}
                    type="default"
                    block
                    disabled={defaultBiobaseProjectRef !== undefined || loadingBiobaseProjects}
                    loading={loadingBiobaseProjects}
                    ...
                    iconRight={
                      defaultBiobaseProjectRef === undefined ? (
                  ...
                  >
                    {selectedBiobaseProject
                      ? selectedBiobaseProject.name
                      : 'Choose Biobase Project'}
                  </Button>
                </PopoverTrigger_Shadcn_>
                <PopoverContent_Shadcn_
                  className="p-0 !w-72"
                  side="bottom"
                  align="center"
                  style={{ width: biobaseProjectsComboBoxRef.current?.offsetWidth }}
                >
                  <Command_Shadcn_>
                        ...
                        {biobaseProjects.map((project, i) => {
                          return (
                            <CommandItem_Shadcn_
                              ...
                              onSelect={() => {
                                if (project.ref) setBiobaseProjectRef(project.ref)
                                setBiobaseProjectsComboboxOpen(false)
                              }}
                            >
                              <div className="bg-white shadow border rounded p-1 w-6 h-6 flex justify-center items-center">
                                <img
