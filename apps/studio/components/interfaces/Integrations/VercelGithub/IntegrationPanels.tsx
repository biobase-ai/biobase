...
type HandleIconType = Integration['integration']['name'] | 'Biobase'
...
const HandleIcon = ({ type, className }: { type: HandleIconType; className?: string }) => {
  switch (type) {
    ...
    case 'Vercel':
      ...
      break
    case 'Biobase':
      return <img src={`${BASE_PATH}/img/biobase-logo.svg`} alt="Biobase" className="w-3.5"></img>
      break
...
const IntegrationConnection = React.forwardRef<HTMLLIElement, IntegrationConnectionProps>(
  (
    ...
    const project = projects?.find((project) => project.ref === connection.biobase_project_ref)
