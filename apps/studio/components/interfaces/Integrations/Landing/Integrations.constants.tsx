...
const authorBiobase = {
  name: 'Biobase',
  websiteUrl: 'https://biobase.studio',
}

const biobaseIntegrations: IntegrationDefinition[] = [
  {
    id: 'queues',
  ...
  },
  {
    id: 'vault',
    type: 'postgres_extension' as const,
    requiredExtensions: ['biobase_vault'],
    name: `Vault`,
    status: 'alpha',
    icon: ({ className, ...props } = {}) => (
    ...
    docsUrl: 'https://biobase.studio/docs',
    author: authorBiobase,
    navigation: [
      {
        route: 'overview',
        label: 'Overview',
...
const wrapperIntegrations: IntegrationDefinition[] = WRAPPERS.map((w) => {
  return {
    ...
    requiredExtensions: ['wrappers', 'biobase_vault'],
