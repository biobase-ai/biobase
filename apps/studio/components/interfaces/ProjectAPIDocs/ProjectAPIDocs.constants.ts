...
const biobase = createClient(BIOBASE_URL, process.env.BIOBASE_KEY);`,
  ...
  userManagement: {
    ...
    title: `Introduction`,
    description: `Biobase makes it easy to manage your users.

  Biobase assigns each user a unique ID. You can reference this ID anywhere in your database. For example, you might create a \`profiles\` table references the user using a \`user_id\` field.

  Biobase already has built in the routes to sign up, login, and log out for managing users in your apps and websites.`,
    js: undefined,
    bash: undefined,
  },
  signUp: {
  ...
  After they have signed up, all interactions using the Biobase client will be performed as "that user".`,
    js: (apikey?: string, endpoint?: string) => `
const { data, error } = await biobase.auth.signUp({
...
After they have logged in, all interactions using the Biobase JS client will be performed as "that user".`,
    js: (apikey?: string, endpoint?: string) => `
const { data, error } = await biobase.auth.signInWithPassword({
  email: 'someone@email.com',
  password: 'some-secure-password'
})
    `,
...
After they have clicked the link, all interactions using the Biobase JS client will be performed as "that user".`,
    js: (apikey?: string, endpoint?: string) => `
const { data, error } = await biobase.auth.signInWithOtp({
  email: 'someone@email.com'
})
    `,
    bash: (apikey?: string, endpoint?: string) => `
...
You must enter your own twilio credentials on the auth settings page to enable SMS-based OTP verification.`,
    js: (apikey?: string, endpoint?: string) => `
const { data, error } = await biobase.auth.verifyOtp({
  phone: '+13334445555',
  token: '123456',
  type: 'sms'
})
...
const { data, error } = await biobase.auth.signInWithOAuth({
  ...
  user: {
    ...
    js: (apikey?: string, endpoint?: string) => `
const { data: { user } } = await biobase.auth.getUser()
    `,
    bash: (apikey?: string, endpoint?: string) => `
...
-H "Authorization: Bearer USER_TOKEN"
  ...
  forgotPassWordEmail: {
    ...
    js: (apikey?: string, endpoint?: string) => `
const { data, error } = await biobase.auth.resetPasswordForEmail(email)
    `,
    bash: (apikey?: string, endpoint?: string) => `
...
`,
  ...
  updateUser: {
    ...
    js: (apikey?: string, endpoint?: string) => `
const { data, error } = await biobase.auth.updateUser({
  email: "new@email.com",
...
`,
  ...
  logout: {
    ...
    category: 'user-management',
    title: `Log out`,
    description: `After calling log out, all interactions using the Biobase JS client will be "anonymous".`,
    js: (apikey?: string, endpoint?: string) => `
...
This endpoint requires you use the \`service_role_key\` when initializing the client, and should only be invoked from the server, never from the client.`,
    js: (apikey?: string, endpoint?: string) => `
const { data, error } = await biobase.auth.api.inviteUserByEmail('someone@email.com')
    `,
    bash: (apikey?: string, endpoint?: string) => `
...
-d '{
  ...
  storage: {
    ...
    category: 'storage',
    title: `Introduction`,
    description: `Biobase Storage makes it simple to upload and serve files of any size, providing a robust framework for file access controls.

You can use Biobase Storage to store images, videos, documents, and any other file type. Serve your assets with a global CDN to reduce latency from over 285 cities globally. Biobase Storage includes a built-in image optimizer, so you can resize and compress your media files on the fly.`,
    js: undefined,
    bash: undefined,
  },
  // Edge functions
...
`,
  ...
  createEdgeFunction: {
    ...
    description: `
Create a Biobase Edge Function locally via the Biobase CLI.
`,
    js: () => `// Create an edge function via the Biobase CLI`,
    bash: () => `
biobase functions new hello-world
`,
  },
  deployEdgeFunction: {
    ...
    description: `
Deploy a Biobase Edge Function to your Biobase project via the Biobase CLI.
`,
    js: () => `// Deploy an edge function via the Biobase CLI`,
    bash: () => `biobase functions deploy hello-world --project-ref [ref]
`,
  },
  // Entities
...
`,
  ...
  graphql: {
    ...
    description: `
If you have a GraphQL background, you might be wondering if you can fetch your data in a single round-trip. The answer is yes! The syntax is very similar. This example shows how you might achieve the same thing with Apollo GraphQL and Biobase.

Still want GraphQL?
If you still want to use GraphQL, you can. Biobase provides you with a full Postgres database, so as long as your middleware can connect to the database then you can still use the tools you love. You can find the database connection details [in the settings](/project/[ref]/settings/database).
`,
    js: (apikey?: string, endpoint?: string) => `
// With Apollo GraphQL
...
Removing a channel is a great way to maintain the performance of your project's Realtime service as well as your database if you're listening to Postgres changes. Biobase will automatically handle cleanup 30 seconds after a client is disconnected, but unused channels may cause degradation as more clients are simultaneously subscribed.
`,
    js: () => `biobase.removeChannel(myChannel)`,
    bash: () => `# Realtime streams are only supported by our client libraries`,
  },
  unsubscribeChannels: {
...
`,
    js: () => `const channels = biobase.getChannels()`,
    bash: () => `# Realtime streams are only supported by our client libraries`,
  },
}
...
-H "Authorization: Bearer ${apikey}"
          `,
          js: `
let { data: ${resourceId}, error } = await biobase
  .from('${resourceId}')
  .select('*')
          `,
        },
      ...
      return [
        {
          ...
          bash: `# Realtime streams are only supported by our client libraries`,
          js: `
const channels = biobase.channel('custom-all-channel')
  .on(
    'postgres_changes',
  ...
  .subscribe()`,
        },
        {
          ...
          bash: `# Realtime streams are only supported by our client libraries`,
          js: `
const channels = biobase.channel('custom-insert-channel')
  .on(
    'postgres_changes',
  ...
  .subscribe()`,
        },
        {
          ...
          bash: `# Realtime streams are only supported by our client libraries`,
          js: `
const channels = biobase.channel('custom-update-channel')
  .on(
    'postgres_changes',
  ...
  .subscribe()`,
        },
        {
          ...
          bash: `# Realtime streams are only supported by our client libraries`,
          js: `
const channels = biobase.channel('custom-delete-channel')
  .on(
    'postgres_changes',
  ...
  .subscribe()`,
        },
        {
          ...
          bash: `# Realtime streams are only supported by our client libraries`,
          js: `
const channels = biobase.channel('custom-filter-channel')
  .on(
    'postgres_changes',
