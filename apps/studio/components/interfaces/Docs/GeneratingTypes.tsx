...
export default function GeneratingTypes({ selectedLang }: Props) {
  ...
  const onClickGenerateTypes = async () => {
    try {
      ...
      element.setAttribute('download', 'biobase.ts')
    ...
    <>
      ...
      <div className="doc-section">
        <article className="code-column text-foreground">
          <p>
            Biobase APIs are generated from your database, which means that we can use database
...
`,
  ...
  generateTypes: (ref: string) => ({
    ...
    bash: {
      code: `
npx biobase gen types typescript --project-id "${ref}" --schema public > types/biobase.ts
`,
    },
  }),
}
