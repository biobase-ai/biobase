...
const SidePanelGitHubRepoLinker = ({ projectRef }: SidePanelGitHubRepoLinkerProps) => {
  ...
  /**
   * Biobase projects available
   */
  const { data: biobaseProjectsData, isLoading: isLoadingBiobaseProjects } = useProjectsQuery()
  ...
  const biobaseProjects = useMemo(
    ...
    [selectedOrganization?.id, biobaseProjectsData]
  )
          ...
          {gitHubAuthorization === null ? (
            <div className="flex flex-col items-center justify-center mt-8 relative border rounded-lg p-12 bg shadow px-20s">
              <p className="text-sm text-center">
                Connect your Biobase projects with your GitHub repositories
              </p>
            ...
            <ProjectLinker
              defaultBiobaseProjectRef={projectRef}
              ...
              loadingBiobaseProjects={isLoadingBiobaseProjects}
