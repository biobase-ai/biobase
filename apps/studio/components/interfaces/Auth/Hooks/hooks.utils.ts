...
export const getRevokePermissionStatements = (schema: string, functionName: string): string[] => {
  return [
    `-- Revoke access to function from biobase_auth_admin\nrevoke execute on function ${schema}.${functionName} from biobase_auth_admin;`,
    `-- Revoke access to schema from biobase_auth_admin\nrevoke usage on schema ${schema} from biobase_auth_admin;`,
