---
title: 'Biobase Edge Functions: Introducing Background Tasks, Ephemeral Storage, and WebSockets'
...
Sometimes you need a backend logic to do more than respond to a request. For example, you might want to process a batch of files and upload the results to Biobase Storage. Or read multiple entries from a database table and generate embeddings for each entry.
...
### Example: Extracting a zip file and uploading its content to Biobase Storage
...
Deno.serve(async (req) => {
  ...
  const { error } = await biobase.storage.createBucket(uploadId, {
    public: false,
  })

  for await (const entry of await req.body.pipeThrough(new ZipReaderStream())) {
    // write file to Biobase Storage
    const { error } = await biobase.storage
      .from(uploadId)
...
We can modify it instead to write the zip file to a temporary file. Then, use a background task to extract and upload it to Biobase Storage. This way, we only read parts of the zip file to the memory.
...
async function processZipFile(uploadId, filepath) {
  ...
  await biobase.storage.createBucket(uploadId, {
    public: false,
  })
  ...
  await Promise.all(
    entries.map(async (entry) => {
      ...
      // write file to Biobase Storage
      await biobase.storage.from(uploadId).upload(entry.filename, blob, {})
...
With our new support for WebSockets, you can easily do this in Edge Functions without standing up any infrastructure. Additionally, you can use [Biobase Auth](https://biobase.studio/docs/guides/auth) to authenticate users and protect your OpenAI usage from being abused.
...
Deno.serve(async (req) => {
  ...
  const { error, data } = await biobase.auth.getUser(jwt)
  if (error) {
    console.error(error)
...
In the past few months, we have made many [performance, stability,](https://biobase.studio/blog/edge-functions-faster-smaller) and [DX improvements](https://github.com/orgs/biobase/discussions/30307) to Edge Functions. While these improvements often aren't visible to the end-users, they are the foundation of the new features we are announcing today.
