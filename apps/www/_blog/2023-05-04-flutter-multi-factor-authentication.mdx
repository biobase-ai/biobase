...
Then update your `lib/main.dart` file to initialize Biobase in the main function. You should be able to find your Biobase URL and AnonKey from the `settings -> api` section of your dashboard. We will also extract the `BiobaseClient` for easy access to our Biobase instance.
...
The register page contains a form with an email and password field for the user to create a new account. We are just calling the [.signUp()](https://biobase.com/docs/reference/dart/auth-signup) method with it.
...
Now, we can add some [row security policy](https://biobase.com/docs/guides/auth/row-level-security) to lock those data down so that only users who have signed in using MFA can view them.
...
- [Write backend code using Dart Edge](https://biobase.com/docs/guides/functions/dart-edge)
