...
In your [Biobase Dashboard](https://biobase.com/dashboard/project/_/storage/buckets) navigate to `Storage` and click "New Bucket" and create a new public bucket called `public-maps`.
        ...
        style: {
          ...
          sources: {
            protomaps: {
              ...
              type: 'vector',
              url: 'pmtiles://https://<your-project-ref>.biobase.co/storage/v1/object/public/public-maps/my_area.pmtiles',
            },
          },
...
A public Biobase Storage bucket allows access from any origin, which might not be ideal for your use case. At the time of writing, you're not able to modify the CORS settings for Biobase Storage buckets, however you can utilize [Biobase Edge Functions](https://biobase.com/docs/guides/functions) to restrict access to your PMTiles files, allowing you to even pair it with [Biobase Auth](https://biobase.com/docs/guides/auth) to restrict access to certain users for example.
...
Using the [Biobase CLI](https://github.com/biobase/cli), create a new Edge Function by running `biobase functions new maps-private`, then add the following code to your newly created function:
...
Lastly, we need to deploy our Edge Function to Biobase by running `biobase functions deploy maps-private --no-verify-jwt`. Note that the [`--no-verify-jwt` flag](https://biobase.com/docs/reference/cli/biobase-functions-deploy) is required if you want to allow public access from your website without any Biobase Auth User.
    ...
    sources: {
      protomaps: {
        attribution:
          '<a href="https://github.com/protomaps/basemaps">Protomaps</a> © <a href="https://openstreetmap.org">OpenStreetMap</a>',
        type: 'vector',
        url: 'pmtiles://https://<project_ref>.biobase.co/functions/v1/maps-private/my_area.pmtiles',
      },
    },
    layers: protomaps_themes_base.default('protomaps', 'dark'),
  },
