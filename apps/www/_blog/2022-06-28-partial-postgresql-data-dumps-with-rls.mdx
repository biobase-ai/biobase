...
| `id` | `name`       | `email`                  |
| ---- | ------------ | ------------------------ |
| `1`  | `Employee 1` | `employee1@biobase.com` |
| `2`  | `Employee 2` | `employee2@biobase.com` |
| `3`  | `Employee 3` | `employee3@biobase.com` |
...
```sql hideCopy
-- Only dump data for biobase employees
create policy "Data dump rule" on profiles
  for select
  to exporter
  using (
    substring(email from '@(.*)$') = 'biobase.com'
  );
...
[Snaplet](https://docs.snaplet.dev/tutorials/biobase-clone-environments).
