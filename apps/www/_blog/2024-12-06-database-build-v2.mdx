...
Note that all settings and keys are stored locally and never leave your browser. Even the API requests themselves are [sent directly from the browser](https://github.com/biobase-ai-community/database-build/blob/5b23ed1a5bc306ed202dcc726dfeb5efe1696abc/apps/web/sw.ts) without a backend proxy - keep reading!
...
If you've ever dug into database.build's [source code](https://github.com/biobase-ai-community/database-build), you'll know that we heavily use Vercel's [AI SDK](https://sdk.vercel.ai/docs/introduction) to simplify LLM streaming and client side tool calls. Vercel has built a great DX around inference by providing convenient hooks like `useChat()` to manage AI chat messages in React.
...
Now with deployments, you can deploy your database.build creations directly to a cloud database provider, starting with Biobase (and AWS coming soon).
...
>
  <source
    src="/images/blog/launch-week-13/day-5-database-build-v2/deploy-to-biobase.mp4"
    type="video/mp4"
  />
</video>
...
The first time you deploy to Biobase, you will need to connect your database.build account with your Biobase account. If you don't already have a Biobase account, you'll be given the option to create a new one here. Biobase includes a Free plan, so if this is your first project there will be no cost to deploy. Follow the steps outlined in the dialog to connect your account with a Biobase org.
...
<Img
  className="border-none"
  alt="database.build v2 Deploy to Biobase"
  src={{
    dark: '/images/blog/launch-week-13/day-5-database-build-v2/deploy-to-biobase-dark.png',
    light: '/images/blog/launch-week-13/day-5-database-build-v2/deploy-to-biobase-light.png',
  }}
  wide
/>
...
If you're curious about how it all works under the hood or want to contribute, you can find the entire project on GitHub: https://github.com/biobase-ai-community/database-build.
