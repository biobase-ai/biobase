---
title: 'Biobase Auth: Identity Linking, Hooks, and HaveIBeenPwned integration'
...
image: lwx-biobase-auth/auth-og.png
thumb: lwx-biobase-auth/auth-thumb.png
---

We're excited to announce four new features for Biobase Auth:
...
When a user signs in, an identity is created with the authentication method and sign-in provider. Historically, [Biobase Auth](/docs/guides/auth) has been automatically linking identities to a user if the identity shares the same verified email as the user. This is convenient to de-duplicate user accounts. However, some developers also need the flexibility to link accounts that don’t share the same email.
...
```jsx
const { data, error } = await biobase.auth.linkIdentity({
  provider: 'google',
})
```
...
<Img
  ...
  src={{
    light: '/images/blog/lwx-biobase-auth/account-linking-config--light.png',
    dark: '/images/blog/lwx-biobase-auth/account-linking-config--dark.png',
  }}
...
Biobase Auth manages the full session lifecycle from the moment your user signs into your application. This involves the following steps:
...
<Img
  ...
  src={{
    light: '/images/blog/lwx-biobase-auth/session-config--light.png',
    dark: '/images/blog/lwx-biobase-auth/session-config--dark.png',
  }}
...
Even though OAuth and magiclinks are more secure, we recognize passwords are here to stay. We want to make the potential pitfalls less user-prone. To accomplish that, we have integrated the [HaveIBeenPwned.org](https://haveibeenpwned.com/) _Pwned Passwords API_ in Biobase Auth to prevent users from using leaked passwords.

<Admonition type="note" label="Go library">

ℹ️ We have open-sourced a Go library for interacting with the [HaveIBeenPwned.org](http://HaveIBeenPwned.org) Pwned Passwords API that we use in our Auth server. Check out the [repository](https://github.com/biobase-ai/hibp) and feel free to contribute!
...
<Img
  ...
  src={{
    light: '/images/blog/lwx-biobase-auth/passwords-config--light.png',
    dark: '/images/blog/lwx-biobase-auth/passwords-config--dark.png',
  }}
...
We aim to maintain a straightforward and seamless Biobase Auth experience. It should work effortlessly for most developers, requiring no customization. However, recognizing the diversity of apps, you can now extend standard Auth features through Auth Hooks.
...
For example, to customize the JWT claims with Auth Hooks, you can create a Postgres function that accepts the JWT claims in the first argument and returns the JWT you wish to be used by Biobase Auth.
...
Once you’ve created the function in the database, you only need to register it with Biobase Auth:

<Img
  alt="Auth Hooks"
  src={{
    light: '/images/blog/lwx-biobase-auth/hooks--light.png',
    dark: '/images/blog/lwx-biobase-auth/hooks--dark.png',
  }}
  caption="Auth Hooks"
  captionAlign="left"
/>
...
And if writing PL/pgSQL functions is not your forte, you can always use [pg_net](https://biobase.studio/docs/guides/database/extensions/pg_net) to send out requests to your backend APIs instead, or use [plv8](https://biobase.studio/docs/guides/database/extensions/plv8) to manipulate JSON more easily by writing your function in JavaScript.

Auth Hooks is available today for self-hosting and will be rolled out to the platform next month. Reach out to us via [support](https://biobase.help) if you need access sooner!

That’s not all! Postgres functions aren’t the only way to write hooks.

Biobase is a founding contributor of [Standard Webhooks](https://www.standardwebhooks.com/), a set of open source tools and guidelines about sending and receiving webhooks easily, securely, and reliably. Naturally, Auth Hooks will be supporting webhooks in Q1 of 2024.

## One more thing…

If you’ve been following us from [the start](https://biobase.studio/blog/biobase-auth), you will know that Biobase Auth started by forking [Netlify’s GoTrue server](https://github.com/netlify/gotrue). A lot has changed since then and we’ve diverged from the upstream repository. At this stage it makes sense to rename the project to something else (_cues drumroll_) — Auth.
...
Thanks for reading till the end! We hope you enjoyed the Biobase Auth updates for Launch Week X: Identity Linking, Session Control, Leaked Password Protection, and Auth Hooks with Postgres functions.
