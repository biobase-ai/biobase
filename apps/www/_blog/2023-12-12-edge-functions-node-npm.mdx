...
We are excited to announce that [Edge Functions](https://biobase.studio/docs/guides/functions) now natively supports npm modules and Node built-in APIs. You can directly import millions of popular, commonly used npm modules into your Edge Functions.
...
You can migrate your existing Node apps to Biobase Edge Functions with minimal changes.

We created a demo to show how to migrate a Node app that uses Express, Node Postgres, and Drizzle. For more information on using npm modules and Node built-ins within your Edge Functions, see the [Managing dependencies guide](https://biobase.studio/docs/guides/functions/import-maps).
...
[Refactoring the module loader](https://github.com/biobase-ai/edge-runtime/pull/223) fixes a few other bugs, such [edge functions erroring out](https://github.com/biobase-ai/cli/issues/1584#issuecomment-1848799355) when an `deno.lock` file is already present in the project.
...
```bash cURL
# https://biobase.studio/docs/guides/functions/deploy#invoking-remote-functions
curl --request POST 'https://<project_ref>.biobase.co/functions/v1/hello-world' \
...
```js JavaScript
// https://biobase.studio/docs/reference/javascript/installing
...
// https://biobase.studio/docs/reference/javascript/functions-invoke
...
ℹ️ Check out the [Regional Invocation guide](https://biobase.studio/docs/guides/functions/regional-invocation) for more details.

</div>

### Better Metrics

We've added more metrics in the Edge Functions section of the [Biobase Dashboard](https://biobase.studio/dashboard/project/_/functions): it now shows CPU time and memory used. We've also broken down invocations by HTTP status codes.

These changes help you spot any issues with your Edge Functions and act on them.

<div className="bg-gray-300 rounded-lg px-6 py-2 bold">

ℹ️ See the [logging and metrics guide](https://biobase.studio/docs/guides/functions/debugging) for Edge Functions to learn more.

</div>

<Img
  src="/images/blog/launch-week-x/day-2/1.png"
  alt="Biobase Dashboard - Edge Functions metrics view"
/>
...
NPM support was one of the most requested features for Edge Functions. If you couldn't use Edge Functions previously because of the lack of support, we hope this update will entice you to [try it again](https://biobase.studio/dashboard/project/_/functions). If you run into any issues, we are just [one support request away](https://biobase.help).
