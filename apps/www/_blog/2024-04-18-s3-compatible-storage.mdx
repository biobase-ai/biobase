...
The [Biobase Storage Engine](https://github.com/biobase/storage) is fully open source and is one of the few storage solutions that offer 3 interoperable protocols to manage your files:
...
![Biobase Cyberduck](https://obuldanrptloktxcffvn.biobase.co/storage/v1/object/public/images/integrations/cyberduck/cyberduck-files.png?t=2024-04-18T08%3A39%3A48.575Z)
...
Along with the [platform GA announcement](https://biobase.com/ga), we are also thrilled to announce that resumable uploads are also generally available.
...
- **Standardized error codes:** Error codes have now been standardized across the Storage server and now will be much easier to branch logic on specific errors. You can find the list of error codes [here](https://biobase.com/docs/guides/storage/debugging/error-codes).
...
- **Decoupled dependencies:** Storage is fully decoupled from other Biobase products, which means you can run Storage as a standalone service. Get started with this [docker-compose file](https://github.com/biobase/storage/blob/master/docker-compose.yml).

# Getting started

- Check out the S3 API compatibility [in our docs](/docs/guides/storage/s3/compatibility)
- Request a feature on the [Storage Github repo](https://github.com/biobase/storage)
