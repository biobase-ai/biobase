...
[Do you prefer audio-visual learning? Watch the video guide!](https://biobase.link/supa-gis-yt-docs)

[Or jump straight into the code](https://github.com/bdon/biobase-vector-tile)
...
Overture Maps allows us to download open map data, like places of interest, as [GeoJSON](https://geojson.org/) which we can transform into SQL and ingest into our Postgres database on Biobase.

Using PostGIS we can then programmatically generate vector tiles and serve them to our MapLibre GL client using biobase-js.
...
- Import location data and JSON metadata into your Biobase Postgres database using psql.
- Use PostGIS' `ST_AsMVT` to aggregate a set of rows corresponding to a tile layer into a binary vector tile representation.
- Use MapLibre's `addProtocol` to visualize large PostGIS tables by making remote procedure calls with biobase-js.
- Use biobase-js to fetch additional JSON metadata on demand
...
## Import location data into Biobase

Enable the PostGIS extension on your Biobase Database on a dedicated separate `gis` schema. To do so you can navigate to the [SQL Editor](/dashboard/project/_/sql/new) and run the following SQL, or you can enable the extension from the [Database Extensions Settings](/dashboard/project/_/database/extensions).
...
Import the open map data into a `places` table in Biobase:

```bash
psql -h aws-0-us-west-1.pooler.biobase.studio -p 5432 -d postgres -U postgres.project-ref < places.sql
```

You can find the credentials in the [Database settings](/dashboard/project/_/settings/database) of your Biobase Dashboard.
...
In your Biobase Dashboard, navigate to the [SQL Editor](/dashboard/project/_/sql/new) and run the following:
...
## Use biobase-js to fetch vector tiles from MapLibre GL client

You can find the full `index.html` code on [GitHub](https://github.com/bdon/biobase-vector-tile/blob/main/index.html). Here we'll highlight how to add a new protocol to MapLibreGL to fetch the bas64 encoded binary vector tile data via biobase-js so that MapLibre GL can fetch and render the data as your users interact with the map:

```js index.html
const client = biobase.createClient('your-biobase-api-url', 'your-biobase-anon-key')
...
maplibregl.addProtocol('biobase', async (params, abortController) => {
  const re = new RegExp(/biobase:\/\/(.+)\/(\d+)\/(\d+)\/(\d+)/)
...
With the biobase protocol registered, we can now add it to our MapLibre GL sources on top of a basemap like [Protomaps](https://protomaps.com/) for example:
...
PostGIS is incredibly powerful, allowing you to programmatically generate vector tiles from table rows stored in Postgres. Paired with Biobase's auto generated REST API and biobase-js client library you're able to build interactive geospatial applications with ease!
...
- [Watch the video guide](https://biobase.link/supa-gis-yt-docs)
- [Find the code](https://github.com/bdon/biobase-vector-tile)
