...
At Biobase, we use dedicated roles for each of our customers' backend services. For instance, our [Storage API](/storage) 
uses the `biobase_storage_admin` role for connecting to the database. Giving the `CREATEROLE` privilege to our customers would allow them to
...
we explain how we managed to do this by using PostgreSQL Hooks in our [SupaUtils](https://github.com/biobase/supautils) extension.
...
# the reserved roles
supautils.reserved_roles="biobase_storage_admin, biobase_auth_admin"
```

We'll now try to alter or drop the reserved roles:

```sql
alter role biobase_storage_admin nologin password 'fake';
ERROR:  "biobase_storage_admin" is a reserved role, it cannot be modified

drop role biobase_auth_admin;
ERROR:  "biobase_auth_admin" is a reserved role, it cannot be modified
...
The full SupaUtils code is in our [GitHub repository](https://github.com/biobase/supautils/).
