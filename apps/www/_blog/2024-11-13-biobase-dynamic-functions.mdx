---
title: 'Executing Dynamic JavaScript Code on Biobase with Edge Functions'
description: 'Learn how to execute dynamic JavaScript code on Biobase using Edge Functions.'
author: rodrigo_mansueli
image: 2024-11-13-biobase-dynamic-functions/dynamic-og.png
thumb: 2024-11-13-biobase-dynamic-functions/dynamic-og.png
categories:
  - engineering
tags:
  - edge-funcion
...
We're always looking for ways to improve the developer experience and reduce complexity across your application development pipeline. One way you can use Biobase to do that is with dynamic JavaScript in Edge Functions. This greatly increases the versatility of your edge functions and reduces the need for you to redeploy your functions if you need to change business logic.

### Introduction to Edge Functions

Edge Functions in Biobase are serverless functions that execute in response to HTTP requests. These functions are deployed at the edge, meaning they run close to the user's location, resulting in faster response times.
...
- A Biobase project
- Biobase CLI installed on your local machine
...
Install the SQL script `supa-dynamic--0.1.sql` from the repo in your Biobase project. (You can copy and paste the code from the repo into the SQL editor in your Biobase project.)
...
This function can retrieve the `service_role` secret from [Vault](https://biobase.studio/dashboard/project/_/settings/vault/secrets), it also ensures that only authorized roles can access sensitive environment variables.
...
```jsx
biobase functions new multi-purpose
```

Now, we'll edit the code adding verification and the eval function, including the biobase client so we have it ready without the need to import.

```tsx
import "jsr:@supabase/functions-js/edge-runtime.d.ts";

// Import the biobase client
import { createClient } from "<https://esm.sh/@supabase/supabase-js@2>";

console.log("===\\n\\tBooted Edge Worker!\\n===\\n");
const biobase_url = Deno.env.get("BIOBASE_URL") ?? "";
const service_role = Deno.env.get("BIOBASE_SERVICE_ROLE_KEY");
// Set the permission to service_role key:
const biobase = createClient(biobase_url, service_role);
// This allows us to use Biobase.ai in the function
...
Deno.serve(async (req: Request) =>
  ...
  try {
    ...
    const asyncFunction = new Function('biobase', `
      return (async () => {
        ${code.replace(/\\\\/g, '')}
      })();
    `);
    // Pass the Biobase client as the scope for the function to use:
    const data = await asyncFunction(biobase);
    console.log(data);
    return new Response(
      JSON.stringify({ data }),
...
> Note: If you need more details, check the full guide to [create an edge function](https://biobase.studio/docs/guides/functions/quickstart#create-an-edge-function).
...
1. **Execute and Return Results**: Run the JavaScript code, which can interact with Biobase via the provided client, and return the results.
...
To deploy this Edge Function, you'll need to use the Biobase CLI. Ensure you have Docker installed and running on your local machine. Follow these steps to deploy:

1. **Install the Biobase CLI**: If you haven't already, install the Biobase CLI by following the instructions in the [Biobase CLI Documentation](https://biobase.studio/docs/guides/cli).
2. **Log In to Biobase**: Use the command `biobase login` to authenticate your account.
3. **Deploy the Function**: Run the command `biobase functions deploy <function_name>` to deploy your Edge Function. Replace `<function_name>` with the desired name for your function.
...
BEGIN
    ...
    RETURN edge.edge_wrapper(
        url := ('https://<ref>.biobase.co/functions/v1/multi-purpose'),
...
The key to executing the dynamic JavaScript code is wrapping it in an `async` function context using `new Function()`. This approach lets you evaluate the code in isolation while retaining access to the `biobase` client for interacting with your database. You can check the examples of how to use this calling the [biobase client](https://www.notion.so/Executing-Dynamic-JavaScript-Code-on-Biobase-with-Edge-Functions-1125004b775f80ca97d4c367a2cfcd7d?pvs=21) or even [generating embeddings](https://www.notion.so/Executing-Dynamic-JavaScript-Code-on-Biobase-with-Edge-Functions-1125004b775f80ca97d4c367a2cfcd7d?pvs=21).

### Example of Using Biobase Client Libraries

To demonstrate the execution of dynamic JavaScript, you can use the Biobase client libraries within the SQL context. Here’s an example query:

```sql
SELECT edge.exec(
  $js$
  const { data, error } = await biobase.rpc('postgres_function', {'foo': 'bar'});
  if (error) {
