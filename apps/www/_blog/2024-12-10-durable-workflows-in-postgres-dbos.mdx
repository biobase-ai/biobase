...
description: Technical deep dive into the new DBOS integration for Biobase
...
The DBOS team have released a Biobase integration, so you can use your Postgres database as a durable workflow engine.

<Img
  alt="DBOS Biobase integration"
  src={{
...
I really love the design of DBOS, so I'm going to write more below. Their design is aligned with our philosophy at Biobase: “just use Postgres”. I'll take you through the lower-level details in the rest of this post. If you just want to get started using DBOS with Biobase, get started using their tutorial:

[Use DBOS With Biobase →](https://docs.dbos.dev/integrations/biobase)
...
DBOS is a platform where you can write your programming logic in serverless functions (similar to Biobase Edge Functions). Functions can be written in either [Python](https://docs.dbos.dev/python/programming-guide) or [TypeScript](https://docs.dbos.dev/typescript/programming-guide).

### Creating workflows with decorators

One thing that's different to Biobase Edge Functions is the ability to add **decorators** to your Functions with `DBOS.step()` and `DBOS.workflow()`:
...
To get started with DBOS and Biobase, check out their official integration docs:
