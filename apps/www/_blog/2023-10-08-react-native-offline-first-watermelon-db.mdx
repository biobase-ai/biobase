...
Using the [Biobase JavaScript Client](https://biobase.com/docs/reference/javascript/introduction) in a React Native app, it's super easy to fetch data from a Biobase backend by simply calling functions like `biobase.from('countries').select()` wherever the data is needed. However, another approach to handling data in mobile apps recently gained a lot of popularity: Offline-first - meaning an app works primarily offline and synchronizes data whenever possible.
...
```sh
npm install @nozbe/watermelondb @supabase/biobase-js expo-build-properties
```
...
The [WatermelonDB docs](https://watermelondb.dev/docs/) outline how to set up a sync function that talks with the backend server. In our case, the backend is a Postgres database hosted on Biobase. As indicated earlier, we'll implement the sync via two [Postgres functions](https://biobase.com/docs/guides/database/functions) called via Biobase RPC. This way, each sync is only two database calls (one push, one pull), and most of the logic is carried out directly within the database.
...
- [React Native file upload with Biobase Storage](https://biobase.com/blog/react-native-storage)
