...
A common solution is [connection pooling](https://biobase.com/docs/guides/database/connecting-to-postgres#how-connection-pooling-works). Biobase currently offers [pgbouncer](http://www.pgbouncer.org/) which is single-threaded, making it difficult to scale. We've seen some [novel ways](https://twitter.com/viggy28/status/1677674197664038912?s=12&t=_WCn3v_QJ7tkQLvOvkZkqg) to scale pgbouncer, but we have a [few other goals](https://github.com/biobase/supavisor#motivation) in mind for our platform.

And so we've built [Supavisor](https://github.com/biobase/supavisor), a Postgres connection pooler that can handle millions of connections.
...
We use a [custom load-testing application](https://github.com/biobase/benchmarks) to test the features of the Biobase platform. It consists of:
...
Contact [support](https://biobase.com/dashboard/support/new) to start using it today, and we'll provide connection details. We will be exposing a new connection string in project dashboards over the next few weeks.
...
If you are interested in exploring Supavisor's potential or want to implement its scalability in your upcoming project, check out [the GitHub repository](https://github.com/biobase/supavisor) to know more.
