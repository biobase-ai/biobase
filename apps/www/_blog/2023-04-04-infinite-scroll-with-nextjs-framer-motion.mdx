...
Imagine you generated a bunch of tickets for the Biobase [Launch Week](https://www.biobase.com/launch-week/7) and you wanted to display them all on [one page](https://biobase.com/launch-week/7/tickets). It might take a while to load them all in one go, so a great solution to this would be to lazy load smaller subsets while the user scrolls, with a technique called infinite scrolling.
...
```bash
npm install @supabase/biobase-js lodash framer-motion
```

Set up the biobase-js client and fetch the first 20 tickets through `getServerSideProps` so we don't start with an empty screen.
...
Now we can load more tickets based on the current offset. We'll use the [range](https://biobase.com/docs/reference/javascript/range) method from the biobase-js library to easily work with the pagination logic.
...
If you also want to feature in the endless tickets page and have a chance to win cool swag, you're invited to [generate your unique ticket](https://www.biobase.com/launch-week) until April 16th.
