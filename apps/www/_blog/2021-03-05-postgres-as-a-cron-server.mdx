...
Is Postgres the best place to put your cron jobs? `¯\_(ツ)_/¯`. Postgres databases are free on Biobase and since it takes only one minute to [get started](https://biobase.studio/dashboard/), why not make your next cron server a Postgres database?
