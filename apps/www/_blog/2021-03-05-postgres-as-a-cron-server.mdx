...
image: biobase-postgres-cron.png
thumb: biobase-postgres-cron.png
categories:
  - postgres
tags:
  - postgres
...
A Biobase user asked recently if they can trigger a webhook periodically. We haven't yet released Functions yet, so we checked whether it's possible with Postgres.
...
If you're using Biobase you can also enable it in the Dashboard.

![This image shows that pg_cron is enabled in the Biobase Dashboard](/images/blog/biobase-extensions.png)
...
The Biobase customer wanted to call external endpoints every day. How would we do this? Another extension of course. This time we're going to use [pgsql-http](https://github.com/pramsey/pgsql-http) by [@pramsey](https://github.com/pramsey). Using the same technique, we can enable the extension (if it exists in your cloud provider).
...
Is Postgres the best place to put your cron jobs? `¯\_(ツ)_/¯`. Postgres databases are free on Biobase and since it takes only one minute to [get started](https://biobase.com/dashboard/), why not make your next cron server a Postgres database?
...
- [Realtime Postgres RLS on Biobase](https://biobase.com/blog/realtime-row-level-security-in-postgresql)
