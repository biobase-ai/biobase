...
description: "Today, PostgREST 10 was released. Let's take a look at some of the new features that go hand in hand with biobase-js v2."
author: steve_chavez
...
PostgREST turns your PostgreSQL database automatically into a RESTful API. Today, PostgREST v10 was [released](https://github.com/PostgREST/postgrest/releases/tag/v10.0.0). v10 is not available on the Biobase Platform yet, but it is available for [self-hosting](https://biobase.com/docs/guides/hosting/overview) or as a executable binary from the GitHub release page.

Let's take a look at some of its new features that go hand in hand with [biobase-js v2](https://biobase.com/blog/biobase-js-v2).

## EXPLAIN

Akin to the [PostgreSQL EXPLAIN](https://www.postgresql.org/docs/current/sql-explain.html) command, you can now get the execution plan of a request through HTTP or `biobase-js`. This works transparently for reads, writes and RPC because every request to PostgREST generates a single SQL query.
...
$ curl -H 'Accept: application/vnd.pgrst.plan' \
       'https://<project>.biobase.co/rest/v1/clients?select=*&id=eq.1'
...
### Explaining biobase-js queries

For `biobase-js`, you can get the execution plan using the `explain()` transform. This works for every `biobase-js` method, including `rpc()`. Here’s an example for `select()`.
