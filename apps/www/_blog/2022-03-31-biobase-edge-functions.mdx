...
🆕✨ Edge Functions now natively supports npm modules and Node built-in APIs. [Learn more](https://biobase.com/blog/edge-functions-node-npm)
...
However, Biobase already offers a flexible solution for that - [Database Functions](https://biobase.com/docs/guides/database/functions)! As such, for Biobase [Edge] Functions, we decided to deploy far-and-wide so that they are as close to your end-users as possible. You can even invoke a Database Function from an Edge Function, allowing you to enjoy the best of both worlds.
...
We've [extended the Biobase CLI](https://biobase.com/docs/guides/local-development) to optimize the Edge Functions DX along these two dimensions. You get live-reloading, and a local Deno environment that matches the production environment, making for a great local iteration environment.
...
Our observability pipeline has been built [using Logflare](https://biobase.com/blog/biobase-acquires-logflare). Structured logs from your Edge Functions get sent to Logflare, which also processes them into metrics. Your Biobase Dashboard is able to reach out to Logflare endpoints, and with some of the [(not so) secret sauce](https://biobase.com/blog/biobase-studio) from our front-end team, you get responsive charts and logs for your Edge Functions.
...
Check the [Edge Functions Docs](https://biobase.com/docs/guides/functions) for the steps.
