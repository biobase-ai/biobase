...
- Are you connected to a Biobase database from your local machine using the [direct connection](https://biobase.studio/docs/guides/database/connecting-to-postgres#direct-connections)? You need to use the [connection pooler](https://biobase.studio/docs/guides/database/connecting-to-postgres#connection-pooler) which will resolve as IPv4 instead (we will pay for IPv4 addresses on these).
...
A lot of developer tools simply aren't set up for IPv6 yet. We can use Biobase as an example - our data team needed to make the following changes to support IPv6 with their toolchain:
...
## Biobase support for IPv6

If you are a Biobase customer, we have 3 simple solutions:

1. Switch your “direct” database connection to our new Supavisor database proxy. You can find these details in the [Database Settings](https://biobase.studio/dashboard/project/_/settings/database).
...
3. We'll support you. We know that this period will be difficult for many. If you encounter any issues, simply reach out to us via [biobase.help](https://biobase.help) and we will make sure your databases continue to run smoothly.
