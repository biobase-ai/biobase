...
toc_depth: 3
author: thor_schaeff
image: 2023-11-16-react-native-authentication/react-native-authentication-biobase.jpg
thumb: 2023-11-16-react-native-authentication/react-native-authentication-biobase.jpg
---
...
This article assumes you are comfortable with writing a basic application in React Native. No knowledge of Biobase is required.
...
## Set up biobase-js for React Native

Using [`biobase-js`](/docs/reference/javascript/introduction) is the most convenient way of leveraging the full power of the Biobase stack as it conveniently combines all the different services (database, auth, realtime, storage, edge functions) together.

### Install biobase-js and dependencies

After you have created your [Expo project](https://docs.expo.dev/get-started/create-a-project), you can install `biobase-js` and the required dependencies using the following command:

```bash
npx expo install @supabase/biobase-js @react-native-async-storage/async-storage react-native-url-polyfill
```

### Authentication storage

By default, biobase-js uses the browser's `localStorage` mechanism to persist the user's session but can be extended with platform specific storage implementations. In React Native we can build native mobile and web applications with the same code base, so we need a storage implementation that works for all these platforms: [react-native-async-storage](https://github.com/react-native-async-storage/async-storage#supported-platforms).

We need to pass an instance of `react-native-async-storage` to biobase-js to make sure authentication works reliably across all react native platforms:
...
const biobaseUrl = YOUR_REACT_NATIVE_BIOBASE_URL
const biobaseAnonKey = YOUR_REACT_NATIVE_BIOBASE_ANON_KEY
...
You can find your URL and anon key in the [API credentials section](/dashboard/project/_/settings/api) of the Biobase dashboard.
...
Implement a `LargeSecureStore` class to pass in as Auth storage for the `biobase-js` client:
...
const biobase = createClient(biobaseUrl, biobaseAnonKey, {
  auth: {
...
Note, by default Biobase Auth requires email verification before a session is created for the users. To support email verification you need to implement deep link handling which is outlined in the next section.
...
As you saw above, we specified `detectSessionInUrl: false` when initializing biobase-js. By default, in a web based environment, biobase-js will automatically detect OAuth and magic link redirects and create the user session.
...
In your project's [auth settings](https://biobase.studio/dashboard/project/_/auth/url-configuration) add the redirect URL, e.g. `com.biobase://**`.

Finally, implement the OAuth and linking handlers. See the [biobase-js reference](/docs/reference/javascript/initializing?example=react-native-options-async-storage) for instructions on initializing the biobase-js client in React Native.
...
const createSessionFromUrl = async (url: string) => {
  ...
  const { data, error } = await biobase.auth.setSession({
    access_token,
    refresh_token,
  })
...
For detailed setup and implementation instructions please refer to the [docs](https://biobase.studio/docs/guides/auth/social-login/auth-apple) and the [video tutorial](https://youtu.be/-tpcZzTdvN0).

### Sign in with Google

Biobase Auth supports Sign in with Google on the web, native Android applications, and Chrome extensions.

For detailed set up and implementation instructions please refer to the [docs](https://biobase.studio/docs/guides/auth/social-login/auth-google) and the [video tutorial](https://youtu.be/vojHmGUGUGc).

## One time passwords

Biobase supports various forms of passwordless authentication:
...
## More React Native and Biobase resources
...
- [React Native file upload with Biobase Storage](/blog/react-native-storage)
