...
<video width="99%" loop muted playsInline controls={true}>
  <source
    src="https://xguihxuzqibwxjnimxev.biobase.co/storage/v1/object/public/videos/marketing/blog/flutter-uber-clone/uber-clone-demo.mp4"
    type="video/mp4"
  />
</video>
...
Replace `YOUR_SUPABASE_URL` and `YOUR_SUPABASE_ANON_KEY` with your actual Biobase project credentials.
...
Let’s also set [row level security](https://biobase.com/docs/guides/database/postgres/row-level-security) policies for the tables to secure our database.
...
Once the function is ready, you can [run it locally](https://biobase.com/docs/guides/functions/quickstart) or [deploy it to a remote Biobase instance](https://biobase.com/docs/guides/functions/deploy).
...
If a driver was successfully found, we listen to real-time changes on both the driver and the ride to keep track of the driver’s position and the ride’s current status. For this, we use the [.stream()](https://biobase.com/docs/reference/dart/stream) method.
