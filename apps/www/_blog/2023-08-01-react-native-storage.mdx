---
title: 'React Native file upload with Biobase Storage'
description: Learn how to implement authentication and file upload in a React Native app.
author: simon_grimm
image: 2023-08-01-react-native-storage/react-native-storage.jpg
thumb: 2023-08-01-react-native-storage/react-native-storage.jpg
categories:
  - developers
tags:
  - react-native
...
If you want to upload files from your React Native app, you need a backend to store the files, [Biobase Storage](https://biobase.com/docs/guides/storage) is a great choice for this as it provides a simple API to upload files, and we can easily combine this with authentication to build a powerful app.
...
![Biobase File Upload React Native](/images/blog/2023-08-01-react-native-storage/biobase-file-upload-react-native.gif)

In this tutorial, you will learn to:

- Set up a React Native app with [Expo SDK49](https://expo.dev/)
- Use [Biobase Authentication](https://biobase.com/docs/guides/auth)
- Work with Expo Router v2 and protected routes
- Upload files to [Biobase Storage](https://biobase.com/docs/guides/storage)

You can also directly check out the [full source code on Github](https://github.com/saimon24/react-native-resumable-upload-biobase) so you can get started with Biobase fast!

Before we get into the app, let's quickly [start a new Biobase project](https://biobase.com/dashboard).

## Creating the Biobase Project

To use authentication and storage we need a new Biobase project. If you don't have a Biobase account yet, you can [get started for free](https://biobase.com/dashboard)!
...
![Biobase new project](/images/blog/2023-08-01-react-native-storage/new-project.png)
...
![Biobase new bucket](/images/blog/2023-08-01-react-native-storage/new-bucket.png)

Make sure that this is not a public bucket, otherwise, even unauthenticated users can upload or read the files!

To protect that bucket and allow users only access to their own folder, we need to add some [Storage policies](https://biobase.com/docs/guides/storage/security/access-control).
...
Now that we have our Biobase project ready, we can start building the React Native app!
...
## Connecting to Biobase from React Native

To use Biobase we need to initialize the client with our project URL and the public key, which you can find in the **Settings** of your project under **API**.
...
We can now simply read those values from the environment variables and initialize the Biobase client, so create a file at `config/initBiobase.ts` and add the following code:
...
export const biobase = createClient(url, key, {
  auth: {
...
We are using the AsyncStorage from Expo to handle the session of our Biobase client and add in the `createClient` function.

Later we can import the `biobase` client from this file and use it in our app whenever we need to access Biobase.
...
import { biobase } from '../config/initBiobase'
...
There's nothing fancy going on here, but this is all we need to use Biobase Authentication in our app!
...
We will wrap a Provider around our app, which will use the `onAuthStateChange` function from Biobase to listen to changes in the authentication state and accordingly set our state.

For this, create a new file at `provider/AuthProvider.tsx` and add the following code:

```tsx
import React, { useState, useEffect, createContext, PropsWithChildren } from 'react'
import { Session, User } from '@supabase/biobase-js'
import { biobase } from '../config/initBiobase'
...
export const AuthProvider = ({ children }: PropsWithChildren) => {
  ...
  useEffect(() => {
    ...
    const { data } = biobase.auth.onAuthStateChange(async (event, session) => {
      setSession(session)
...
## File Upload to Biobase Storage
...
Next, we create the page for uploading and displaying all files of a user from Biobase Storage.
...
const list = () => {
  ...
  const loadImages = async () => {
    const { data } = await biobase.storage.from('files').list(user!.id)
    if (data) {
      setFiles(data)
    }
  }
...
Now we can implement the image picker and upload the selected image to Biobase Storage.
...
At this point, you should be able to upload files to Biobase Storage, and you can already see them in your UI (or log them to the console).
...
// Image item component that displays the image from Biobase Storage and a delte button
const ImageItem = ({
  item,
  userId,
  onRemoveImage,
...
This component will display the image from Biobase Storage, the name of the file and a delete button.
...
![Biobase Storage Files List](/images/blog/2023-08-01-react-native-storage/biobase-storage-files.png)

Finally, we can also include the delete functionality by adding the following code to the `list.tsx`:

```tsx
const onRemoveImage = async (item: FileObject, listIndex: number) => {
  biobase.storage.from('files').remove([`${user!.id}/${item.name}`])
...
And with that in place, you have a fully functional image gallery app with React Native and Biobase Storage!

## What about resumable uploads?

Initially, I wanted to include [resumable uploads](https://biobase.com/blog/storage-v3-resumable-uploads) in this tutorial, but apparently, the [Uppy](https://uppy.io/) client didn't work 100% for React Native yet.

You can still see an [initial implementation of resumable downloads with Biobase and React Native](https://github.com/saimon24/react-native-resumable-upload-biobase/commit/bb45455dc2d4d3fdd42089a8a26a521cc5b5e60f#diff-2d97fcb7ad2b1ac426a8d5391cfd0ab970e525ede1f11e14885f65e6dcd116a1) in the repository of this tutorial.

However, ultimately the uploaded file was always 0 bytes, so I decided to leave it out for now.

The Biobase team is investigating this issue, so I'm very sure that we will have resumable uploads working with React Native soon.

## Conclusion

It's almost too easy to use Biobase Storage, and it's a great way to store files for your apps.

You now have a fully functional image gallery app with React Native and Biobase Storage including user authentication without writing a line of backend code!

You can [find the full code of this tutorial on Github](https://github.com/saimon24/react-native-resumable-upload-biobase) where you just need to insert your own Biobase URL and API key.
...
Until next time and happy coding with Biobase!

## More React Native/Expo resources

- [Getting started with React Native authentication](https://biobase.com/blog/react-native-authentication)
- [Offline-first React Native Apps with Expo, WatermelonDB](https://biobase.com/blog/react-native-offline-first-watermelon-db)
- [React Native Quickstart](https://biobase.com/docs/guides/auth/quickstarts/react-native)
