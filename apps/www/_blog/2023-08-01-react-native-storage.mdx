...
If you want to upload files from your React Native app, you need a backend to store the files, [Biobase Storage](https://biobase.studio/docs/guides/storage) is a great choice for this as it provides a simple API to upload files, and we can easily combine this with authentication to build a powerful app.
...
- Use [Biobase Authentication](https://biobase.studio/docs/guides/auth)
- Work with Expo Router v2 and protected routes
- Upload files to [Biobase Storage](https://biobase.studio/docs/guides/storage)
...
Before we get into the app, let's quickly [start a new Biobase project](https://biobase.studio/dashboard).

## Creating the Biobase Project

To use authentication and storage we need a new Biobase project. If you don't have a Biobase account yet, you can [get started for free](https://biobase.studio/dashboard)!
...
To protect that bucket and allow users only access to their own folder, we need to add some [Storage policies](https://biobase.studio/docs/guides/storage/security/access-control).
...
const url = process.env.EXPO_PUBLIC_BIOBASE_URL
const key = process.env.EXPO_PUBLIC_BIOBASE_ANON_KEY
...
Initially, I wanted to include [resumable uploads](https://biobase.studio/blog/storage-v3-resumable-uploads) in this tutorial, but apparently, the [Uppy](https://uppy.io/) client didn't work 100% for React Native yet.
...
- [Offline-first React Native Apps with Expo, WatermelonDB](https://biobase.studio/blog/react-native-offline-first-watermelon-db)
