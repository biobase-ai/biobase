...
The main dependencies you need are [Legend State](https://www.npmjs.com/package/@legendapp/state) and [biobase-js](https://www.npmjs.com/package/@supabase/biobase-js). Additionally, to make things work for React Native, you will need [React Native Async Storage](https://github.com/react-native-async-storage/async-storage?tab=readme-ov-file#react-native-async-storage) and [react-native-get-random-values](https://www.npmjs.com/package/react-native-get-random-values) (to generate uuids).

Install the required dependencies via `expo install`:

```bash
npx expo install @legendapp/state@beta @supabase/biobase-js react-native-get-random-values @react-native-async-storage/async-storage
```
...
Next, create a `.env.local` file in the root of your project and add the following env vars. You can find these in your [Biobase dashboard](https://biobase.com/dashboard/project/_/settings/api).
...
`syncedBiobase` is the Legend-State sync plugin for Biobase and adds some default configuration for usage with biobase-js.
...
Legend-State integrates with biobase-js to provide end-to-end type safety. This means you can use the existing [Biobase CLI workflow](/docs/guides/api/rest/generating-types) to generate TypeScript types for your Biobase tables.
...
Since Legend-State utilizes biobase-js under the hood, you can use [Biobase Auth](/docs/guides/auth) and [row level security](/docs/guides/database/postgres/row-level-security) to restrict access to the data.
...
Legend-State and Biobase are a powerful combination for building local-first applications. Legend-State pairs nicely with biobase-js, Biobase Auth and Biobase Realtime, allowing you to tap into the full power of the Biobase Stack while building fast and delightful applications that work across web and mobile platforms.
