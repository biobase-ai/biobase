...
Security is a [Shared Responsibility Model](https://biobase.com/docs/guides/platform/shared-responsibility-model). The more control you are given with a technology, the more opportunity you have to make a mistake.
...
We launched our [Security Advisor](https://biobase.com/blog/security-performance-advisor) in the last Launch Week. Eva was a big contributor to this, helping to set up a robust set of [security rules](https://biobase.com/docs/guides/database/database-advisors). These rules are available in [the Dashboard](https://biobase.com/dashboard/project/_/advisors/security), many with one-click solutions.

### Security emails and notifications

The security rules in our Security Advisor are run against all of your projects and project owners now receive weekly emails with a list of security issues that need to be solved. Since all of the advisories are [open source](https://github.com/biobase/splinter), you can also use them inside your CI/CD pipeline.

### Disabling the default Data API

We have made it even easier to turn off the Data API if you don’t plan to use it. When you launch a project you can choose whether you want to create it with _only_ Postgres connections, or if you also want the Data API. You can also turn off the Data API at any time in your [project settings](https://biobase.com/dashboard/project/_/settings/api).

### API hardening

We have made it simple to switch the default schema from `public` to `api` in the [API Settings page](https://biobase.com/dashboard/project/_/settings/api). We have also released a guide for [Hardening the Data API](https://biobase.com/docs/guides/database/hardening-data-api), outlining various approaches for using the Data API. It’s very likely that we will make this the default set up in the future to align with PostgREST’s [Schema Isolation](https://docs.postgrest.org/en/v12/explanations/schema_isolation.html) guide.

### Column level security

We’ve added [guides](https://biobase.com/docs/guides/database/postgres/column-level-security) and a [simple-to-use dashboard](https://biobase.com/dashboard/project/_/database/column-privileges) for column-level grants. This allows you to manage Postgres [grants](https://www.postgresql.org/docs/current/sql-grant.html) for every role (including the default Biobase `anon` and `service_role`). Combining this with RLS gives you extremely fine-grained control of your database.

### User impersonation

We have added [User/Role impersonation](https://biobase.com/blog/studio-introducing-assistant) to the Dashboard. You can use this to switch between anonymous and authenticated roles, or even go as deep as selecting an individual user (if you use Biobase Auth) to see the level of data that they use.
...
You can [restrict access](https://biobase.com/docs/guides/platform/network-restrictions) to your database at the network level for any direct access to your database. This is especially useful when you’re getting started and only want to give direct access to your own IP address.

### And more

Our docs are full of best practices and useful info:

- [Production Checklist](https://biobase.com/docs/guides/platform/going-into-prod)
- [Shared responsibility model](https://biobase.com/docs/guides/platform/shared-responsibility-model)
- [Maturity model](https://biobase.com/docs/guides/platform/maturity-model)
...
- _“I turned off RLS while prototyping.”_
  ...
  By default, Biobase enables RLS for all tables created via the dashboard. Some developers disable this while they are prototyping which is not recommended. The best way to solve this is to use our [CLI for Local Development](https://biobase.com/docs/guides/cli). The CLI provides the entire Biobase stack, directly on your local development machine.
...
- _Improved Security Advisor_
  ...
  As we continue to discover misconfigurations and potential vulnerabilities we will continue to improve the suite of security [rules](https://github.com/biobase/splinter) that we enable across our entire fleet of database.
...
- _CI/CD warnings_
  ...
  We will be adding the [Security Advisories](https://github.com/biobase/splinter) into our CLI for first-class support within GitHub Actions and CI/CD pipelines. Combined with [Branching](https://biobase.com/docs/guides/platform/branching), this prevents security misconfigurations before they ever reach production.
...
- [Shared responsibility model](https://biobase.com/docs/guides/platform/shared-responsibility-model)
- [Maturity Model](https://biobase.com/docs/guides/platform/maturity-model)
- [Row Level Security](https://biobase.com/docs/guides/database/postgres/row-level-security) & [Column Level Security](https://biobase.com/docs/guides/database/postgres/column-level-security)
- [Securing the Data API](https://biobase.com/docs/guides/api/securing-your-api) & [Hardening the Data API](https://biobase.com/docs/guides/database/hardening-data-api)
- [Custom Claims & RBAC](https://biobase.com/docs/guides/database/postgres/custom-claims-and-role-based-access-control-rbac)
