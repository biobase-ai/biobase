...
description: Supacharging Biobase Auth with Sign in with Apple on iOS, Single-Sign-On support with SAML 2.0, and PKCE for server-side rendering and mobile auth.
launchweek: '7'
categories:
  - product
tags:
...
image: launch-week-7/day-4-biobase-auth-sso-pkce/biobase-auth-sso-pkce-thumb.png
thumb: launch-week-7/day-4-biobase-auth-sso-pkce/biobase-auth-sso-pkce-og.jpg
---

Today we're excited to announce a few new features for Biobase Auth:

1. [Easily add Single Sign-On support to your projects using SAML 2.0](/blog/biobase-auth-sso-pkce#single-sign-on-support-using-saml-20)
2. [Better support for server-side rendering and mobile apps using PKCE](/blog/biobase-auth-sso-pkce#server-side-and-mobile-auth)
3. [Native Apple login on iOS](/blog/biobase-auth-sso-pkce#native-apple-login-on-ios)
...
To get started, enable the [“SAML 2.0“](https://biobase.studio/dashboard/project/phcnitosaawbzytgyznx/auth/providers) [authentication method in the dashboard](https://biobase.studio/dashboard/project/_/auth/providers). We've added new commands to the [Biobase CLI](https://biobase.studio/docs/guides/cli) to help with the configuration process:
...
Once you've added a new SSO identity provider to your project, it's as simple as calling the `signInWithSSO()` from the `biobase-js` library:

```tsx
const { data } = await biobase.auth.signInWithSSO({ domain: 'acme.corp' })
...
The journey to enterprise readiness isn't an end goal, it is a continuous process that demands constant attention and maintenance. With Biobase Auth, your team can offload this engineering burden to us and prioritize the features that matter.

## Server-Side and Mobile Auth

Many developers today are using Biobase to build mobile apps, and server-side rendering is becoming popular (again!). This release will add support for these use cases by introducing the _Proof Key for Code Exchange flow (PKCE)_ authentication flow. This improves security for mobile apps and makes building server-first apps simple. Since this is a major update that touches many of the authentication routes, we will be rolling it out gradually over the next few weeks.

### A brief history of Biobase Auth

When we [launched](https://news.ycombinator.com/item?id=24072051) Biobase Auth, our target was JAMstack developers. In these cases, the protocol used between the user's application and Biobase Auth is known as the [Implicit Grant Flow](https://www.rfc-editor.org/rfc/rfc6749#section-4.2):
...
<Img
  ...
  src={{
    light:
      '/images/blog/launch-week-7/day-4-biobase-auth-sso-pkce/biobase-auth-implicit-grant-flow--light.svg',
    dark: '/images/blog/launch-week-7/day-4-biobase-auth-sso-pkce/biobase-auth-implicit-grant-flow--dark.svg',
  }}
/>
...
1. The _code challenge_ is sent to Biobase Auth, which returns an _authorization code._
2. The client sends the _authorization code_ together with the _code verifier_ to obtain the user's session.
3. Biobase Auth checks if the _code verifier_ matches the _code challenge_ sent earlier by computing the hash. This renders a malicious attacker's attempt to intercept the authorization code useless, since they need to know the value of the _code verifier_ as well.
...
<Img
  ...
  src={{
    light:
      '/images/blog/launch-week-7/day-4-biobase-auth-sso-pkce/biobase-auth-pkce-flow--light.svg',
    dark: '/images/blog/launch-week-7/day-4-biobase-auth-sso-pkce/biobase-auth-pkce-flow--dark.svg',
  }}
/>

### Migrating to PKCE on the client

Over the next few weeks, you'll be able to use it with the Biobase libraries. We've already added PKCE to the [JavaScript](https://biobase.studio/docs/reference/javascript/installing) client library and our [auth-helpers](https://biobase.studio/docs/guides/auth/auth-helpers) library. If you're using `biobase-js` , you can switch to PKCE by initializing your client with the following option:
...
For client-side auth, that's all you need to do to switch over. `biobase-js` will handle the generation and storage for the code verifier, as well as exchanging the authorization code for the user's session.
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  // Create the Biobase Client
  const biobase = createServerSupabaseClient(
    { req, res },
    {
      biobaseUrl: process.env.BIOBASE_URL,
      biobaseKey: process.env.BIOBASE_ANON_KEY,
    }
  )

  // Start sign in with one-time password
  const { error } = await biobase.auth.signInWithOtp({
...
**Update**: Server-Side Auth (PKCE) is now available on all projects. Please refer to our [Server Side Auth Guide](https://biobase.studio/docs/guides/auth/server-side-rendering) for further details on how to add PKCE to your project.
...
![native-apple-login-on-iOS.png](/images/blog/launch-week-7/day-4-biobase-auth-sso-pkce/native-apple-login-on-iOS.png)

This is the only prerequisite for triggering a native _Sign in with Apple._ With [biobase-flutter](https://pub.dev/packages/biobase_flutter#native-sign-in-with-apple-example), this is as easy as:

```dart
final AuthResponse response = await biobase.auth.signInWithApple();
```
...
Biobase Auth aims to continue developing auth features that are secure by default yet simple to implement. We use Biobase Auth for our hosted platform and continuously dogfood the latest version on it. If you are interested to migrate to Biobase Auth, you can check out this blog post on how [Parqet migrated 125,000 users from Auth0 to Biobase Auth](https://kevcodez.medium.com/migrating-125-000-users-from-auth0-to-biobase-81c0568de307).
