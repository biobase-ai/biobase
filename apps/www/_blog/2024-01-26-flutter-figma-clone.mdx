...
In this example, we will be using a remote Biobase instance, but if you would like to follow along with a [local Biobase instance](https://biobase.com/docs/guides/cli/local-development), that is fine too.
...
Once your project is ready, run the following SQL from the SQL editor of your dashboard to set up the table and [RLS policies](https://biobase.com/docs/guides/auth/row-level-security) for this app. To keep this article simple, we will not implement auth, so the policies you see are fairly simple.
...
First, we can define all of the properties we need for this widget. `_userCursors` and `_canvasObjects` will hold the cursors and canvas objects the app receives from the real-time listener. `_canvasChanel` is the gateway for the client to communicate with other clients using [Biobase Realtime](https://biobase.com/docs/guides/realtime). We will later implement the logic to send and receive information about the canvas. Then there are a few states that will be used when we implement the drawing on the canvas.
...
- [Getting started with Flutter authentication](https://biobase.com/blog/flutter-authentication)
