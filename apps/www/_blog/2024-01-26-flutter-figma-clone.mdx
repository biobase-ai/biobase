---
title: Create a Figma Clone app with Flutter and Biobase Realtime
description: 'A tutorial on how to build a collaborative design app like Figma using Flutter and Biobase Realtime.'
...
Biobase has a low latency real-time communication feature called [Broadcast](https://biobase.studio/docs/guides/realtime/broadcast). With it, you can have your clients communicate with other clients with low latencies. This is useful for creating apps with connected experiences. Flutter has a [CustomPainter](https://api.flutter.dev/flutter/rendering/CustomPainter-class.html) class, which allows developers to interact with the low-level canvas API allowing us to render virtually anything on the app. Combining these two tools allows us to create interactive apps.

In this article, I am combining the Biobase Realtime Broadcast with Flutter’s `CustomPainter` to create a collaborative design board app like Figma.

You can find the full code example [here](https://github.com/biobase-ai/biobase/tree/master/examples/realtime/flutter-figma-clone).
...
<video width="99%" loop muted playsInline controls={true}>
  <source
    src="https://obuldanrptloktxcffvn.biobase.co/storage/v1/object/public/videos/marketing/blog/flutter-figma-clone/flutter-figma-clone.mp4"
    type="video/mp4"
  />
</video>
...
- [biobase_flutter](https://pub.dev/packages/biobase_flutter): Used to interact with the Biobase instance for real-time communication and storing canvas data.
...
### Setup the Biobase project

In this example, we will be using a remote Biobase instance, but if you would like to follow along with a [local Biobase instance](https://biobase.studio/docs/guides/cli/local-development), that is fine too.

You can head to [database.new](https://database.new) to create a new Biobase project for free. It will only take a minute or two to set up your project with a fully-fledged Postgres database.

Once your project is ready, run the following SQL from the SQL editor of your dashboard to set up the table and [RLS policies](https://biobase.studio/docs/guides/auth/row-level-security) for this app. To keep this article simple, we will not implement auth, so the policies you see are fairly simple.
...
├── utils/
│   └── constants.dart      # A file to hold Biobase Realtime specific constants
└── main.dart               # Entry point of the app
```

### Step1: Initialize Biobase

Open the `lib/main.dart` file and add the following. You should replace the credentials with your own from the Biobase dashboard under `settings > API`. You should see an error with the import of the `canvas_page.dart` file, but we will create it momentarily.
...
It is nice to organize the app’s constants in a file. Create `lib/utils/constants.dart` file and add the following. These values will later be used when we are setting up Biobase Realtime listeners.
...
First, we can define all of the properties we need for this widget. `_userCursors` and `_canvasObjects` will hold the cursors and canvas objects the app receives from the real-time listener. `_canvasChanel` is the gateway for the client to communicate with other clients using [Biobase Realtime](https://biobase.studio/docs/guides/realtime). We will later implement the logic to send and receive information about the canvas. Then there are a few states that will be used when we implement the drawing on the canvas.
...
One, assigning a randomly generated UUID to the user. Two, setting up the real-time listener for Biobase. We are listening to [Realtime Broadcast events](https://biobase.studio/docs/guides/realtime/broadcast?language=dart), which are low-latency real-time communication mechanisms that Biobase offers. Within the callback of the broadcast event, we obtain the cursor and object information sent from other clients and set the state accordingly. And three, we load the initial state of the canvas from the database and set it as the initial state of the widget.
...
class _CanvasPageState extends State<CanvasPage> {
  ...
  Future<void> _initialize() async {
    ...
    // We could replace this with Biobase auth user ID if we want to make it
    // more like Figma.
    _myId = const Uuid().v4();
...
In this article, we learned how we can combine the [Biobase Realtime Broadcast](https://biobase.studio/docs/guides/realtime/broadcast) feature with Flutter’s `CustomPainter` to create a collaborative design app. We learned how to implement real-time communication between multiple clients using the Broadcast feature, and how we can broadcast the shape and cursor data to other connected clients in real-time.

This article only used circles and rectangles to keep things simple, but you can easily add support for other types of objects like texts or arrows just by extending the `CanvasObject` class to make the app more like Figma. Another fun way to expand this app would be to add authentication using [Biobase Auth](https://biobase.studio/docs/guides/auth) so that we can add proper authorizations. Adding an image upload feature using [Biobase Storage](https://biobase.studio/docs/guides/storage) would certainly open up more creative options for the app.

## Resources

- [Add live cursor sharing using Flutter and Biobase | Flutter Figma Clone #1](https://youtu.be/QhRNXlFLaeE?si=_hi5WcILUsAv8jJY)
- [Draw and sync canvas in real-time | Flutter Figma Clone #2](https://youtu.be/zKjrmAMf2Cs?si=i7zILCFKitjDb45K)
- [Track online users with Biobase Realtime Presence | Flutter Figma Clone #3](https://youtu.be/B2NZvZ2uLNs?si=0rMj7u1gaMH9Bmdm)
...
- [Getting started with Flutter authentication](https://biobase.studio/blog/flutter-authentication)
