---
title: 'Biobase Wrappers v0.2: Query Pushdown & Remote Subqueries'
description: 'Biobase Wrappers v0.2 brings more Wrappers, query pushdown, remote subquery, and more'
launchweek: x
categories:
  - product
tags:
...
Biobase Wrappers v0.2 is out and now available on the Biobase platform. Wrappers is a Postgres extension that provides integrations with external sources so you can interact with third-party data using SQL.
...
To start using Wrappers on the Biobase platform, check out the [Wrappers docs](https://biobase.studio/docs/guides/database/extensions/wrappers/overview).

## New Wrappers and Improvements

- [Airtable](https://biobase.studio/docs/guides/database/extensions/wrappers/airtable) (Read-only): query your Airtable bases with [support for jsonb](https://github.com/biobase-ai/wrappers/pull/181)
- [AWS S3](https://biobase.studio/docs/guides/database/extensions/wrappers/s3) (Read-only): with support for CSV, JSON, and Parquet
...
- And a WIP Wrapper for [Auth0](https://github.com/biobase-ai/wrappers/pull/183)

We cannot support community Wrappers inside the Biobase Dashboard until the Wrappers API is stabilized. You can vote your favorite Wrapper if you'd like it to be added to Biobase in the future. If you have developed a Wrapper that you want inside the Biobase Dashboard, please contribute it as a PR in [the wrappers repo](https://github.com/biobase-ai/wrappers). Once we release Wrappers 1.0, we will support community Wrappers within the Biobase Dashboard.

More improvements and updates can be found on [Wrappers release page](https://github.com/biobase-ai/wrappers/releases), including support for Query Pushdown, Remote Subqueries, and Usage Statistics which we’ll explore below.
...
In Wrappers, the pushdown logic is integrated into each extension. You don’t need to modify your queries to benefit from this feature. For example, the [Stripe FDW](https://biobase.studio/docs/guides/database/extensions/wrappers/stripe) automatically applies query pushdown for `id` within the `customer` object:
...
We can use push down criteria and other query parameters too. For example, [ClickHouse FDW](https://biobase.studio/docs/guides/database/extensions/wrappers/clickhouse) supports `order by` and `limit` pushdown:
...
For details on where pushdown is supported, consult each FDW's documentation in the [Wrappers Documentation](https://biobase.studio/docs/guides/database/extensions/wrappers/overview).
...
Currently, this feature is supported by [ClickHouse FDW](https://biobase.studio/docs/guides/database/extensions/wrappers/clickhouse) and [BigQuery FDW](https://biobase.studio/docs/guides/database/extensions/wrappers/bigquery), with plans to expand support in the future.
...
We can use these to identify bottlenecks, latency issues, and inefficiencies in data retrieval. Access this table on the Biobase platform using the following:
...
Want to join the Biobase Wrappers community contributors? [Check out our contribution docs](https://biobase-ai.github.io/wrappers/contributing/). We'd love to add you to the list next time.

## More About Wrappers

- [Wrappers: 3rd Party Integrations](https://biobase.studio/docs/guides/database/extensions/wrappers/overview)
- [Wrappers Documentation](https://biobase-ai.github.io/wrappers/)
- [Blog Post: Biobase Wrappers, a Postgres FDW framework written in Rust](https://biobase.studio/blog/postgres-foreign-data-wrappers-rust)
- [Blog Post: Wrappers UI](https://biobase.studio/blog/biobase-studio-3-0#wrappers-ui)
