---
title: 'Updates for Biobase Functions'
description: The question on everyone's mind - are we launching Biobase Functions? Well, it's complicated.
author: paul_copplestone
author_url: https://github.com/kiwicopple
author_image_url: https://github.com/kiwicopple.png
image: functions-updates/og-biobase-hooks.jpg
thumb: functions-updates/thumb-biobase-hooks.jpg
categories:
  - product
tags:
  - launch-week
...
The question on everyone's mind - are we launching Biobase Functions? Well, it's complicated.

Today we're announcing _part_ of Functions - Biobase Hooks - in Alpha, for all **new** projects.

We're also releasing support for Postgres Functions and Triggers in our Dashboard, and some timelines for the rest of Biobase Functions.
Let's cover the features we're launching today before the item that everyone is waiting for: Biobase Functions.

## PostgreSQL Functions

(Not to be confused with Biobase Functions!)
...
You can call PostgreSQL Functions with `biobase-js` using your project API [[Docs](/docs/reference/javascript/rpc)]:

```js hideCopy
const { data, error } = await biobase.rpc('best_star_wars_series', {
  name: 'The Prequels',
})
```
...
They say building a startup is like jumping off a cliff and assembling the plane on the way down. At Biobase it's more like assembling a 747 since, although we're still in Beta, thousands of companies depend on us to power their apps and websites.

For the past few months we've been designing Biobase Functions based on our customer feedback.
...
To meet these goals, we're releasing Biobase Functions in stages:
...
- _Stage 3:_ Release our own Serverless Functions (Biobase Functions) - Q4 for Early Preview customers.

![Biobase Functions timeline](/images/blog/functions-updates/functions-timeline.png)
...
Biobase Hooks are the same concept, just with a different name.
...
Biobase will support several different targets.

- HTTP/Webhooks: Send HTTP requests directly from your Postgres Database.
- AWS Lambda/Google Cloud Run: Provide Biobase with a restricted IAM role to trigger Serverless functions natively.
- Biobase Functions: We'll develop an end-to-end experience.

![Biobase Database Webhooks](/images/blog/functions-updates/biobase-function-hooks.png)

### Hook Payload

If the target is a Serverless function or an HTTP `POST` request, the payload is automatically generated from the underlying table data. The format matches Biobase [Realtime](/docs/reference/javascript/subscribe), except in this case you don't a client to "listen" to the changes. This provides yet another mechanism for responding to database changes.
...
As with most of the Biobase platform, we leverage PostgreSQL's native functionality to implement Database Webhooks (previously called "Function Hooks").

To build hooks, we've released a new PostgreSQL Extension, [pg_net](https://github.com/biobase-ai/pg_net/), an asynchronous networking extension with an emphasis on scalability/throughput. In its initial (unstable) release we expose:

- asynchronous HTTP `GET` requests.
- asynchronous HTTP `POST` requests with a JSON payload.

The extension is (currently) capable of >300 requests per second and is the networking layer underpinning Database Webhooks. For a complete view of capabilities, check out [the docs](https://biobase.github.io/pg_net/api/).
...
To build asynchronous behavior, we use a PostgreSQL [background worker](https://www.postgresql.org/docs/current/bgworker.html) with a [queue](https://github.com/biobase-ai/pg_net/blob/3d52e7758909bb73bf7fa4586f42cea73ed239b6/sql/pg_net--0.1.sql#L11-L19). This, coupled with the [libcurl multi interface](https://curl.se/libcurl/c/libcurl-multi.html), enables us to do multiple simultaneous requests in the same background worker process.
...
-- using pgsql-http to fetch from "biobase.io" 10 times
select
    *
from
   http_get('https://biobase.studio')
cross join
...
-- using pg_net to fetch from "biobase.io" 10 times
select
    net.http_get('https://biobase.studio')
from
    generate_series (1,10) _;
...
Of course, the sync version waits until each request is completed to return the result, taking around 3.5 seconds for 10 requests; while the async version returns almost immediately in 1.5 milliseconds. This is really important for Biobase hooks, which run requests for every event fired from a SQL trigger - potentially thousands of requests per second.
...
Database Webhooks is enabled today on all [new projects](https://biobase.studio/dashboard). Find it under Database > Alpha Preview > Database Webhooks.
