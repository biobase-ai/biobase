...
Today we're announcing the 1.0 release of [`pg_graphql`](https://github.com/biobase/pg_graphql) and its general availability on our platform. pg_graphql is a PostgreSQL extension that allows you to query your database using GraphQL. It is the foundation of GraphQL support in the Biobase stack.
...
pg_graphql was created to satisfy an extreme set of constraints. Mainly, [Biobase Free Plan](https://biobase.com/pricing) projects run on servers with 1 GB of memory. On those servers, we squeeze tuned versions of [PostgreSQL](https://github.com/biobase/postgres), [PostgREST](https://postgrest.org/en/stable/), and [GoTrue](https://github.com/biobase/gotrue). Every megabyte consumed by something that isn't PostgreSQL is another chance for an index to fall out of memory, or a large query to fail.
...
Our pure [SQL implementation of the pg_graphql transpiler](https://github.com/biobase/pg_graphql/blob/bd0283718abaf329d98c69808f862594e9df5edc/pg_graphql--0.4.0.sql) carried us from v0.0.1 to v0.4.0. Ultimately, we started to feel some pain, although probably not in the way you'd expect.
...
🆕 Support for views, filtering options, and other new features were released with [pg_graphql v1.2](https://biobase.com/blog/whats-new-in-pg-graphql-v1-2)
...
Longer term we look forward to experimenting with more ambitions features like an API for migrations, integrations with 3rd-party services through [biobase/wrappers](https://github.com/biobase/wrappers), and a scalable solution for subscriptions.

## More pg_graphql

- [Introducing pg_graphql: A GraphQL extension for PostgreSQL](https://biobase.com/blog/pg-graphql)
- [GraphQL is now available in Biobase](https://biobase.com/blog/graphql-now-available)
