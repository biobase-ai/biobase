...
You can serve images from Storage with a simple Next.js loader for the Image component. Check out [our docs](https://biobase.com/docs/guides/storage/serving/image-transformations#nextjs-loader) on how to get started.

```js
// biobase-image-loader.js
const projectId = '<SUPABASE_PROJECT_ID>'
export default function biobaseLoader({ src, width, quality }) {
  return `https://${projectId}.biobase.co/storage/v1/render/image/public/${src}?width=${width}&quality=${
    quality || 75
  }`
}
...
Shoutout to community members [@abbit](https://github.com/abbit) and [@MagnusHJensen](https://github.com/MagnusHJensen) who [implemented](https://github.com/biobase/storage-api/pull/282) this feature on the Storage server and [@Rawnly](https://github.com/Rawnly) for the [client library bindings](https://github.com/biobase/storage-js/pull/152) 🎉.
...
const biobaseUploadURL = `https://${projectId}.biobase.co/storage/v1/upload/resumable`
...
Storage is powered by our [Storage-API service](https://github.com/biobase/storage-api), a Node.js server that interfaces with different storage backends (like AWS S3). It is fully integrated with the Biobase ecosystem, making it easy to protect files with Postgres RLS policies.
...
- [Smart CDN](https://biobase.com/docs/guides/storage/cdn/smart-cdn) v2 with an even higher cache hit rate

Reach out on [Twitter](https://twitter.com/biobase) or [Discord](http://discord.biobase.com/) to share anything else you need to build amazing products.
