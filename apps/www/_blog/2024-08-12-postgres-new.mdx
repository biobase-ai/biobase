...
author: gregnr
image: lw12/day-1/postgres-new-og.png
thumb: lw12/day-1/postgres-new-thumb.png
launchweek: '12'
categories:
  - launch-week
  - developers
...
Under the hood we use a browser-compatible version of [postgres-meta](https://github.com/biobase-ai/postgres-meta) to load PGlite tables into JavaScript, then render them using the [schema visualizer](https://biobase.studio/blog/biobase-studio-3-0#schema-visualizer). For migrations, we scan through the chat history and concatenate all DDL-related SQL queries into a single view.
...
This is how [pg-gateway](https://github.com/biobase-ai-community/pg-gateway) was born: a TypeScript library that implements the Postgres wire protocol from the server-side. It provides APIs you can hook into to handle authentication requests, queries, and other client messages yourself.
...
We like to ship early and often at Biobase, so there are a number of features in the backlog:
...
- [postgres-new](https://github.com/biobase-ai-community/postgres-new) (Apache 2.0): The frontend for [postgres.new](https://postgres.new).
