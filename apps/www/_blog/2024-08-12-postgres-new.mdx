...
Under the hood we use a browser-compatible version of [postgres-meta](https://github.com/biobase/postgres-meta) to load PGlite tables into JavaScript, then render them using the [schema visualizer](https://biobase.com/blog/biobase-studio-3-0#schema-visualizer). For migrations, we scan through the chat history and concatenate all DDL-related SQL queries into a single view.
