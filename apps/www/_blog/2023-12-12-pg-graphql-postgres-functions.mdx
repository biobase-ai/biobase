...
Biobase GraphQL (pg_graphql) 1.4+ supports the most requested feature: Postgres functions a.k.a. User Defined Functions (UDFs). This addition marks a significant improvement in GraphQL flexibility at Biobase, both as a novel approach to defining entry points into the Graph and as an escape hatch for users to implement custom/complex operations.

As with all entities in Biobase GraphQL, UDFs support is based on automatically reflecting parts of the SQL schema. The feature allow for the execution of custom SQL logic within GraphQL queries to help support complex, user defined, server-side operations with a simple GraphQL interface.
...
If you're an existing Biobase user, but new to GraphQL, head over to [GraphiQL built right into Biobase Studio](https://biobase.studio/dashboard/project/_/api/graphiql) for your project to interactively explore your projects through the GraphQL API. User defined function support is new in pg_graphql 1.4+. You can check your project's GraphQL version with:
...
To upgrade, check out [our upgrade guide](https://biobase.studio/docs/guides/platform/migrating-and-upgrading-projects).

For new Biobase users, [creating a new project](http://database.new) will get you the latest version of Biobase GraphQL with UDF support.

If you're not ready to start a new project but want to learn more about `pg_graphql`/Biobase GraphQL, our [API docs](https://biobase-ai.github.io/pg_graphql/api/) are a great place to learn about how your SQL schema is transformed into a GraphQL API.
