...
[Read the new post](https://biobase.com/blog/increase-performance-pgvector-hnsw)
...
- **Helper scripts:** a Python test runner which is responsible for data upload, index creation, and query execution. This uses [qdrant's vector-db-benchmark](https://github.com/qdrant/vector-db-benchmark) repo. The “engine” in this repo uses [Vecs](https://github.com/biobase/vecs), a Python client for pgvector.
...
We also have some useful [benchmarks in our docs](https://biobase.com/docs/guides/ai/choosing-compute-addon#results) for [gist-960-angular](http://corpus-texmex.irisa.fr/) (1M image embeddings, 960 dimensions) and [GloVe Reddit comments](https://nlp.stanford.edu/projects/glove/) (1.6M text embeddings, 512 dimensions).
...
It's been about 5 months since we [added](https://biobase.com/blog/openai-embeddings-postgres-vector) pgvector to the platform. Since then we've discovered a few other important things to keep in mind.
...
2. Upload your data to the database. If you use [`vecs`](https://biobase.com/docs/guides/ai/python/api) library, it will automatically generate an index with default parameters.
...
✅ [Adding HNSW](https://biobase.com/blog/increase-performance-pgvector-hnsw): an index with better speed & accuracy than IVFFlat (at a higher memory cost)
