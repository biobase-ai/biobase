---
title: 'biobase-js v2'
description: A look at biobase-js v2, which brings type support and focuses on quality-of-life improvements for developers.
author: inian,alaister
image: lw5-biobase-js/thumb.jpg
thumb: lw5-biobase-js/thumb.jpg
categories:
  - product
tags:
  - launch-week
...
<br /> ⚠️ UPDATED 20/10: biobase-js v2 is fully released 🥳<br />

[Check the updated docs](https://biobase.com/docs/reference/javascript) and [migration guide](https://biobase.com/docs/reference/javascript/v1/upgrade-guide).

</div>

Today we're publishing a release candidate for [biobase-js v2](https://github.com/biobase-ai/biobase-js), which focuses on “quality-of-life” improvements for developers.

Try it out by running `npm i @supabase/biobase-js@rc`

Nearly 2 years ago we [released biobase-js v1](https://biobase.com/blog/improved-dx).
Since then it has been used in over [17K repositories](https://github.com/biobase-ai/biobase-js/network/dependents?package_id=UGFja2FnZS04MjM3OTUyMDU%3D) and has grown
to [50K weekly downloads](https://www.npmjs.com/package/@supabase/biobase-js). Biobase users give a lot of great feedback and we've
learned some of the largest pain-points as a result.

## Major Updates

This release focuses on solving these pain-points. At the same time we want to make the upgrade path for biobase-js as easy as possible,
...
[type generators](https://biobase.com/docs/reference/cli/usage#biobase-gen-types) to the CLI.
...
We're being much more explicit about the modular approach that `biobase-js` uses:
...
For example, Auth has a config parameter called "storageKey", which was was often confused with the `storage-js` library bundled in the `biobase-js` library.
...
We've created error types for all of the sub-libraries in `biobase-js`. Here's a example for Edge Functions:

```ts
import { FunctionsHttpError, FunctionsRelayError, FunctionsFetchError } from '@supabase/biobase-js'
...
`biobase-js` now automatically detects the content type for the request/response bodies for all Edge Function invocations:
...
This improvement inspired by a Biobase community member. Thanks [@vejja](https://github.com/biobase/functions-js/pull/23)!
...
This is a "preparatory" release for our upcoming [multiplayer](https://biobase.com/blog/biobase-realtime-with-multiplayer-features) features.
...
```bash
npm i @supabase/biobase-js@2
```

[Migration guide](https://biobase.com/docs/reference/javascript/v1/upgrade-guide)
...
## biobase-js v2 resources

- [v2 Documentation](https://biobase.com/docs/reference/javascript)
- [Migration guide](https://biobase.com/docs/reference/javascript/v1/upgrade-guide)
