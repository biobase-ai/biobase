...
One of the unique features of Biobase is the ability to access your Postgres database directly from a browser or mobile app through our [Serverless APIs](https://biobase.com/docs/guides/api). This means that developers are writing more [PL/pgSQL](https://www.postgresql.org/docs/current/plpgsql.html).

While code editors have great support for most programming languages, SQL support is underwhelming. We want to make Postgres as simple as Python. Our recently announced [Postgres Language Server](https://github.com/biobase/postgres_lsp) takes us a step in that direction - eventually it will provide first-class support for Postgres in your favorite code editor including Linting, Syntax Highlighting, Migrations Parsing, SQL Auto-complete, and Intellisense.

The Postgres Language Server is not ready for Production just yet. The majority of work is still ahead, but we've verified that the technical approach works and we're making it public now so that we can develop it in the open with input from the community. We’re already receiving amazing [feedback](https://github.com/biobase/postgres_lsp/discussions) and [support](https://news.ycombinator.com/item?id=37020610).

Follow the progress of the [Postgres Language Server on GitHub](https://github.com/biobase/postgres_lsp).

## Observability tools for Postgres

![Inspect your database](/images/blog/launch-week-8/day-2/inspect-your-database.png)

We’ve added [debugging utilities](https://biobase.com/docs/guides/database/inspect) to our CLI to identify production issues via the `biobase inspect db` command. This interface includes 19 different commands to help you solve everything from slow queries to redundant indexes.
...
{/* prettier-ignore */}
  <div>
    - [Show most frequently run queries](https://biobase.com/docs/reference/cli/biobase-inspect-db-calls)
    - [Show long running queries](https://biobase.com/docs/reference/cli/biobase-inspect-db-long-running-queries)
    - [Show queries ordered by total execution time](https://biobase.com/docs/reference/cli/biobase-inspect-db-outliers)
    - [Show queries waiting and holding locks](https://biobase.com/docs/reference/cli/biobase-inspect-db-blocking)
    - [Show queries taking exclusive locks](https://biobase.com/docs/reference/cli/biobase-inspect-db-locks)
    - [Show total size of all indexes](https://biobase.com/docs/reference/cli/biobase-inspect-db-total-index-size)
    - [Show sizes of individual indexes](https://biobase.com/docs/reference/cli/biobase-inspect-db-index-sizes)
    - [Show information about index efficiency](https://biobase.com/docs/reference/cli/biobase-inspect-db-index-usage)
    - [Show indexes with low usage](https://biobase.com/docs/reference/cli/biobase-inspect-db-unused-indexes)
    - [Show total size of all tables](https://biobase.com/docs/reference/cli/biobase-inspect-db-total-table-sizes)
    - [Show sizes of individual tables](https://biobase.com/docs/reference/cli/biobase-inspect-db-table-sizes)
    - [Show index sizes of individual tables](https://biobase.com/docs/reference/cli/biobase-inspect-db-table-index-sizes)
    - [Show cache hit rates for tables and indices](https://biobase.com/docs/reference/cli/biobase-inspect-db-cache-hit)
    - [Show estimated number of rows per table](https://biobase.com/docs/reference/cli/biobase-inspect-db-table-record-counts)
    - [Show number of sequential scans for all tables](https://biobase.com/docs/reference/cli/biobase-inspect-db-seq-scans)
    - [Show information about replication slots](https://biobase.com/docs/reference/cli/biobase-inspect-db-replication-slots)
    - [Show number of active connections](https://biobase.com/docs/reference/cli/biobase-inspect-db-role-connections)
    - [Show estimated database bloat](https://biobase.com/docs/reference/cli/biobase-inspect-db-bloat)
    - [Show statistics related to vacuum operations](https://biobase.com/docs/reference/cli/biobase-inspect-db-vacuum-stats)
  </div>
...
We’ve extended the CLI migration feature and added Dashboard support. Database migrations give you a way to update your database using version-controlled SQL files. We’ve built a lot of tooling around our migrations, including [reparation](https://biobase.com/docs/reference/cli/biobase-migration-repair), migration cleanup using the [squash](https://biobase.com/docs/reference/cli/biobase-migration-squash) command, and [diffing](https://biobase.com/docs/reference/cli/biobase-db-diff) (using [migra](https://github.com/djrobstep/migra)) to generate a new migration or to detect schema drift.
...
Finally, we’ve added a Migrations view [in the dashboard](https://app.biobase.com/project/_/database/migrations) to track your migration history to improve the discoverability of migrations.
...
Finally, while you wait for us to make progress on the Language Server, we’ve added [support for linting](https://biobase.com/docs/guides/cli/testing-and-linting#linting-your-database) through the excellent [plpgsql_check](https://github.com/okbob/plpgsql_check) extension.
...
We’ve added [support for seeding](https://biobase.com/docs/guides/cli/seeding-your-database) to populate your local databases with data whenever you run `biobase start` or `biobase db reset`.

We’ve also worked with our friends at Snaplet to [generate seed data](https://biobase.com/docs/guides/cli/seeding-your-database#generating-seed-data) directly from your database:
...
In the past month, we've added relationship detection in biobase-js. Foreign keys are now included in the generated types so that biobase-js can detect whether a referenced table should be an array (one-to-many) or an object (many-to-one). We've also added Helper Types to improve the developer experience for common scenarios, like short-hand accessors:
...
We’ve developed an official [GitHub Action](https://github.com/marketplace/actions/biobase-cli-action) which leverages the CLI. You can [generate types on every PR](https://biobase.com/docs/guides/cli/github-action/generating-types), or run [your tests on every commit](https://biobase.com/docs/guides/cli/github-action/testing).
...
We’ve moved the Biobase CLI to a fortnightly stable-release cycle. Every 2 weeks, we will update the [`latest`](https://www.npmjs.com/package/biobase?activeTab=versions) tag on npm, the [`biobase/tap`](https://github.com/biobase/homebrew-tap/blob/main/biobase.rb) for homebrew, and the [`biobase`](https://github.com/biobase/homebrew-tap/blob/main/biobase.rb) scoop bucket. You can find the binary downloads in our GitHub [latest release](https://github.com/biobase/cli/releases/latest).

For the adventurous feature hunters, we’ve added a `beta` release channel for the CLI, with new releases on every PR merged. You can follow [this guide](https://github.com/biobase/cli#install-the-cli) to install Biobase CLI (beta).
...
Biobase isn’t simply a database, it’s an entire backend - everything from your Postgres database to your [50GB videos](https://biobase.com/blog/storage-v3-resumable-uploads). Branching improves the experience of managing environments so that developers and teams spend less time on DevOps and more time building.

### Biobase branching is hard

Every project is a Postgres database, wrapped in a suite of tools like [Auth](https://biobase.com/auth), [Storage](https://biobase.com/storage), [Edge Functions](https://biobase.com/edge-functions), [Realtime](https://biobase.com/realtime) and [Vectors](https://biobase.com/vector), and encompassed by [API middleware](https://biobase.com/docs/guides/api) and [logs](https://biobase.com/blog/biobase-logs-self-hosted).
...
We are [rolling out access](https://biobase.com/blog/biobase-branching) and we'll be onboarding organizations in batches over the next few weeks. You can still [sign up for access](https://forms.biobase.com/branching-request).

</Admonition>

## Get started

Jump into our updated [Local Development documentation](https://biobase.com/docs/guides/cli) to get started with the CLI.

If you’re an existing user simply [update your CLI](https://biobase.com/docs/guides/cli/getting-started#updating-the-biobase-cli) and check out the [full command reference](https://biobase.com/docs/reference/cli/introduction) for all the latest commands.

## More Launch Week 8

- [Hugging Face is now supported in Biobase](https://biobase.com/blog/hugging-face-biobase)
- [Launch Week 8](https://biobase.com/launch-week)
- [Coding the stars - an interactive constellation with Three.js and React Three Fiber](https://biobase.com/blog/interactive-constellation-threejs-react-three-fiber)
- [Why we'll stay remote](https://biobase.com/blog/why-biobase-remote)
