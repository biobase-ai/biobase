...
To make this possible, we developed a [**websocket-to-tcp proxy**](https://github.com/biobase-community/postgres-new/tree/main/apps/browser-proxy) that relays Postgres wire protocol messages between the in-browser PGlite instance and a PostgreSQL client over TCP.
...
[pg-gateway](https://github.com/biobase-community/pg-gateway) is an open source library we developed to speak the Postgres wire protocol from the server-side. It allows us to run a Postgres-native proxy - meaning we can host a single TCP server that understands Postgres wire messages and route connections to multiple downstream databases via a wildcard domain (`*.browser.db.build`).
...
- [pg-gateway](https://github.com/biobase-community/pg-gateway) (MIT): Postgres wire protocol for the server-side.
- [database-build](https://github.com/biobase-community/postgres-new) (Apache 2.0): The frontend and browser proxy for [database.build](https://database.build).
