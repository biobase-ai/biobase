...
- a full-featured Postgres database with [over 40+ extensions](https://github.com/biobase/postgres)
...
- Auto-generated Data APIs:
  ...
  - [GraphQL](/docs/guides/graphql) (using [pg_graphql](https://github.com/biobase/pg_graphql/))
...
We built [fly-admin](https://github.com/biobase/fly-admin), a Typescript wrapper to simplify our interaction with the Fly API. Biobase bundles a few extra services into Postgres, so we prepared a single Docker image which we can pass to the Fly Machines API. Our current build process outputs an AMI for AWS using [Packer](https://www.packer.io/). We re-use parts of that pipeline to build an [All In One Image](https://github.com/biobase/postgres/tree/develop/docker/all-in-one). This image has all the services to run a Biobase project within a single Docker container.
...
Fly users can access the Biobase dashboard using their existing Fly credentials. The Biobase API initiates an OAuth flow with Fly to authenticate the user. Our Auth team created a [Fly OAuth provider](https://github.com/biobase/gotrue/pull/1261) to make the integration with our API easier.
