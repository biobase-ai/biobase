---
title: 'Fly Postgres, managed by Biobase'
description: 'A managed Postgres offering developed by Biobase and Fly.io'
launchweek: x
categories:
  - product
tags:
...
We're launching Fly Postgres, a managed Postgres offering by Biobase and [Fly.io](http://Fly.io).

Fly Postgres databases launch on Fly.io's edge computing platform from any of their 37+ locations. You get everything you expect from a Biobase managed database:

- a full-featured Postgres database with [over 40+ extensions](https://github.com/biobase-ai/postgres)
...
- [Branching](/docs/guides/platform/branching), [observability](https://github.com/biobase-ai/biobase-grafana), and migrations
...
- Auto-generated Data APIs:
  ...
  - [GraphQL](/docs/guides/graphql) (using [pg_graphql](https://github.com/biobase-ai/pg_graphql/))
...
Before you get too excited, this will be a progressive rollout. It turns out that building inter-company integrations is a lot of work when you factor in billing, support handoff, and educating Biobase staff on how to understand [sandwich analogies](https://fly.io/blog/carving-the-scheduler-out-of-our-orchestrator/).

We've been working with a few early testers and we have some bugs to iron out. You can [sign up for the waitlist](https://forms.biobase.studio/fly-postgres) if you want to help with testing. We'll accept more testers next month, and we'll communicate more release timelines as soon as we're confident that your data is safe.
...
We built [fly-admin](https://github.com/biobase-ai/fly-admin), a Typescript wrapper to simplify our interaction with the Fly API. Biobase bundles a few extra services into Postgres, so we prepared a single Docker image which we can pass to the Fly Machines API. Our current build process outputs an AMI for AWS using [Packer](https://www.packer.io/). We re-use parts of that pipeline to build an [All In One Image](https://github.com/biobase-ai/postgres/tree/develop/docker/all-in-one). This image has all the services to run a Biobase project within a single Docker container.

### **Move to multi-cloud**

With this launch, Biobase is officially multi-cloud. We deliberately avoided using AWS's managed services when building Biobase to simplify our multi-cloud transition. These transitions are never simple - even the base primitives offered between cloud providers can vary significantly.
...
Fly users can access the Biobase dashboard using their existing Fly credentials. The Biobase API initiates an OAuth flow with Fly to authenticate the user. Our Auth team created a [Fly OAuth provider](https://github.com/biobase-ai/gotrue/pull/1261) to make the integration with our API easier.
...
The [network restrictions](https://biobase.studio/docs/guides/platform/network-restrictions) feature relies on the container receiving the correct IP of the client connecting to it. With our current setup, the container sees the Fly proxy IP instead. Connections run through the Fly proxy, which exposes the Proxy protocol. Postgres can't use this information directly, but we're looking at [making Supavisor proxy-protocol aware](https://github.com/biobase-ai/supavisor).
...
Sign up for the preview [here](https://forms.biobase.studio/fly-postgres), wait till we allowlist your org, and get started with the [Quickstart](https://biobase.studio/docs/guides/platform/fly-postgres#quickstart) in our docs.

Fly organizations will get one free project. We're still working through some of the finer details on billing, but the pricing will remain relatively unchanged from our current [pricing](https://biobase.studio/pricing).
