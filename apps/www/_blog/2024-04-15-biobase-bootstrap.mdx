...
The list of starter templates is published on GitHub as [samples.json](https://github.com/biobase-community/biobase-samples/blob/main/samples.json). Whenever we (and in the future the community) add a new starter, it will automatically become available to all Biobase users.
