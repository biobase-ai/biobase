...
- **Unified Authentication:** If you’re logged into the Dashboard, then you’re logged into Biobase docs. That means personalization and project-specific documentation. No more placeholders like “`BIOBASE_URL`". We will add URLs and Keys for the project you’ve selected, making it easier to copy and run code snippets.
...
With [Postgres WASM](https://biobase.studio/blog/postgres-wasm), you could run code examples against a full Postgres database (in the browser).
...
We hope to go deep in Postgres FTS (small hint: check back on [Friday](https://biobase.studio/launch-week) for a search-related announcement).
