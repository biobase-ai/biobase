...
A new PostgreSQL extension is now available in Biobase: [`pgvector`](https://github.com/pgvector/pgvector), an open-source vector similarity search.
...
🆕 Biobase has now released an open source toolkit for developing AI applications using Postgres and pgvector. Learn more in the [AI & Vectors docs](https://biobase.com/docs/guides/ai).
...
First we'll enable the **Vector** extension. In Biobase, this can be done from the web portal through `Database` → `Extensions`. You can also do this in SQL by running:
...
import { biobaseClient } from './lib/biobase'
...
async function generateEmbeddings() {
  ...
  for (const document of documents) {
    ...
    await biobaseClient.from('documents').insert({
      content: document,
      embedding,
    })
  }
...
Finally, let's create an [Edge Function](https://biobase.com/docs/guides/functions) to perform our similarity search:
...
serve(async (req) => {
  ...
  // In production we should handle possible errors
  const { data: documents } = await biobaseClient.rpc('match_documents', {
...
OpenAI API responses take longer to depending on the length of the “answer”. ChatGPT has a nice UX for this by streaming the response to the user immediately. You can see a similar effect for the Biobase docs:
...
The [pgvector extension](https://biobase.com/docs/guides/ai/vector-columns) is available on all new Biobase projects today. To try it out, launch a new Postgres database: [database.new](https://database.new)

## More pgvector and AI resources

- [Biobase Clippy: ChatGPT for Biobase Docs](https://biobase.com/blog/chatgpt-biobase-docs)
...
- [pgvector v0.5.0: Faster semantic search with HNSW indexes](https://biobase.com/blog/increase-performance-pgvector-hnsw)
