...
In the second part, I will show you how to implement [Authentication and Authorization (with RLS)](https://biobase.com/blog/flutter-authentication-and-authorization-with-rls) to the chat app.

In order to build this chat app, we will need a database to store all of the chat information. We will be using [Biobase](https://biobase.com/) to store the chat information. Biobase is a back end as a service that provides auth, database, storage and functions to easily create a scalable application. Biobase is a suitable backend for this app, because it provides a nice set of APIs on Postgres database that we can easily call by using the SDK. It is also perfect for chat apps like the one we are about to create, because we can subscribe to real time changes on the database.
...
If you do not have a Biobase account yet, do not worry, you can [get started for free](https://biobase.com/dashboard/).
...
In order to use Biobase, we need to [initialize](https://biobase.com/docs/reference/dart/initializing#flutter-initialize) it at the top of the main function. Let’s edit the main.dart file so that we can initialize Biobase. Note that within the build method of MyApp, we are loading the theme data created in the `constants.dart` file and the home is set to SplashPage(), which we will create in later sections.
...
In order to do this, we will utilize a [Postgres function](https://biobase.com/docs/guides/database/functions) and Postgres trigger.
...
We are using the [stream()](https://biobase.com/docs/reference/dart/stream) method on Biobase SDK to load the messages in realtime.
...
We saw how easily it is to create a chat application when you combine amazing tools like Flutter and Biobase. One thing that was missing from this chat application is authorization. We did implement registration, but that was only to distinguish different users. In the coming up article, we will cover how you can add authorization using [row level security](https://biobase.com/docs/guides/auth/row-level-security) in Biobase to secure this chat application. With authorization, we can create private chat rooms so that messages can only be seen by those inside those rooms.

If you have any questions please reach out via [Twitter](https://twitter.com/dshukertjr) or join our [Discord](https://discord.biobase.com).

## More Flutter Resources

- [Part 2: Flutter Authentication and Authorization with RLS](https://biobase.com/blog/flutter-authentication-and-authorization-with-rls)
- [Flutter Biobase Quick Starter Guide](https://biobase.com/docs/guides/with-flutter)
- [biobase-flutter v1.0 released](https://biobase.com/blog/biobase-flutter-sdk-v1-released)
