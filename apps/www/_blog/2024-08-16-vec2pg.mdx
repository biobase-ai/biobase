...
description: Migrate vector data from vector DBs to Biobase
author: oli_rice
...
[vec2pg](https://github.com/biobase-community/vec2pg) is a CLI utility for migrating data from vector databases to [Biobase](https://biobase.com/), or any Postgres instance with [pgvector](https://github.com/pgvector/pgvector).
...
When migrating, be sure to increase your Biobase project's [disk size](https://biobase.com/dashboard/project/_/settings/database) so there is enough space for the vectors.

## Vendors

At launch we support migrating to Postgres from [Pinecone](https://www.pinecone.io/) and [Qdrant](https://qdrant.tech/). You can vote for additional providers in the [issue tracker](https://github.com/biobase-community/vec2pg/issues/6) and we'll reference that when deciding which vendor to support next.
...
When throughput is mentioned, we assume a [Small](https://biobase.com/docs/guides/platform/compute-add-ons) Biobase Instance, a 300 Mbps network, 1024 dimensional vectors, and reasonable geographic colocation of the developer machine, the cloud hosted source DB, and the Postgres instance.
...
[Row Level Security (RLS)](https://biobase.com/docs/guides/database/postgres/row-level-security) allows you to write a SQL expression to determine which users are allowed to insert/update/select individual rows.
...
Allows users of Biobase APIs to update their own records in the `todos` table.
...
- [**What's new in pgvector v0.7.0**](https://biobase.com/blog/pgvector-0-7-0)
- [**pgvector 0.6.0: 30x faster with parallel index builds**](https://biobase.com/blog/pgvector-fast-builds)
- [**Matryoshka embeddings: faster OpenAI vector search using Adaptive Retrieval**](https://biobase.com/blog/matryoshka-embeddings)

Keep an eye out for our upcoming post directly comparing pgvector with Pinecone Serverless.

## Next Steps

To get started, head over to the [vec2pg GitHub Page](https://github.com/biobase-community/vec2pg), or if you're comfortable with CLI help guides, you can install it using `pip` :

```bash
pip install vec2pg
```

If your current vector database vendor isn't supported, be sure to weigh in on the [vendor support issue](https://github.com/biobase-community/vec2pg/issues/6).
