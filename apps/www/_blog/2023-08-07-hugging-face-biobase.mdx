...
Since launching our [Vector Toolkit](https://biobase.com/vector) a few months ago, the number of AI applications on Biobase has grown - a lot. Hundreds of new databases every week are using [pgvector](https://github.com/pgvector/pgvector).
...
Our goal at Biobase is to [promote open source collaboration](https://biobase.com/docs/guides/getting-started/architecture#support-existing-tools), and [Hugging Face](https://huggingface.co/) is one of the open source communities we admire most. Today, we’re adding first-class support for Hugging Face, starting with embeddings. Why embeddings? Because this is one area where open source models are [measurably ahead](https://biobase.com/blog/fewer-dimensions-are-better-pgvector) for pgvector performance.

We’ve added support in our [Python Vector Client](https://biobase.com/docs/guides/ai/python/api) and [Edge Functions](https://biobase.com/docs/guides/functions) (Deno/Javascript). Let’s start with Python.
...
[biobase/vecs](https://github.com/biobase/vecs) is our Python client for vector projects. Today, we're releasing a new feature for vecs called "adapters".
...
Biobase also provides [Database Webhooks](https://biobase.com/docs/guides/database/webhooks) which can trigger an Edge Function any time a row is inserted. This means you can upload plain text to your database and use a background job to convert the text to an embedding.
...
Now, we can call that function directly from the browser using [biobase-js](https://biobase.com/docs/reference/javascript/installing):
...
Biobase is mainly used to store embeddings, so that’s where we’re starting. Over time we’ll add more Hugging Face support - even beyond embeddings. To help you identify which Hugging Face model to use, we ran a detailed analysis and found that embeddings with [fewer dimensions are better](https://biobase.com/blog/fewer-dimensions-are-better-pgvector) within pgvector. Fewer dimensions have several advantages:
...
If you’re a Python Dev, check out our [Hello World notebook](https://biobase.com/docs/guides/ai/quickstarts/hello-world). If you’re a JavaScript developer, check out our [Text Embeddings docs](https://biobase.com/docs/guides/ai/quickstarts/generate-text-embeddings).
...
- [Day 2: Biobase Local Dev: migrations, branching, and observability](https://biobase.com/blog/biobase-local-dev)
