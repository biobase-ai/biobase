---
title: 'Implementing semantic image search with Amazon Titan and Biobase Vector'
description: 'Implementing semantic image search with Amazon Titan and Biobase Vector in Python.'
author: thor_schaeff
image: getting-started/amazon-bedrock/amazon-bedrock-biobase-vecs.png
thumb: getting-started/amazon-bedrock/amazon-bedrock-biobase-vecs.png
categories:
  - engineering
tags:
  - postgres
...
In this post we'll look at how we can get started with Amazon Bedrock and Biobase Vector in Python using the Amazon Titan multimodal model and the [vecs client](/docs/guides/ai/vecs-python-client).

You can find the full application code as a Python Poetry project on [GitHub](https://github.com/biobase-ai/biobase/tree/master/examples/ai/aws_bedrock_image_search).
...
If you haven't already, head over to [database.new](https://database.new) and create a new project. Every Biobase project comes with a full Postgres database and the [pgvector extension](/docs/guides/database/extensions/pgvector) preconfigured.
...
You can find the database connection string in your Biobase Dashboard [database settings](https://biobase.studio/dashboard/project/_/settings/database). Select "Use connection pooling" with `Mode: Session` for a direct connection to your Postgres database. It will look something like this:

```txt
postgresql://postgres.[PROJECT-REF]:[YOUR-PASSWORD]@aws-0-[REGION].pooler.biobase.studio:5432/postgres
```
...
- [`vecs`](https://github.com/biobase-ai/vecs#vecs): Biobase Vector Python Client.
...
DB_CONNECTION = "postgresql://postgres.[PROJECT-REF]:[YOUR-PASSWORD]@aws-0-[REGION].pooler.biobase.studio:5432/postgres"
```
...
In the root of your project, create a new folder called `images` and add some images. You can use the images from the example project on [GitHub](https://github.com/biobase-ai/biobase/tree/master/examples/ai/aws_bedrock_image_search/images) or you can find license free images on [unsplash](https://unsplash.com).
...
Next, create a `seed` method, which will create a new Biobase Vector Collection, generate embeddings for your images, and upsert the embeddings into your database:
...
After activating the virtual environtment with `poetry shell` you can now run your seed script via `poetry run seed`. You can inspect the generated embeddings in your Biobase Dashboard by visiting the [Table Editor](https://biobase.studio/dashboard/project/_/editor), selecting the `vecs` schema, and the `image_vectors` table.

## Perform an image search from a text query

With Biobase Vector we can easily query our embeddings. We can use either an image as the search input or alternatively we can generate an embedding from a string input and use that as the query input:
...
With just a couple of lines of Python you are able to implement image search as well as reverse image search using the Amazon Titan multimodal model and Biobase Vector.
