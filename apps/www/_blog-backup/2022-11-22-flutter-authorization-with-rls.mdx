...
description: Learn how you can secure your Flutter app using Biobase Row Level Security.
author: tyler_shukert
image: flutter-chat-auth/biobase-flutter-auth.jpg
thumb: flutter-chat-auth/biobase-flutter-auth.jpg
categories:
  - developers
tags:
  - flutter
...
In this article, I will show you how you can make a secure chat application by introducing authorization to the basic chat app that we created [previously](https://biobase.studio/blog/flutter-tutorial-building-a-chat-app).

We will store the chat data on [Biobase](https://biobase.studio/).
Biobase utilizes the built in authorization mechanism of PostgreSQL called [Row Level Security](https://biobase.studio/docs/guides/auth/row-level-security) or RLS to prevent unauthorized access from accessing or writing data to your database.
...
You can also find a complete code example [here](https://github.com/biobase-ai-community/flutter-chat/tree/with_auth) to follow along.
...
We will also introduce a `create_new_room` function, which is a [database function](https://biobase.studio/docs/guides/database/functions) that handles chat room creation. It knows to create a new room if a chat room with the two users does not exist yet, or to just return the room ID if it already exists.
...
alter publication biobase_realtime add table public.room_participants;
...
Since biobase_flutter has a mechanism to detect and handle deep links, we will register a `io.biobase.chat://login` as our deep link for the app and bring the users back after confirming their email address.
  ...
  <application ...>
    <activity ...>
      ...
      <intent-filter>
        ...
        <data
          android:scheme="io.biobase.chat"
          android:host="login" />
      </intent-filter>
...
We also need to set the deep link in our Biobase dashboard. Go to `Authentication > URL Configuration` in your dashboard and add `io.biobase.chat://login` as one of the redirect URLs.
...
A [cubit](https://bloclibrary.dev/#/coreconcepts?id=cubit) is a class within the flutter_bloc library where we will make requests to Biobase to get the data and transform them into states and emit them to the UI widgets.
...
class RoomCubit extends Cubit<RoomState> {
  ...
  Future<void> initializeRooms(BuildContext context) async {
    ...
    _myUserId = biobase.auth.currentUser!.id;
    ...
    _rawRoomsSubscription =
        biobase.from('room_participants').stream(
  ...
  void _getNewestMessage({
    ...
    _messageSubscriptions[roomId] = biobase
        .from('messages')
  ...
  Future<String> createRoom(String otherUserId) async {
    final data = await biobase.rpc('create_new_room',
...
class ChatCubit extends Cubit<ChatState> {
  ...
  Future<void> sendMessage(String text) async {
    ...
    try {
      await biobase.from('messages').insert(message.toMap());
    } catch (_) {
...
Because we have modified the setting of our Biobase to send a confirmation email, we need to make some modifications to the register page and login page as well.
  ...
  Future<void> _signUp() async {
    ...
    try {
      await biobase.auth.signUp(
        ...
        emailRedirectTo: 'io.biobase.chat://login',
      );
...
class SplashPageState extends State<SplashPage> {
  ...
  Future<void> getInitialSession() async {
    ...
    try {
      final session =
          await BiobaseAuth.instance.initialSession;
      if (session == null) {
        Navigator.of(context).pushAndRemoveUntil(
...
There are two ways we can define Row Level Security policies in Biobase: with the GUI or through SQL. Today we will use SQL.
...
Continuing from our [previous article](https://biobase.studio/blog/flutter-tutorial-building-a-chat-app), we added proper authorization to our chat application using Row Level Security, which enabled us to add 1 on 1 chat feature.
We used bloc for our state management solution. One thing we could have done differently if we were to write test codes was to pass the biobase instance as a parameter of the cubit so that we could [write tests using the bloc_test package](https://bloclibrary.dev/#/testing).
...
We can for example update this to a list of users that are online at the same time. We can implement this using the [presence feature](https://biobase.studio/docs/guides/realtime/presence) of Biobase.

## More Flutter Resources

- [Complete set of code of this chat app](https://github.com/biobase-ai-community/flutter-chat/tree/with_auth)
- [biobase-flutter docs](https://biobase.studio/docs/reference/dart)
