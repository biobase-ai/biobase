...
Each image includes the user’s GitHub details. We use [`supabase-js`](https://biobase.studio/docs/reference/javascript/installing) for authentication: users log in with their GitHub account and we store their username in a table in Postgres.
...
Once generated, we can conveniently upload the image to Biobase Storage using `supabase-js`. This ensures fast response times as well as efficient resource usage.
