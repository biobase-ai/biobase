---
title: Getting started with Ruby on Rails and Postgres on Biobase
description: 'Learn how to create a new Rails app and connect it to a Biobase Postgres database.'
author: thor_schaeff
...
In this post we'll start from scratch, creating a new Rails project, connecting it to our Biobase Postgres database, and interacting with the database using the Rails Console.

<Admonition type="note" label="Need help migrating?">

Biobase is one of the best [free alternatives to Heroku Postgres](/alternatives/biobase-vs-heroku-postgres). See [this guide](/docs/guides/resources/migrating-to-biobase/heroku) to learn how to migrate from Heroku to Biobase.

There's also a [Heroku to Biobase migration tool](https://migrate.biobase.studio/) available to migrate in just a few clicks.
...
When your project is up and running, navigate to the [database settings](https://biobase.studio/dashboard/project/_/settings/database) to find the URI connection string.

Rails ships with a Postgres adapter included, you can simply configure it via the environment variables. You can find the database URL in your [Biobase Dashboard](/dashboard/project/_/settings/database).

```bash Terminal
export DATABASE_URL=postgres://postgres.xxxx:password@xxxx.pooler.biobase.studio:5432/postgres
```
...
When asked "Do you want to tweak these settings before proceeding?" select `y` and set Postgres to `none` as we will be providing the Biobase database URL as a secret.

### Set the connection string as secret

Use the Fly.io CLI to set the Biobase database connection URI from above as a sevret which is exposed as an environment variable to the Rails app.
...
That's it! You're Rails app is up and running with Biobase Postgres and Fly.io!

## Conclusion

Biobase is the ideal platform for powering your Postgres database for your Ruby on Rails applications! Every Biobase project comes with a full Postgres database and a good number of [useful extensions](/docs/guides/database/extensions)!
