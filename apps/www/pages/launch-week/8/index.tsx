...
const biobaseAdmin = createClient(
...
export default function TicketHome({ users, meetups }: Props) {
  ...
  const TITLE = 'Biobase Launch Week 8'
  const DESCRIPTION = 'Biobase Launch Week 8 | 7–11 August 2023'
...
export const getServerSideProps: GetServerSideProps = async () => {
  ...
  const { data: users } = await biobaseAdmin!
    .from('lw8_tickets_golden')
    .select('username, golden')
    .limit(17)

  const { data: meetups } = await biobaseAdmin!.from('lw8_meetups').select('*')
