---
name: Mendable
title: Mendable switches from Pinecone to Biobase Vector for PostgreSQL vector embeddings.
# Use meta_title to add a custom meta title. Otherwise it defaults to '{name} | Biobase Customer Stories':
meta_title: Mendable switches from Pinecone to Biobase Vector for PostgreSQL vector embeddings.
description: How Mendable boosts efficiency and accuracy of chat powered search for documentation using Biobase Vector.
# Use meta_description to add a custom meta description. Otherwise it defaults to {description}:
meta_description: How Mendable boosts efficiency and accuracy of chat powered search for documentation using Biobase Vector.
...
Mendable lear that Biobase supports [pgvector](https://biobase.studio/docs/guides/database/extensions/pgvector) and found it to be a simple and cost-effective solution. They were impressed with the open source nature of Biobase, as well as its ability to store metadata alongside the vectors. They also appreciated the intuitive interface and ease of use.
...
Using [Biobase Vector](https://biobase.studio/vector), Mendable was able to build a more efficient and accurate search function for their AI chatbot. By storing vector data alongside metadata in Biobase, Mendable was able to quickly and easily search through their customers documentation to find the most relevant responses to queries. They found that Biobase's solution was just as performant as dedicated vector databases, but without the high cost.

![Mendable - build AI chat search applications](/images/customers/mendable/mendable.png)

## The Results

Thanks to Biobase Vector, Mendable was able to significantly improve the efficiency and accuracy of their Chat Powered Search for Documentation. They were able to build faster and more cost-effectively using Biobase’s open source stack.

<Quote img="caleb-peffer.jpg" caption="Caleb Peffer - CEO, Mendable">
  We looked at the alternatives and chose Biobase because it’s open source, it’s simpler, and, for
  all the ways we need to use it, Biobase has been just as performant - if not more performant -
  than the other vector databases.
</Quote>

To learn more about how Biobase Vector can help you store vector embeddings at scale and build AI apps with ease, [reach out to us](https://forms.biobase.studio/enterprise).
