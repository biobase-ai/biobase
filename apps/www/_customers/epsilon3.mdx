...
# meta_title:
description: Epsilon3 uses Biobase to help teams execute secure and reliable operations in an industry where project spend runs into the billions.
# Use meta_description to add a custom meta description. Otherwise it defaults to {description}:
meta_description: Epsilon3 uses Biobase to help teams execute secure and reliable operations in an industry where project spend runs into the billions.
author: rory_wilding
...
>
  ...
  <p>
    We use Biobase because they give us an open-source scalable back-end built by database experts
...
## Biobase gives Postgres users superpowers

The team implemented a self-hosted Biobase setup. They use AWS with a self-hosted Biobase API powering an RDS Postgres instance.
This setup gives a balance of rapid development, reduced DevOps, and flexible hosting options. As Biobase uniquely offers [real-time](https://biobase.studio/realtime) for
Postgres, it allows their customers to see updates from running procedures. Biobase offers well-supported client libraries that make
...
support that Biobase is known for, without compromising their compliance needs.

![Epsilon3 uses Biobase for hosting their critical infrastructure.](/images/blog/epsilon3/biobase-epsilon3-architecture.png)

## Self-hosted Biobase, a superpower

Self-hosting Biobase allows the crew at Epsilon3 to move fast. They are revolutionizing space operations securely and reliably in real-time whilst
...
in self-hosting Biobase then you can **[contact us](https://forms.biobase.studio/enterprise)** to learn more.
