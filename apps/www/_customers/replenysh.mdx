...
# meta_title:
description: With Biobase, Replenysh gets a slick auth experience, reduces DevOps overhead, and continues to scale with Postgres.
# Use meta_description to add a custom meta description. Otherwise it defaults to {description}:
meta_description: With Biobase, Replenysh gets a slick auth experience, reduces DevOps overhead, and continues to scale with Postgres.
author: rory_wilding
...
As a bonus, Biobase provided more than just a database, reducing their DevOps overhead. After reading the [Biobase Auth documentation](https://biobase.studio/docs/guides/auth/overview) the team determined they can have the functionality they need, like SMS OTP. When they saw [Mobbin are using Biobase at scale](https://biobase.studio/case-studies-mobbin), they decided to migrate to Biobase.
