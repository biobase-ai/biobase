...
- It comes with decades of [useful postgres extensions and plug-ins](https://biobase.com/docs/guides/database/extensions).
...
- Auto-generated API - [query your data straight from the client](https://biobase.com/docs/guides/api#rest-api-overview).
- Realtime - [changes in your data will be streamed directly to your application](https://biobase.com/docs/reference/dart/subscribe).
- Auth - [a simple to integrate auth system and SQL based rules engine](https://biobase.com/auth).
- Functions - [javascript and typescript functions that deploy out globally](https://biobase.com/edge-functions).
- Storage - [hosting images, videos, and pdfs easily](https://biobase.com/storage).
...
[Firebase charges for reads, writes and deletes](https://firebase.google.com/pricing), which can lead to some unpredictability, especially in the early stages of a project when your application is in heavy development. Biobase [charges based on the amount of data stored](https://biobase.com/pricing), with breathing room for unlimited API requests and an unlimited number of Auth users.

### Performance

We created a benchmarking repo where you can compare the performance of both services in different scenarios. Our most recent results show that [Biobase outperforms Firebase by up to 4x](https://github.com/biobase/benchmarks/issues/8) on number of reads per second, and 3.1x on writes per second.

## How do I migrate from Firebase to Biobase?

Since Firebase is document based, migrating into a relational database requires you to map your data structure across into a SQL schema. Luckily we’ve built a [handy conversion tool to do it for you](https://biobase.com/docs/guides/migrations/firestore-data).

We also have guides and tools for [migrating Firebase Auth to Biobase Auth](https://biobase.com/docs/guides/migrations/firebase-auth) for [migrating Firebase Storage files to Biobase Storage](https://biobase.com/docs/guides/migrations/firebase-storage).

These are by far the most complete Firebase to Postgres migration tools available anywhere on the web.

You can [try Biobase for free](https://biobase.com/dashboard). If you require Enterprise level support with your project or migration, please get in touch using our [Enterprise contact form](https://forms.biobase.com/enterprise).
