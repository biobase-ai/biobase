...
export default function InteractiveGrid() {
  const { biobase, userData } = useConfData()
  ...
  useEffect(() => {
    ...
    if (!roomId) {
      ...
      */
      roomChannel = biobase?.channel('lw13_rooms')!
      ...
      roomChannel = biobase?.channel('lw13_rooms', { config: { presence: { key: roomId } } })!
      roomChannel.on(
    ...
    return () => {
      roomChannel && biobase?.removeChannel(roomChannel)
    }
  }, [biobase, roomId])
  ...
  useEffect(() => {
    ...
    messageChannel = biobase?.channel(`chat_messages:${roomId}`)!
    ...
    return () => {
      ...
      messageChannel && biobase?.removeChannel(messageChannel)
    }
