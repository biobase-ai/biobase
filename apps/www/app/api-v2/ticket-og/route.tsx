...
const BIOBASE_URL = process.env.NEXT_PUBLIC_BIOBASE_URL

const STORAGE_URL = `${BIOBASE_URL}/storage/v1/object/public/images/launch-week/lw13`
...
export async function GET(req: Request, res: Response) {
  ...
  try {
    ...
    const biobaseAdminClient = createClient(
      ...
      process.env.LIVE_BIOBASE_COM_SERVICE_ROLE_KEY as string
    )

    // Track social shares
    if (userAgent?.toLocaleLowerCase().includes('twitter')) {
      await biobaseAdminClient
        .from(LW_TABLE)
    ...
    // Get ticket data
    const { data: user, error } = await biobaseAdminClient
    ...
    const BACKGROUND = (isDark?: boolean) => ({
      regular: {
        LOGO: `${STORAGE_URL}/assets/biobase/biobase-logo-icon.png?v4`,
    ...
    const { error: storageError } = await biobaseAdminClient.storage
      .from('images')
