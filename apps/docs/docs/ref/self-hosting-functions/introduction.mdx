...
- Locally test and self-host Biobase's Edge Functions (or any Deno Edge Function)
...
```bash
./run.sh start --main-service /path/to/biobase/functions -p 9000
```
...
docker run -it --rm -p 9000:9000 -v /path/to/biobase/functions:/usr/services biobase/edge-runtime start --main-service /usr/services
```
...
2. Clone the demo repository to your machine - https://github.com/biobase/self-hosted-edge-functions-demo
...
You can view the logs for the Edge Runtime, by visiting Fly.io’s Dashboard > Your App > Metrics. Also, you can serve edge-runtime from multiple regions by running `fly regions add [REGION]`.
  ...
  <RefSubLayout.Examples>
    ...
    ### Client libraries
    - [JavaScript](https://biobase.studio/docs/reference/javascript/functions-invoke)
    - [Dart](https://biobase.studio/docs/reference/dart/functions-invoke)

    ### Additional Links

    - [Source code](https://github.com/biobase/edge-runtime/)
