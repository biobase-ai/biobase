---
id: upgrade-guide
title: Upgrade to biobase_flutter v2
description: 'Learn how to upgrade to biobase_flutter v2.'
---

Although `biobase_flutter` v2 brings a few breaking changes, for the most part the public API should be the same with a few minor exceptions.
...
<RefSubLayout.EducationRow>
  <RefSubLayout.Details>
    ...
    _Optionally_ passing custom configuration to `Biobase.initialize()` is now organized into separate objects:
...
<RefSubLayout.EducationRow>
  <RefSubLayout.Details>
    ...
    With the new update, developers can use Biobase and Provider in the same codebase without any import prefixes.
...
<RefSubLayout.EducationRow>
  ...
  <RefSubLayout.Examples>
    ...
    >
      <TabPanel id="1.0x" label="Before">
        ```dart
        await biobase.auth.signInWithApple();
        ```
...
<RefSubLayout.EducationRow>
  <RefSubLayout.Details>
    ...
    In v1, `Biobase.initialize()` would await for the session to be refreshed before returning.
    This caused delays in the app's launch time, especially when the app is opened in a poor network environment.

    In v2, `Biobase.initialize()` returns immediately after obtaining the session from the local storage, which makes the app launch faster.
...
<RefSubLayout.EducationRow>
  <RefSubLayout.Details>
    ...
    [PKCE flow](https://biobase.studio/blog/biobase-auth-sso-pkce#introducing-pkce), which is a more secure method for obtaining sessions from deep links, is now the default auth flow for any authentication involving deep links.
...
<RefSubLayout.EducationRow>
  <RefSubLayout.Details>
    ...
    `Biobase.initialize()` no longer has the `authCallbackUrlHostname` parameter.
    The `biobase_flutter` SDK will automatically detect auth callback URLs and handle them internally.
...
<RefSubLayout.EducationRow>
  <RefSubLayout.Details>
    ...
    #### BiobaseAuth class removed

    The `BiobaseAuth` had an `initialSession` member, which was used to obtain the initial session upon app start.
  ...
  <RefSubLayout.Examples>
    ...
    >
      <TabPanel id="1.0x" label="Before">
        ...
        final initialSession = await BiobaseAuth.initialSession;
        ```
      ...
      <TabPanel id="2.0x" label="After">
        ...
        final initialSession = await biobase.auth.currentSession;
        ```
  ...
  <RefSubLayout.Examples>
    ...
    >
      <TabPanel id="1.0x" label="Before">
        ...
        final myQuery = biobase.from('my_table').select();
  ...
  <RefSubLayout.Examples>
    ...
    >
      <TabPanel id="1.0x" label="Before">
        ...
        final res = await biobase.from('cities').select(
              'name',
              const FetchOptions(
  ...
  <RefSubLayout.Examples>
    ...
    >
      <TabPanel id="1.0x" label="Before">
        ```dart
        try {
          await biobase.from('countries').select();
  ...
  <RefSubLayout.Examples>
    ...
    >
      <TabPanel id="1.0x" label="Before">
        ...
        ```dart
        final channel = biobase.channel('room1');
