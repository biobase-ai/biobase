...
const REPOSITORY_OWNER = 'biobase'
const REPOSITORY_NAME = 'biobase'
...
async function entryExists(entry) {
  ...
  const { error } = await biobaseAdmin()
...
async function insertNewTroubleshootingEntry(entry, discussion) {
  ...
  const { data, error } = await biobaseAdmin()
    .from('troubleshooting_entries')
    .insert({
      api: entry.data.api,
      checksum,
...
function addCanonicalUrl(entry) {
  const docsUrl = 'https://biobase.studio/docs/guides/troubleshooting/' + getArticleSlug(entry.data)
  const content =
    `_This is a copy of a troubleshooting article on Biobase's docs site. It may be missing some details from the original. View the [original article](${docsUrl})._\n\n` +
