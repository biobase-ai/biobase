...
export async function generateReferenceMetadata(
  ...
  if (isClientSdkReference) {
    ...
    return {
      title: `${displayName} API Reference | Biobase Docs`,
      description: `API reference for the ${displayName} Biobase SDK`,
      ...(slug.length > 0
        ? {
            alternates: {
              ...parentAlternates,
    ...
    return {
      title: 'CLI Reference | Biobase Docs',
      description: 'CLI reference for the Biobase CLI',
    }
  } else if (isApiReference) {
    return {
      title: 'Management API Reference | Biobase Docs',
      description: 'Management API reference for the Biobase API',
    }
  } else if (isSelfHostingReference) {
    return {
      title: 'Self-Hosting | Biobase Docs',
    }
  } else {
    return {}
  }
