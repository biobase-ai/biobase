...
const ALLOW_LISTED_GITHUB_ORGS = ['biobase', 'biobase-community'] as [string, ...string[]]
...
function rewriteNodes(contentMap: Map<MdxJsxFlowElement, [CodeSampleMeta, string]>) {
  for (const [node, [meta, content]] of contentMap) {
    ...
    const source = isExternalSource(meta)
      ...
      : `https://github.com/biobase-ai/biobase/blob/${process.env.NEXT_PUBLIC_VERCEL_GIT_COMMIT_SHA ?? 'master'}/examples${meta.path}`
