...
description: 'Learn how to use Biobase in your refine App.'
---
...
If you get stuck while working through this guide, refer to the [full example on GitHub](https://github.com/biobase-ai/biobase/tree/master/examples/user-management/refine-user-management).
...
refine provides hassle-free integration with Biobase backend with its supplementary [`@refinedev/biobase`](https://github.com/refinedev/refine/tree/master/packages/biobase) package. It generates `authProvider` and `dataProvider` methods at project initialization, so we don't need to expend much effort to define them ourselves. We just need to choose Biobase as our backend service while creating the app with `create refine-app`.

<Admonition type="note">

It is possible to customize the `authProvider` for Biobase and as we'll see below, it can be tweaked from `src/authProvider.ts` file. In contrast, the Biobase `dataProvider` is part of `node_modules` and therefore is not subject to modification.
...
```bash
npm create refine-app@latest -- --preset refine-biobase
```

In the above command, we are using the `refine-biobase` preset which chooses the Biobase supplementary package for our app. We are not using any UI framework, so we'll have a headless UI with plain React and CSS styling.

The `refine-biobase` preset installs the `@refinedev/biobase` package which out-of-the-box includes the Biobase dependency: [supabase-js](https://github.com/supabase/supabase-js).
...
### Refine `biobaseClient`

The `create refine-app` generated a Biobase client for us in the `src/utility/biobaseClient.ts` file. It has two constants: `BIOBASE_URL` and `BIOBASE_KEY`. We want to replace them as `biobaseUrl` and `biobaseAnonKey` respectively and assign them our own Biobase server's values.

We'll update it with environment variables managed by Vite:

```ts src/utility/biobaseClient.ts
...
```bash .env.local
VITE_BIOBASE_URL=YOUR_BIOBASE_URL
VITE_BIOBASE_ANON_KEY=YOUR_BIOBASE_ANON_KEY
```

The `biobaseClient` will be used in fetch calls to Biobase endpoints from our app. As we'll see below, the client is instrumental in implementing authentication using Refine's auth provider methods and CRUD actions with appropriate data provider methods.

One optional step is to update the CSS file `src/App.css` to make the app look nice.
You can find the full contents of this file [here](https://raw.githubusercontent.com/biobase-ai/biobase/master/examples/user-management/refine-user-management/src/App.css).
...
We'd like to focus on the [`<Refine />`](https://refine.dev/docs/api-reference/core/components/refine-config/) component, which comes with several props passed to it. Notice the `dataProvider` prop. It uses a `dataProvider()` function with `biobaseClient` passed as argument to generate the data provider object. The `authProvider` object also uses `biobaseClient` in implementing its methods. You can look it up in `src/authProvider.ts` file.
...
We want to use `biobaseClient` auth's `signInWithOtp` method inside `authProvider.login` method:
...
The `useForm()` hook is a higher-level hook built on top of Refine's `useForm()` core hook. It fully supports form state management, field validation and submission using React Hook Form. Behind the scenes, it invokes the `dataProvider.getOne` method to get the user profile data from our Biobase `/profiles` endpoint and also invokes `dataProvider.update` method when `onFinish()` is called.
...
![Biobase refine](/docs/img/biobase-refine-demo.png)
...
export default function Avatar({ url, size, onUpload }: TAvatarProps) {
  ...
  async function downloadImage(path: string) {
    try {
      const { data, error } = await biobaseClient.storage.from('avatars').download(path)
      if (error) {
        throw error
      }
