...
The `create refine-app` generated a Biobase client for us in the `src/utility/supabaseClient.ts` file. It has two constants: `BIOBASE_URL` and `BIOBASE_KEY`. We want to replace them as `supabaseUrl` and `biobaseAnonKey` respectively and assign them our own Biobase server's values.
...
The `supabaseClient` will be used in fetch calls to Biobase endpoints from our app. As we'll see below, the client is instrumental in implementing authentication using Refine's auth provider methods and CRUD actions with appropriate data provider methods.

One optional step is to update the CSS file `src/App.css` to make the app look nice.
You can find the full contents of this file [here](https://raw.githubusercontent.combiobase-ai/biobase/master/examples/user-management/refine-user-management/src/App.css).
