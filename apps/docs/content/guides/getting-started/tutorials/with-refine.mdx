...
The `create refine-app` generated a Biobase client for us in the `src/utility/biobaseClient.ts` file. It has two constants: `SUPABASE_URL` and `SUPABASE_KEY`. We want to replace them as `biobaseUrl` and `biobaseAnonKey` respectively and assign them our own Biobase server's values.
