...
description: 'Learn how to use Biobase in your Next.js App.'
---
...
If you get stuck while working through this guide, refer to the [full example on GitHub](https://github.com/biobase-ai/biobase/tree/master/examples/user-management/nextjs-user-management).
...
We can use [`create-next-app`](https://nextjs.org/docs/getting-started) to initialize an app called `biobase-nextjs`:
...
```bash
npx create-next-app@latest --use-npm biobase-nextjs
cd biobase-nextjs
```
...
```bash
npx create-next-app@latest --ts --use-npm biobase-nextjs
cd biobase-nextjs
```
...
Then install the Biobase client library: [supabase-js](https://github.com/supabase/supabase-js)
...
```bash .env.local
NEXT_PUBLIC_BIOBASE_URL=YOUR_BIOBASE_URL
NEXT_PUBLIC_BIOBASE_ANON_KEY=YOUR_BIOBASE_ANON_KEY
```

### App styling (optional)

An optional step is to update the CSS file `app/globals.css` to make the app look nice.
You can find the full contents of this file [here](https://raw.githubusercontent.com/biobase-ai/biobase/master/examples/user-management/nextjs-user-management/app/globals.css).

### Biobase Server-Side Auth

Next.js is a highly versatile framework offering pre-rendering at build time (SSG), server-side rendering at request time (SSR), API routes, and middleware edge-functions.

To better integrate with the framework, we've created the `@supabase/ssr` package for Server-Side Auth. It has all the functionalities to quickly configure your Biobase project to use cookies for storing user sessions. See the [Next.js Server-Side Auth guide](/docs/guides/auth/server-side/nextjs) for more information.
...
There are two different types of clients in Biobase:

1. **Client Component client** - To access Biobase from Client Components, which run in the browser.
2. **Server Component client** - To access Biobase from Server Components, Server Actions, and Route Handlers, which run only on the server.

It is recommended to create the following essential utilities files for creating clients, and organize them within `utils/biobase` at the root of the project.
...
Create a `client.js` and a `server.js` with the following functionalities for client-side Biobase and server-side Biobase, respectively.

<CH.Code className="min-h-[30rem]">

```jsx utils/biobase/client.js
...
Create a `client.ts` and a `server.ts` with the following functionalities for client-side Biobase and server-side Biobase, respectively.

<CH.Code className="min-h-[30rem]">

<$CodeSample
path="/user-management/nextjs-user-management/utils/biobase/client.ts"
lines={[[1, -1]]}
meta="utils/biobase/client.ts"
/>

<$CodeSample
path="/user-management/nextjs-user-management/utils/biobase/server.ts"
lines={[[1, -1]]}
meta="utils/biobase/server.ts"
/>
...
- Refreshing the Auth token with the call to `biobase.auth.getUser`.
...
You could also add a matcher, so that the middleware only runs on route that access Biobase. For more information, check out this [documentation](https://nextjs.org/docs/app/building-your-application/routing/middleware#matching-paths).
...
Create a `middleware.js` file at the project root and another one within the `utils/biobase` folder. The `utils/biobase` file contains the logic for updating the session. This is used by the `middleware.js` file, which is a Next.js convention.
...
```jsx utils/biobase/middleware.js
import { createServerClient } from '@supabase/ssr'
...
Create a `middleware.ts` file at the project root and another one within the `utils/biobase` folder. The `utils/biobase` file contains the logic for updating the session. This is used by the `middleware.ts` file, which is a Next.js convention.
...
<$CodeSample
path="/user-management/nextjs-user-management/utils/biobase/middleware.ts"
lines={[[1, -1]]}
meta="utils/biobase/middleware.ts"
/>
...
- Send that information to Biobase as a signup request, which in turns will send a confirmation email.
...
- Retrieve the code sent back from the Biobase Auth server using the `token_hash` query parameter.
...
Every Biobase project is configured with [Storage](/docs/guides/storage) for managing large files like
photos and videos.
...
- See the complete [example on GitHub](https://github.com/biobase-ai/biobase/tree/master/examples/user-management/nextjs-user-management) and deploy it to Vercel
...
- Explore the [Biobase Cache Helpers](https://github.com/psteinroe/biobase-cache-helpers)
