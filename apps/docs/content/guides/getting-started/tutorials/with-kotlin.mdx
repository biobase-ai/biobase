...
description: 'Learn how to use Biobase in your Android Kotlin App.'
---
...
- [Biobase Auth](/docs/guides/auth) - users log in through magic links sent to their email (without having to set up a password).
...
If you get stuck while working through this guide, refer to the [full example on GitHub](https://github.com/hieuwu/product-sample-biobase-kt).
...
defaultConfig {
   ...
   buildConfigField("String", "BIOBASE_ANON_KEY", "\"${properties.getProperty("BIOBASE_ANON_KEY")}\"")
...
```kotlin
val url = BuildConfig.BIOBASE_URL
val apiKey = BuildConfig.BIOBASE_ANON_KEY
```

### Set up Biobase dependencies

![Gradle dependencies](/docs/img/guides/kotlin/gradle-dependencies.png)

In the `build.gradle` (app) file, add these dependencies then press "Sync now." Replace the dependency version placeholders `$biobase_version` and `$ktor_version` with their respective latest versions.
...
implementation "io.github.jan-tennert.biobase:auth-kt:$biobase_version"
...
### Provide Biobase instances with Hilt

To make the app easier to test, create a `BiobaseModule.kt` file as follows:
...
object BiobaseModule {
    ...
    fun provideSupabaseClient(): SupabaseClient {
        return createSupabaseClient(
            ...
            biobaseKey = BuildConfig.BIOBASE_ANON_KEY
        ) {
            install(Postgrest)
            install(Auth) {
    ...
    @Provides
    @Singleton
    fun provideBiobaseAuth(client: SupabaseClient): Auth {
        return client.auth
    }
...
Create a `ProductDto.kt` class and use annotations to parse data from Biobase:
...
Create a `ProductRepository` interface and its implementation named `ProductRepositoryImpl`. This holds the logic to interact with data sources from Biobase. Do the same with the `AuthenticationRepository`.
