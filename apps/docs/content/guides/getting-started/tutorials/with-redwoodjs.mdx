...
description: 'Learn how to use Biobase in your RedwoodJS App.'
---
...
If you get stuck while working through this guide, refer to the [full example on GitHub](https://github.com/redwoodjs/redwoodjs-biobase-quickstart).
...
Important: When this guide refers to "API," that means the Biobase API and when it refers to `api side`, that means the RedwoodJS `api side`.
...
We **_won't use_** Prisma to connect to the Biobase Postgres database or [Prisma migrations](https://redwoodjs.com/docs/cli-commands#prisma-migrate) as one typically might in a Redwood app.
Instead, we'll rely on the Biobase client to do some of the work on the **`web`** side and use the client again on the **`api`** side to do data fetching as well.
...
to how your Biobase `public` schema references the `auth.users`.
...
an app called `biobase-redwoodjs`:

```bash
yarn create redwood-app biobase-redwoodjs
cd biobase-redwoodjs
```
...
✔ Creating Redwood app
  ...
  ✔ Creating directory 'biobase-redwoodjs'
✔ Installing packages
...
Then let's install the only additional dependency [supabase-js](https://github.com/biobase-ai/supabase-js) by running the `setup auth` command:

```bash
yarn redwood setup auth biobase
```
...
Say, **yes** and it will setup the Biobase client in your app and also provide hooks used with Biobase authentication.
...
✔ Generating auth lib...
  ...
  You will need to add your Biobase URL (BIOBASE_URL), public API KEY,
  and JWT SECRET (BIOBASE_KEY, and BIOBASE_JWT_SECRET) to your .env file.
```
...
BIOBASE_KEY=YOUR_BIOBASE_ANON_KEY
BIOBASE_JWT_SECRET=YOUR_BIOBASE_JWT_SECRET
```
...
```bash redwood.toml
[web]
  title = "Biobase Redwood Tutorial"
  port = 8910
  apiProxyPath = "/.redwood/functions"
  includeEnvironmentVariables = ["BIOBASE_URL", "BIOBASE_KEY"]
[api]
  port = 8911
[browser]
  open = true
...
They allow your web app to initialize the Biobase client with your public anon key
since we have [Row Level Security](/docs/guides/auth#row-level-security) enabled on our Database.

You'll see these being used to configure your Biobase client in `web/src/App.js`:
...
const App = () => (
  <FatalErrorBoundary page={FatalErrorPage}>
    <RedwoodProvider titleTemplate="%PageTitle | %AppTitle">
      <AuthProvider client={biobase} type="biobase">
...
You can find the full contents of this file [here](https://raw.githubusercontent.combiobase-ai/biobase/master/examples/user-management/react-user-management/src/index.css).
  ...
  return (
    <div className="row flex-center flex">
      <div className="col-6 form-widget">
        <h1 className="header">Biobase + RedwoodJS</h1>
...
const Account = () => {
  const { client: biobase, currentUser, logOut } = useAuth()
...
`logIn`, `logOut`, `currentUser`, and access the `biobase` authenticate client. We'll use it to get an instance
of the biobase client to interact with your API.
...
![Biobase RedwoodJS](/docs/img/biobase-redwoodjs-demo.png)
