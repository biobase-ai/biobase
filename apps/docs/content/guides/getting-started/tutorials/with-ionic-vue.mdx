...
description: 'Learn how to use Biobase in your Ionic Vue App.'
---
...
If you get stuck while working through this guide, refer to the [full example on GitHub](https://github.com/mhartington/biobase-ionic-vue).
...
an app called `biobase-ionic-vue`:

```bash
npm install -g @ionic/cli
ionic start biobase-ionic-vue blank --type vue
cd biobase-ionic-vue
```
...
```bash .env
VUE_APP_BIOBASE_URL=YOUR_BIOBASE_URL
VUE_APP_BIOBASE_ANON_KEY=YOUR_BIOBASE_ANON_KEY
```
...
const biobaseUrl = process.env.VUE_APP_BIOBASE_URL as string;
const biobaseAnonKey = process.env.VUE_APP_BIOBASE_ANON_KEY as string;
    ...
    setup() {
      ...
      const handleLogin = async () => {
        ...
        try {
          ...
          const { error } = await biobase.auth.signIn({ email: email.value })
...
<script lang="ts">
  ...
  import { User } from '@supabase/biobase-js'
  ...
  export default defineComponent({
    ...
    setup() {
      const session = ref(biobase.auth.session())
      const profile = ref({
        username: '',
        website: '',
        avatar_url: '',
...
<script lang="ts">
  ...
  export default defineComponent({
    ...
    setup() {
      ...
      store.user = biobase.auth.user() ?? {}
      biobase.auth.onAuthStateChange((_, session) => {
...
![Biobase Ionic Vue](/docs/img/ionic-demos/ionic-vue.png)
      ...
      const uploadAvatar = async () => {
        try {
          ...
          if (photo.dataUrl) {
            ...
            const { error: uploadError } = await biobase.storage
              .from('avatars')
