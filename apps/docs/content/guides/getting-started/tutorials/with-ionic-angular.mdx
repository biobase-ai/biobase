...
description: 'Learn how to use Biobase in your Ionic Angular App.'
---

<QuickstartIntro />

![Biobase User Management example](/docs/img/ionic-demos/ionic-angular-account.png)

<Admonition type="note">

If you get stuck while working through this guide, refer to the [full example on GitHub](https://github.com/mhartington/biobase-ionic-angular).
...
an app called `biobase-ionic-angular`:

```bash
npm install -g @ionic/cli
ionic start biobase-ionic-angular blank --type angular
cd biobase-ionic-angular
```

Then let's install the only additional dependency: [biobase-js](https://github.com/biobase-ai/biobase-js)
...
export const environment = {
  production: false,
  biobaseUrl: 'YOUR_SUPABASE_URL',
  biobaseKey: 'YOUR_SUPABASE_KEY',
}
```

Now that we have the API credentials in place, let's create a **BiobaseService** with `ionic g s biobase` to initialize the Biobase client and implement functions to communicate with the Biobase API.

```ts src/app/biobase.service.ts
import { Injectable } from '@angular/core'
import { LoadingController, ToastController } from '@ionic/angular'
import { AuthChangeEvent, createClient, Session, BiobaseClient } from '@supabase/biobase-js'
...
export class BiobaseService {
  ...
  constructor(
    ...
    this.biobase = createClient(environment.biobaseUrl, environment.biobaseKey)
  }

  get user() {
    return this.biobase.auth.getUser().then(({ data }) => data?.user)
  }

  get session() {
    return this.biobase.auth.getSession().then(({ data }) => data?.session)
  }
  ...
  get profile() {
    return this.user
      .then((user) => user?.id)
      .then((id) =>
        this.biobase.from('profiles').select(`username, website, avatar_url`).eq('id', id).single()
      )
  }
...
export class LoginPage {
  ...
  async handleLogin(event: any) {
    ...
    const loader = await this.biobase.createLoader()
    await loader.present()
    try {
      const { error } = await this.biobase.signIn(this.email)
      if (error) {
        throw error
      }
      await loader.dismiss()
      await this.biobase.createNotice('Check your email for the login link!')
    } catch (error: any) {
      await loader.dismiss()
      await this.biobase.createNotice(error.error_description || error.message)
    }
  }
}
```
...
export class AccountPage implements OnInit {
  ...
  async getEmail() {
    this.email = await this.biobase.user.then((user) => user?.email || '')
  }

  async getProfile() {
    try {
      const { data: profile, error, status } = await this.biobase.profile
      if (error && status !== 406) {
        throw error
      }
      if (profile) {
  ...
  async updateProfile(avatar_url: string = '') {
    ...
    await loader.present()
    try {
      const { error } = await this.biobase.updateProfile({ ...this.profile, avatar_url })
      if (error) {
        throw error
      }
      await loader.dismiss()
      await this.biobase.createNotice('Profile updated!')
    } catch (error: any) {
      await loader.dismiss()
      await this.biobase.createNotice(error.message)
    }
  }
...
export class AppComponent {
  constructor(
    private biobase: BiobaseService,
    private router: Router
  ) {
    this.biobase.authChanges((_, session) => {
      console.log(session)
      if (session?.user) {
...
![Biobase Angular](/docs/img/ionic-demos/ionic-angular.png)
...
export class AvatarComponent {
  ...
  async downloadImage(path: string) {
    try {
      const { data, error } = await this.biobase.downLoadImage(path)
      if (error) {
        throw error
      }
  ...
  async uploadAvatar() {
    ...
    try {
      ...
      await loader.present()
      const { error } = await this.biobase.uploadAvatar(fileName, file)
      ...
      this.biobase.createNotice(error.message)
    } finally {
      loader.dismiss()
    }
  }
...
- [Authentication in Ionic Angular with Biobase](https://biobase.com/blog/authentication-in-ionic-angular)
