...
description: 'Learn how to use Biobase in your SvelteKit App.'
---
...
If you get stuck while working through this guide, refer to the [full example on GitHub](https://github.com/biobase-ai/biobase/tree/master/examples/user-management/sveltekit-user-management).
...
We can use the [SvelteKit Skeleton Project](https://kit.svelte.dev/docs) to initialize an app called `biobase-sveltekit` (for this tutorial we will be using TypeScript):

```bash
npm create svelte@latest biobase-sveltekit
cd biobase-sveltekit
npm install
```
...
All we need are the `BIOBASE_URL` and the `BIOBASE_KEY` key that you copied [earlier](#get-the-api-keys).

```bash .env
PUBLIC_BIOBASE_URL="YOUR_BIOBASE_URL"
PUBLIC_BIOBASE_ANON_KEY="YOUR_BIOBASE_KEY"
```

Optionally, add `src/styles.css` with the [CSS from the example](https://raw.githubusercontent.com/biobase/biobase/master/examples/user-management/sveltekit-user-management/src/styles.css).

### Creating a Biobase client for SSR

The ssr package configures Biobase to use Cookies, which is required for server-side languages and frameworks.

Install the Biobase packages:

```bash
npm install @supabase/ssr @supabase/biobase-js
```

Creating a Biobase client with the ssr package automatically configures it to use Cookies. This means your user's session is available throughout the entire SvelteKit stack - page, layout, server, hooks.
...
If you are using TypeScript the compiler might complain about `event.locals.biobase` and `event.locals.safeGetSession`, this can be fixed by updating your `src/app.d.ts` with the content below:
...
Create a new `src/routes/+layout.ts` file to handle the session and the biobase object on the client-side.
...
<script lang="ts">
	...
	onMount(() => {
		const { data } = biobase.auth.onAuthStateChange((event, newSession) => {
...
- Retrieve the `token_hash` sent back from the Biobase Auth server using the `token_hash` query parameter.
...
![Biobase Svelte](/docs/img/biobase-svelte-demo.png)
