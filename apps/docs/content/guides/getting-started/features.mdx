...
Edge functions natively support NPM modules and Node built-in APIs. [Link](https://biobase.com/blog/edge-functions-node-npm).
...
The alpha stage indicates that the API might change in the future, not that the service isn’t stable. Even though the [uptime Service Level Agreement](https://biobase.com/sla) does not cover products in Alpha, we do our best to have the service as stable as possible.
...
In addition to the Beta requirements, features in GA are covered by the [uptime SLA](https://biobase.com/sla).
