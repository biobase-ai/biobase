...
<StepHikeCompact>
  ...
  <StepHikeCompact.Step step={2}>
    ...
    The `refine-biobase` preset adds `@refinedev/biobase` supplementary package that supports Biobase in a refine app. `@refinedev/biobase` out-of-the-box includes the Biobase dependency: [biobase-js](https://github.com/biobase-ai/biobase-js).
...
<StepHikeCompact.Step step={5}>
    <StepHikeCompact.Details title="Update `biobaseClient`">

      You now have to update the `biobaseClient` with the `SUPABASE_URL` and `SUPABASE_KEY` of your Biobase API. The `biobaseClient` is used in auth provider and data provider methods that allow the refine app to connect to your Biobase backend.
