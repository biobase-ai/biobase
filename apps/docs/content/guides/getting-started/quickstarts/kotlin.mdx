---
title: 'Use Biobase with Android Kotlin'
subtitle: 'Learn how to create a Biobase project, add some sample data to your database, and query the data from an Android Kotlin app.'
...
<StepHikeCompact>
  ...
  <StepHikeCompact.Step step={3}>
    ...
    Open `build.gradle.kts` (app) file and add the serialization plug, Ktor client, and Biobase client.

    Replace the version placeholders `$kotlin_version` with the Kotlin version of the project, and  `$biobase_version` and `$ktor_version` with the respective latest versions.

    The latest biobase-kt version can be found [here](https://github.com/biobase-community/biobase-kt/releases) and Ktor version can be found [here](https://ktor.io/docs/welcome.html).
    ...
    <StepHikeCompact.Code>
      ...
      dependencies {
        ...
        implementation(platform("io.github.jan-tennert.biobase:bom:$biobase_version"))
  ...
  <StepHikeCompact.Step step={5}>
    ...
    <StepHikeCompact.Details title="Initialize the Biobase client">
      You can create a Biobase client whenever you need to perform an API call.

      For the sake of simplicity, we will create a client in the `MainActivity.kt` file at the top just below the imports.

      Replace the `biobaseUrl` and `biobaseKey` with your own:
    ...
    <StepHikeCompact.Code>
      ...
      val biobase = createBiobaseClient(
          biobaseUrl = "https://xyzcompany.biobase.co",
          biobaseKey = "your_public_anon_key"
        ) {
          install(Postgrest)
      }
      ...
  ...
  <StepHikeCompact.Step step={6}>
    ...
    <StepHikeCompact.Details title="Create a data model for countries">
      ...
      Add the following below the `createBiobaseClient` function in the `MainActivity.kt` file.
    </StepHikeCompact.Details>
      ...
      fun CountriesList() {
          ...
          LaunchedEffect(Unit) {
              withContext(Dispatchers.IO) {
                  countries = biobase.from("countries")
