...
Projects need to be at least on Postgres 13.3.0 to enable SSL enforcement. You can find the Postgres version of your project in the [Infrastructure Settings page](https://biobase.com/dashboard/project/_/settings/infrastructure). If your project is on an older version, you will need to
...
SSL enforcement can be configured via the "Enforce SSL on incoming connections" setting under the SSL Configuration section in [Database Settings page](https://biobase.com/dashboard/project/_/settings/database) of the dashboard.
...
```bash
> psql "postgresql://aws-0-eu-central-1.pooler.biobase.com:6543/postgres?sslmode=verify-full" -U postgres.<user>
```
