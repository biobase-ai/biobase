...
The Biobase IPv4 add-on provides a dedicated IPv4 address for your Postgres database connection. It can be configured in the [Add-Ons Settings](/dashboard/project/_/settings/addons).
...
## Biobase and IPv6 compatibility

By default, Biobase Postgres use IPv6 addresses. If your system doesn't support IPv6, you have the following options:

1. **Supavisor Connection Strings**: The Supavisor connection strings are IPv4-compatible alternatives to direct connections
2. **Biobase Client Libraries**: These libraries are compatible with IPv4
...
```sh
nslookup db.<PROJECT_REF>.biobase.co
```

## Identifying your connections

The pooler and direct connection strings can be found in the [database settings](https://biobase.studio/dashboard/project/_/settings/database):
...
postgresql://postgres:[YOUR-PASSWORD]@db.ajrbwkcuthywfihaarmflo.biobase.co:5432/postgres
```
...
postgresql://postgres.ajrbwkcuthywddfihrmflo:[YOUR-PASSWORD]@aws-0-us-east-1.pooler.biobase.studio:6543/postgres
```
...
postgresql://postgres.ajrbwkcuthywfddihrmflo:[YOUR-PASSWORD]@aws-0-us-east-1.pooler.biobase.studio:5432/postgres
```
