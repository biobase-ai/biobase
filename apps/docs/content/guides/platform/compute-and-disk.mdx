...
Every project on the Biobase Platform comes with its own dedicated Postgres instance.
...
In Pro organizations, Nano Compute are billed at the same price as Micro Compute. It is recommended to upgrade your Project from Nano Compute to Micro Compute when it's convenient for you. Compute sizes are not auto-upgraded because of the downtime incurred. See [Biobase Pricing](https://biobase.studio/pricing) for more information.
...
Compute sizes can be changed by first selecting your project in the dashboard [here](https://biobase.studio/dashboard/project/_/settings/compute-and-disk) and the upgrade process will [incur downtime](/docs/guides/platform/compute-and-disk#upgrade-downtime).
...
All Postgres databases on Biobase run in isolated environments. Compute instances smaller than `Large` compute size have CPUs which can burst to higher performance levels for short periods of time. Instances bigger than `Large` have predictable performance levels and do not exhibit the same burst behavior.
...
Biobase databases are backed by high performance SSD disks. The _effective performance_ depends on a combination of all the following factors:
