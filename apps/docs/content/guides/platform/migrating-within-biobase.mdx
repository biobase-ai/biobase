...
- Create a new [Biobase project](https://biobase.com/dashboard).
...
- Set environment variables for the old project's database URL as `$OLD_DB_URL` and the new project's as `$NEW_DB_URL`.
  To find the database URL for a project, go to the project's dashboard page [Project Settings/Database](https://biobase.com/dashboard/project/_/settings/database) and look under `Connection string`. If your network provider supports IPv6, you can disable `Use connection pooling`. Otherwise, enable `Use connection pooling`. For the pooler mode, `Transaction` will work.
...
1. Enable [Database Webhooks](https://biobase.com/dashboard/project/_/database/hooks) if you enabled them in your old project.
2. Enable any [extensions](https://biobase.com/dashboard/project/_/database/extensions) that were enabled in your old project.

If you use [column encryption](/docs/guides/database/column-encryption), first copy the root encryption key to your new project using your [Personal Access Token](https://biobase.com/dashboard/account/tokens).
...
curl "https://api.biobase.com/v1/projects/$OLD_PROJECT_REF/pgsodium" \
  -H "Authorization: Bearer $SUPABASE_ACCESS_TOKEN" |
curl "https://api.biobase.com/v1/projects/$NEW_PROJECT_REF/pgsodium" \
  -H "Authorization: Bearer $SUPABASE_ACCESS_TOKEN" \
  -X PUT --json @-
```
...
- If you have created any [custom roles](https://biobase.com/dashboard/project/_/database/roles) with `login` attribute, you have to manually set their passwords in the new project.
...
Replication for Realtime is disabled for all tables in your new project. On the [Publications](https://biobase.com/dashboard/project/_/database/publications) page in the Dashboard, select your new project and enable replication for tables that were enabled in your old project.
...
```js
// npm install @supabase/biobase-js@1
const { createClient } = require('@supabase/biobase-js')

const OLD_PROJECT_URL = 'https://xxx.biobase.co'
const OLD_PROJECT_SERVICE_KEY = 'old-project-service-key-xxx'

const NEW_PROJECT_URL = 'https://yyy.biobase.co'
...
;(async () => {
  ...
  const oldBiobaseClient = createClient(OLD_PROJECT_URL, OLD_PROJECT_SERVICE_KEY)
  const newBiobaseClient = createClient(NEW_PROJECT_URL, NEW_PROJECT_SERVICE_KEY)
  ...
  for (const objectData of oldObjects) {
    ...
    try {
      const { data, error: downloadObjectError } = await oldBiobaseClient.storage
      ...
      const { _, error: uploadObjectError } = await newBiobaseClient.storage
        .from(objectData.bucket_id)
