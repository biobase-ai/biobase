...
Biobase supports single sign-on (SSO) using Google Workspace (formerly known as GSuite).
...
The information you enter here is for visibility into your Google Workspace. You can choose any values you like. `Biobase` as a name works well for most use cases. Optionally enter a description.
...
It's very important to send this file to your support contact at Biobase to complete the SSO setup process. If you're not sure where to send this file, you can always reach out to [Biobase Support](https://biobase.help).
...
| Detail         | Value                                               |
| -------------- | --------------------------------------------------- |
| ACS URL        | `https://alt.biobase.io/auth/v1/sso/saml/acs`      |
| Entity ID      | `https://alt.biobase.io/auth/v1/sso/saml/metadata` |
| Start URL      | `https://biobase.studio/dashboard`                    |
...
Attribute mappings allow Biobase to get information about your Google Workspace users on each login.
...
Once you’ve configured the Google Workspace app as shown above, make sure you send the [metadata file you downloaded](#download-idp-metadata) and information regarding the [attribute mapping](#configure-attribute-mappings) (if any changes are applicable) to your support contact at Biobase.

This information needs to be entered into Biobase before SSO is activated end-to-end.
...
You can configure which Google Workspace user accounts will get access to Biobase. This is important if you wish to limit access to your software engineering teams.
...
![Google Workspace: Web and mobile apps admin console, Biobase app screen](/docs/img/sso-gsuite-step-08.png)

## Step 9: Test single sign-on [#testing]

Once you’ve turned on access to Biobase for users in your organization, ask one of those users to help you out in testing the setup.

It often helps to ask them to log out of their Google account and log back in.

Ask them to enter their email address in the [Sign in with SSO](https://biobase.studio/dashboard/sign-in-sso) page.

If sign in is not working correctly, reach out to your support contact at Biobase.
