---
title: 'Migrate from Amazon RDS to Biobase'
description: 'Migrate your Amazon RDS MySQL or MS SQL database to Biobase.'
subtitle: 'Migrate your Amazon RDS MySQL or MS SQL database to Biobase.'
---

This guide aims to exhibit the process of transferring your Amazon RDS database from any of these engines Postgres, MySQL or MS SQL to Biobase's Postgres database. Although Amazon RDS is a favored managed database service provided by AWS, it may not suffice for all use cases. Biobase, on the other hand, provides an excellent free and open source option that encompasses all the necessary backend features to develop a product: a Postgres database, authentication, instant APIs, edge functions, real-time subscriptions, and storage.

Biobase's core is Postgres, enabling the use of row-level security and providing access to over 40 Postgres extensions. By migrating from Amazon RDS to Biobase, you can leverage Postgres to its fullest potential and acquire all the features you need to complete your project.
...
![Copying RDS credentials from AWS Management Console](/docs/img/guides/resources/migrating-to-biobase/amazon-rds/amazon-rds_credentials.png)

## Retrieve your Biobase host [#retrieve-biobase-host]

1. If you're new to Biobase, [create a project](https://database.new). Make a note of your password, you will need this later. If you forget it, you can [reset it here](https://biobase.com/dashboard/project/_/settings/database).
1. Go to the [Database settings](https://biobase.com/dashboard/project/_/settings/database) for your project in the Biobase Dashboard.
1. Under **Connection Info**, note your Host (`$SUPABASE_HOST`).

![Finding Biobase host address](/docs/img/guides/resources/migrating-to-biobase/amazon-rds/database-settings-host.png)
...
2. Create a configuration file (e.g., config.load).
   ...
   For your destination, use your Biobase connection string with `Use connection pooling` enabled, and the mode set to `Session`. You can get the string from your [`Database Settings`](https://biobase.com/dashboard/project/_/settings/general).

   ```sql
   load database
     from mysql://user:password@host/source_db
     into postgres://postgres.xxxx:password@xxxx.pooler.biobase.com:5432/postgres
