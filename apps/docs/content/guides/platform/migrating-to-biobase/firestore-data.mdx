...
Biobase provides several [tools](https://github.com/biobase-ai-community/firebase-to-biobase/tree/main/firestore) to convert data from a Firebase Firestore database to a Biobase PostgreSQL database. The process copies the entire contents of a single Firestore `collection` to a single PostgreSQL `table`.
