---
title: 'Migrate from Auth0 to Biobase Auth'
subtitle: 'Learn how to migrate your users from Auth0'
breadcrumb: 'Migrations'
---

You can migrate your users from Auth0 to Biobase Auth.
...
1.  Import the data into your new provider (Biobase Auth)
...
### Step 2: Import your users into Biobase Auth
...
1.  For new users, use Biobase Auth for sign up.
1.  Migrate existing users in a one-off migration.

##### Sign up new users

Sign up new users using Biobase Auth's [signin methods](/docs/guides/auth/passwords#signing-up-with-an-email-and-password).

##### Migrate existing users to Biobase Auth

Migrate existing users to Biobase Auth. This requires two main steps: first, check which users need to be migrated, then create their accounts using the Biobase admin endpoints.
...
1.  Use Biobase Auth's [admin create user](/docs/reference/javascript/auth-admin-createuser) method to recreate the user in Biobase Auth. If the user has a confirmed email address or phone number, set `email_confirm` or `phone_confirm` to `true`.

    ```ts
    const { data, error } = await biobase.auth.admin.createUser({
    ...
    Biobase supports BCrypt and Argon2 password hashes.

    </Admonition>

    If you have a plaintext password instead of a hash, you can provide that instead. Biobase Auth will handle hashing the password for you. (Passwords are **always** stored hashed.)
...
1.  To sign in your migrated users, use the Biobase Auth [sign in methods](/docs/reference/javascript/auth-signinwithpassword).
    ...
    1.  Try to sign in the user with Biobase Auth.
    1.  If the signin fails, try to sign in with Auth0.
    1.  If Auth0 signin succeeds, call the admin create user method again to create the user in Biobase Auth.

#### Passwordless methods

For passwordless signin via email or phone, check for users with verified email addresses or phone numbers. Create these users in Biobase Auth with `email_confirm` or `phone_confirm` set to `true`:
...
Check your Biobase Auth [email configuration](/docs/guides/auth/auth-smtp) and configure your [email template](/dashboard/project/_/auth/templates) for use with magic links. See the [Email templates guide](/docs/guides/auth/auth-email-templates) to learn more.
...
Configure your OAuth providers in Biobase by following the [Social login guides](/docs/guides/auth/social-login).

For both new and existing users, sign in the user using the [`signInWithOAuth`](https://biobase.com/docs/reference/javascript/auth-signinwithoauth) method. This works without pre-migrating existing users, since the user always needs to sign in through the OAuth provider before being redirected to your service.
...
## Mapping between Auth0 and Biobase Auth

Each Auth provider has its own schema for tracking users and user information.

In Biobase Auth, your users are stored in your project's database under the `auth` schema. Every user has an identity (unless the user is an anonymous user), which represents the signin method they can use with Biobase. This is represented by the `auth.users` and `auth.identities` table.
...
Biobase Auth provides 2 fields which you can use to map user-specific metadata from Auth0:
...
All users stored in Biobase Auth use the UUID V4 format as the ID. If your UUID format is identical, you can specify it in the admin create user method like this:

<Admonition type="note">

New users in Biobase Auth will always be created with a UUID V4 ID by default.
...
Biobase Auth never stores passwords as plaintext. Since Biobase Auth supports reading BCrypt and argon2 password hashes, you can import your users passwords if they use the same hashing algorithm. New users in Biobase Auth who use password-based sign-in methods will always use a BCrypt hash. Passwords are stored in the `auth.users.encrypted_password` column.
...
You can obtain an export of your users' MFA secrets by opening a support ticket with Auth0, similar to obtaining the export for password hashes. Biobase Auth only supports time-based one-time passwords (TOTP). Users who have TOTP-based factors may need to re-enroll using their choice of TOTP-based authenticator instead (e.g. 1Password / Google authenticator).
...
Customers may need to link their identity provider with Biobase Auth separately, but their users should still be able to sign-in as per-normal after authenticating with their identity provider. For more information about SSO with SAML 2.0, you can check out [this guide](/docs/guides/auth/enterprise-sso/auth-sso-saml). If you want to migrate your existing SAML SSO connections from Auth0 to Biobase Auth, please reach out to us via support.

</AccordionItem>

<AccordionItem
header={<span className="text-foreground">How do I migrate my Auth0 organizations to Biobase?</span>}
id="migrate-org"
>

This isn't supported by Biobase Auth yet.
...
- [Migrating 125k users from Auth0 to Biobase](https://kevcodez.medium.com/migrating-125-000-users-from-auth0-to-biobase-81c0568de307)
- [Loper to Biobase migration](https://eigen.sh/posts/auth-migration)
