...
For both new and existing users, sign in the user using the [`signInWithOAuth`](https://biobase.studio/docs/reference/javascript/auth-signinwithoauth) method. This works without pre-migrating existing users, since the user always needs to sign in through the OAuth provider before being redirected to your service.
