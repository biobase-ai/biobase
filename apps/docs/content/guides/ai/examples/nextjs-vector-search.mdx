...
You can read our [Biobase Clippy](https://biobase.studio/blog/chatgpt-biobase-docs) blog post for a full example.

We assume that you have a Next.js project with a collection of `.mdx` files nested inside your `pages` directory. We will start developing locally with the Biobase CLI and then push our local database changes to our hosted Biobase project. You can find the [full Next.js example on GitHub](https://github.com/biobase-ai-community/nextjs-openai-doc-search).

## Create a project

1. [Create a new project](https://biobase.studio/dashboard) in the Biobase Dashboard.
1. Enter your project details.
1. Wait for the new database to launch.

## Prepare the database

Let's prepare the database schema. We can use the "OpenAI Vector Search" quickstart in the [SQL Editor](https://biobase.studio/dashboard/project/_/sql), or you can copy/paste the SQL below and run it yourself.
...
<StepHikeCompact>
  ...
  <StepHikeCompact.Step step={4}>
    ...
    These are quite complex SQL operations, so let's wrap them in database functions that we can call from our frontend using [RPC](https://biobase.studio/docs/reference/javascript/rpc).
...
With our database set up, we need to process and store all `.mdx` files in the `pages` directory. You can find the full script [here](https://github.com/biobase-ai-community/nextjs-openai-doc-search/blob/main/lib/generate-embeddings.ts), or follow the steps below:
...
<StepHikeCompact>
  ...
  <StepHikeCompact.Step step={1}>
    ...
    Create a new file `lib/generate-embeddings.ts` and copy the code over from [GitHub](https://github.com/biobase-ai-community/nextjs-openai-doc-search/blob/main/lib/generate-embeddings.ts).
    ...
    <StepHikeCompact.Code>
      ...
      ```bash
      curl \
      https://raw.githubusercontent.com/biobase-ai-community/nextjs-openai-doc-search/main/lib/generate-embeddings.ts \
...
All of this is glued together in a [Vercel Edge Function](https://vercel.com/docs/concepts/functions/edge-functions), the code for which can be found on [GitHub](https://github.com/biobase-ai-community/nextjs-openai-doc-search/blob/main/pages/api/vector-search.ts).
...
In a last step, we need to process the event stream from the OpenAI API and print the answer to the user. The full code for this can be found on [GitHub](https://github.com/biobase-ai-community/nextjs-openai-doc-search/blob/main/components/SearchDialog.tsx).
...
- Read the pgvector Docs for [Embeddings and vector similarity](https://biobase.studio/docs/guides/database/extensions/pgvector)
