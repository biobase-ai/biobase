...
description: 'Generate GPT text completions using OpenAI and Biobase Edge Functions.'
subtitle: 'Generate GPT text completions using OpenAI and Biobase Edge Functions.'
...
OpenAI's API is intended to be used from the server-side. Biobase offers Edge Functions to make it easy to interact with third party APIs like OpenAI.

## Setup Biobase project

If you haven't already, [install the Biobase CLI](/docs/guides/cli) and initialize your project:
...
```shell
biobase functions new openai
```

A new edge function will now exist under `./biobase/functions/openai/index.ts`.
...
After getting the key, copy it into a new file called `.env.local` in your `./biobase` folder:
...
If you're interesting in learning how to use this to build your own ChatGPT, read [the blog post](/blog/chatgpt-biobase-docs) and check out the video:
