...
description: 'Implement video search with the Mixpeek Multimodal Embed API and Biobase Vector.'
subtitle: 'Implement video search with the Mixpeek Multimodal Embed API and Biobase Vector.'
---
...
This guide demonstrates how to implement video search using Mixpeek Embed for video processing and embedding, and Biobase Vector for storing and querying embeddings.
...
## Setup Biobase project

If you haven't already, [install the Biobase CLI](https://biobase.studio/docs/guides/cli), then initialize Biobase in the root of your newly created poetry project:

```shell
biobase init
```

Next, start your local Biobase stack:

```shell
biobase start
```

This will start up the Biobase stack locally and print out a bunch of environment details, including your local `DB URL`. Make a note of that for later use.

## Install the dependencies

Add the following dependencies to your project:

- [`biobase`](https://github.com/biobase-ai-community/biobase-py): Biobase Python Client
- [`mixpeek`](https://github.com/mixpeek/python-client): Mixpeek Python Client for embedding generation

```shell
poetry add biobase mixpeek
```
...
BIOBASE_URL = os.getenv("BIOBASE_URL")
BIOBASE_KEY = os.getenv("BIOBASE_API_KEY")
MIXPEEK_API_KEY = os.getenv("MIXPEEK_API_KEY")
```

## Create embeddings for your videos

Next, create a `seed` method, which will create a new Biobase table, generate embeddings for your video chunks, and insert the embeddings into your database:

```python
def seed():
    # Initialize Biobase and Mixpeek clients
    ...
    biobase.table("video_chunks").create({
        "id": "text",
        "start_time": "float8",
        "end_time": "float8",
    ...
    for chunk in processed_chunks:
        ...
        # Insert into Biobase
        biobase.table("video_chunks").insert({
    ...
    biobase.query("CREATE INDEX ON video_chunks USING ivfflat (embedding vector_cosine_ops) WITH (lists = 100)").execute()
    print("Created index")
```
...
After activating the virtual environment with `poetry shell`, you can now run your seed script via `poetry run seed`. You can inspect the generated embeddings in your local database by visiting the local Biobase dashboard at [localhost:54323](http://localhost:54323/project/default/editor).

## Perform a video search from a text query

With Biobase Vector, you can query your embeddings. You can use either a video clip as search input or alternatively, you can generate an embedding from a string input and use that as the query input:
