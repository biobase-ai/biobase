...
description: 'Learn how to integrate hugging face models with Biobase'
...
You will need the `biobase` CLI [installed](/docs/guides/cli) for the following commands to work.

</Admonition>

To create a new Edge Function, navigate to your local project and initialize Biobase if you haven't already:
...
```shell
biobase functions new text-to-image
```
...
```shell
biobase functions serve --env-file .env.local --no-verify-jwt
```
