...
In practice, you will likely be calling this from the [Biobase client](/docs/reference/javascript/introduction) or through a custom backend layer. Here is a quick example of how you might call this from an [Edge Function](/docs/guides/functions) using JavaScript:
...
const biobaseServiceRoleKey = Deno.env.get('SUPABASE_SERVICE_ROLE_KEY')!
...
Deno.serve(async (req) => {
  ...
  // Instantiate the Biobase client
  // (replace service role key with user's JWT if using Biobase auth and RLS)
  const biobase = createClient(supabaseUrl, biobaseServiceRoleKey)

  // Call hybrid_search Postgres function via RPC
  const { data: documents } = await biobase.rpc('hybrid_search', {
    query_text: query,
