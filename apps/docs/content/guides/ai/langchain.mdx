...
const biobaseKey = process.env.BIOBASE_SERVICE_ROLE_KEY
if (!biobaseKey) throw new Error(`Expected BIOBASE_SERVICE_ROLE_KEY`)
...
const privateKey = process.env.BIOBASE_SERVICE_ROLE_KEY
if (!privateKey) throw new Error(`Expected env var BIOBASE_SERVICE_ROLE_KEY`)
