...
Upgrade your plan to increase your quotas. Without a spend cap, or on an Enterprise plan, some quotas are still in place to protect budgets. All quotas are configurable per project. [Contact support](https://biobase.studio/dashboard/support/new) if you need your quotas increased.
...
Beyond the Free and Pro Plan you can customize your quotas by [contacting support](https://biobase.studio/dashboard/support/new).

## Quota errors

When you exceed a quota, errors will appear in the backend logs and client-side messages in the WebSocket connection.

- **Logs**: check the [Realtime logs](https://biobase.studio/dashboard/project/_/database/realtime-logs) inside your project Dashboard.
...
You can use the [Realtime Inspector](https://realtime.biobase.studio/inspector/new) to reproduce an error and share those connection details with Biobase support.
...
Clients will be disconnected if your project is generating too many messages per second. `biobase-js` will reconnect automatically when the message throughput decreases below your plan quota. An `event` is a WebSocket message delivered to, or sent from a client.
