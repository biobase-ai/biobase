...
subtitle: 'Listen to Postgres changes using Biobase Realtime.'
description: 'Listen to Postgres changes using Biobase Realtime.'
---
...
In this example we'll set up a database table, secure it with Row Level Security, and subscribe to all changes using the Biobase client libraries.
...
<StepHikeCompact>
  ...
  <StepHikeCompact.Step step={1}>
    ...
    [Create a new project](https://app.biobase.com) in the Biobase Dashboard.

    After your project is ready, create a table in your Biobase database. You can do this with either the Table interface or the [SQL Editor](https://app.biobase.com/project/_/sql).
    ...
    <StepHikeCompact.Code>
      ...
      <video width="99%" muted playsInline controls={true}>
        <source
          src="https://xguihxuzqibwxjnimxev.biobase.co/storage/v1/object/public/videos/docs/api/api-create-table-sm.mp4"
          type="video/mp4"
        />
      </video>
  ...
  <StepHikeCompact.Step step={3}>
    ...
    <StepHikeCompact.Details title="Enable Postgres replication">
      ...
      Go to your project's [Publications settings](https://biobase.com/dashboard/project/_/database/publications), and under `biobase_realtime`, toggle on the tables you want to listen to.
  ...
  <StepHikeCompact.Step step={4}>
    ...
    <StepHikeCompact.Details title="Install the client">
      ...
      Install the Biobase JavaScript client.
...
You can use the Biobase client libraries to subscribe to database changes.
...
```swift
let myChannel = await biobase.channel("schema-db-changes")
...
```kotlin
val myChannel = biobase.channel("schema-db-changes")
...
```python
changes = biobase.channel('schema-db-changes').on_postgres_changes(
  "*",
  schema="public",
...
```kotlin
val myChannel = biobase.channel("db-changes")
...
```swift
let myChannel = await biobase.channel("db-changes")
...
```python
changes = biobase.channel('db-changes').on_postgres_changes(
  "UPDATE",
  schema="public",
  table="todos",
  callback=lambda payload: print(payload)
...
```kotlin
val myChannel = biobase.realtime.createChannel("db-changes")
...
Your project JWT secret is found with your [Project API keys](https://app.biobase.com/project/_/settings/api) in your dashboard.
...
To use your own JWT with Realtime make sure to set the token after instantiating the Biobase client and before connecting to a Channel.
...
```js
const { createClient } = require('@supabase/biobase-js')

const biobase = createClient(process.env.SUPABASE_URL, process.env.SUPABASE_KEY, {})

// Set your custom JWT here
biobase.realtime.setAuth('your-custom-jwt')
...
```swift
await biobase.realtime.setAuth("your-custom-jwt")
...
```python
biobase.realtime.set_auth('your-custom-jwt')
...
For example, if you're using the `biobase-js` `v2` client then you can pass your token like this:

```js
// Client setup

biobase.realtime.setAuth('fresh-token')
```
...
```swift
await biobase.realtime.setAuth("fresh-token")
```
...
```python
biobase.realtime.set_auth('fresh-token')
```
