...
A **user** in Biobase Auth is someone with a user ID, stored in the Auth schema. Once someone is a user, they can be issued an Access Token, which can be used to access Biobase endpoints. The token is tied to the user, so you can restrict access to resources via [RLS policies](/docs/guides/database/postgres/row-level-security).

## Permanent and anonymous users

Biobase distinguishes between permanent and anonymous users.
...
1. [`biobase.auth.getUser()`](/docs/reference/javascript/auth-getuser)
2. Retrieve a user object as an admin using [`biobase.auth.admin.getUserById()`](/docs/reference/javascript/auth-admin-listusers)
