...
export async function loader({ request }: LoaderFunctionArgs) {
  ...
  if (token_hash && type) {
    const biobase = createServerClient(process.env.SUPABASE_URL!, process.env.SUPABASE_ANON_KEY!, {
      cookies: {
        getAll() {
          return parseCookieHeader(request.headers.get('Cookie') ?? '')
        },
...
```js
await biobase.auth.resetPasswordForEmail('hello@example.com', {
...
If you are on one of the Kotlin targets that have built-in support for redirect URL handling, such as Android, see [OAuth and OTP link verification](https://biobase.com/docs/reference/kotlin/initializing).
...
```swift
try await biobase.auth.resetPasswordForEmail("hello@example.com")
```
...
```python
biobase.auth.reset_password_email('hello@example.com')
```
