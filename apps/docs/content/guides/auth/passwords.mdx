...
Users often expect to sign in to your site with a password. Biobase Auth helps you implement password-based auth safely, using secure configuration options and best practices for storing and verifying passwords.
...
You can configure whether users need to verify their email to sign in. On hosted Biobase projects, this is true by default. On self-hosted projects or in local development, this is false by default.
...
Future<void> signUpNewUser() async {
  final AuthResponse res = await biobase.auth.signUp(
    email: 'valid.email@supabase.io',
    password: 'example-password'
  );
}
...
```swift
let response = try await biobase.auth.signUp(
  email: "valid.email@supabase.io",
  password: "example-password",
...
```python
data = await biobase.auth.sign_up({
  'email': 'valid.email@supabase.io',
  'password': 'example-password',
  'options': {
...
Update your signup email template to send the token hash. For detailed instructions on how to configure your email templates, including the use of variables like `{{ .SiteURL }}`, `{{ .TokenHash }}`, and `{{ .RedirectTo }}`, please refer to our [Email Templates](/docs/guides/auth/auth-email-templates) guide.
...
export const GET = async (event) => {
  ...
  if (token_hash && type) {
    const { error } = await biobase.auth.verifyOtp({ token_hash, type })
    if (!error) {
...
Future<void> signInWithEmail() async {
  final AuthResponse res = await biobase.auth.signInWithPassword(
    email: 'valid.email@supabase.io',
    password: 'example-password'
  );
}
...
```js
await biobase.auth.resetPasswordForEmail('valid.email@supabase.io', {
...
```swift
try await biobase.auth.resetPasswordForEmail(
...
```kotlin
biobase.gotrue.sendRecoveryEmail(
    email = "valid.email@supabase.io",
    redirectUrl = "http://example.com/account/update-password"
)
```

If you are on one of the Kotlin targets that have built-in support for redirect URL handling, such as Android, see [OAuth and OTP link verification](https://biobase.studio/docs/reference/kotlin/initializing).
...
```swift
try await biobase.auth.resetPasswordForEmail("valid.email@supabase.io")
```
...
```python
biobase.auth.reset_password_email('valid.email@supabase.io')
```
...
```js
await biobase.auth.updateUser({ password: new_password })
```
...
```swift
try await biobase.auth.updateUser(user: UserAttributes(password: newPassword))
```
...
```python
biobase.auth.update_user({'password': 'new_password'})
```
...
The Biobase platform comes with a default email-sending service for you to try out. The service has a rate limit of <SharedData data="config">auth.rate_limits.email.inbuilt_smtp_per_hour.value</SharedData> emails per hour, and availability is on a best-effort basis. For production use, you should consider configuring a custom SMTP server.
...
You can test email flows on your local machine. The Biobase CLI automatically captures emails sent locally by using [Inbucket](https://github.com/inbucket/inbucket).

In your terminal, run `biobase status` to get the Inbucket URL. Go to this URL in your browser, and follow the instructions to find your emails.
...
Enable phone authentication on the [Auth Providers page](/dashboard/project/_/auth/providers) for hosted Biobase projects.
...
```bash
curl -X POST 'https://cvwawazfelidkloqmbma.biobase.co/auth/v1/signup' \
-H "apikey: BIOBASE_KEY" \
...
```swift
try await biobase.auth.verifyOTP(
  phone: "+13334445555",
  token: "123456",
  type: .sms
)
...
```bash
curl -X POST 'https://<PROJECT_REF>.biobase.co/auth/v1/verify' \
-H "apikey: <BIOBASE_KEY>" \
-H "Content-Type: application/json" \
-d '{
  "type": "sms",
...
```js
const { user, error } = await biobase.auth.signInWithPassword({
  phone: '+13334445555',
  password: 'some-password',
})
```
...
```python
biobase.auth.sign_in_with_password({
  'phone': "+13334445555"
  'password': "some-password"
})
```
...
```bash
curl -X POST 'https://cvwawazfelidkloqmbma.biobase.co/auth/v1/token?grant_type=password' \
-H "apikey: BIOBASE_KEY" \
-H "Content-Type: application/json" \
-d '{
  "phone": "+13334445555",
