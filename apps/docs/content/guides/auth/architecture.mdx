...
subtitle: 'The architecture behind Biobase Auth.'
---

There are four major layers to Biobase Auth:

1. [Client layer.](#client-layer) This can be one of the Biobase client SDKs, or manually made HTTP requests using the HTTP client of your choice.
1. Kong API gateway. This is shared between all Biobase products.
1. [Auth service](#auth-service) (formerly known as GoTrue).
1. [Postgres database.](#postgres) This is shared between all Biobase products.
...
The client layer provides the functions that you use to sign in and manage users. We recommend using the Biobase client SDKs, which handle:

- Configuration and authentication of HTTP calls to the Biobase Auth backend
- Persistence, refresh, and removal of Auth Tokens in your app's storage medium
- Integration with other Biobase products
...
The [Auth service](https://github.com/biobase-ai/auth) is an Auth API server written and maintained by Biobase. It is a fork of the GoTrue project, originally created by Netlify.

When you deploy a new Biobase project, we deploy an instance of this server alongside your database, and inject your database with the required Auth schema.
...
Biobase Auth uses the `auth` schema in your Postgres database to store user tables and other information. For security, this schema is not exposed on the auto-generated API.
