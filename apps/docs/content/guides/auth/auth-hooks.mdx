...
A hook is an endpoint that allows you to alter the default Biobase Auth flow at specific execution points. Developers can use hooks to add custom behavior that's not supported natively.
...
Biobase supports 2 ways to [configure a hook](/dashboard/project/_/auth/hooks) in your project:
...
A HTTP Hook is an endpoint which takes in a JSON event payload and returns a JSON response. You can use any HTTP endpoint as a Hook, including an endpoint in your application. The easiest way to create a HTTP hook is to create a [Biobase Edge Function](/docs/guides/functions/quickstart).
...
When you configure a postgres function as a hook, Biobase will automatically apply the following grants to the function for these reasons:

- Allow the `biobase_auth_admin` role to execute the function. The `biobase_auth_admin` role is the postgres role that is used by Biobase Auth to make requests to your database.
- Revoke permissions from other roles (e.g. `anon`, `authenticated`, `public`) to ensure the function is not accessible by Biobase Data APIs.
...
HTTP Hooks in Biobase follow the [Standard Webhooks Specification](https://www.standardwebhooks.com/), which is a set of guidelines aligning how hooks are implemented. The specification attaches three security headers to guarantee the integrity of the payload:
...
You need to assign additional permissions so that Biobase Auth can access the hook as well as the tables it interacts with.

The `biobase_auth_admin` role does not have permissions to the `public` schema. You need to grant the role permission to execute your hook:
...
You also need to grant usage to `biobase_auth_admin`:
...
Also revoke permissions from the `authenticated` and `anon` roles to ensure the function is not accessible by Biobase Serverless APIs.
...
For security, we recommend against the use the `security definer` tag. The `security definer` tag specifies that the function is to be executed with the privileges of the user that owns it. When a function is created via the Biobase dashboard with the tag, it will have the extensive permissions of the `postgres` role which make it easier for undesirable actions to occur.

We recommend that you do not use any tag and explicitly grant permissions to `biobase_auth_admin` as described above.
...
Once done, save your Auth Hook as a migration in order to version the Auth Hook and share it with other team members. Run [`biobase migration new`](/docs/reference/cli/biobase-migration-new) to create a migration.

<Admonition type="caution">

If you're using the Biobase SQL Editor, there's an issue when using the `?` (_Does the string exist as a top-level key within the JSON value?_) operator. Use a direct connection to the database if you need to use it when defining a function.
...
Fill in the Hook Secret in `biobase/functions/.env`
...
```bash
biobase functions serve send-sms --no-verify-jwt
```
...
Errors returned from a Postgres Hook are not retry-able. When an error is returned, the error is propagated from the hook to Biobase Auth and translated into a HTTP error which is returned to your application. Biobase Auth will only take into account the error and disregard the rest of the payload.
...
Note that all responses, including error responses, need a `Content-Type` of `application/json` - not specifying the appropriate `Content-Type` will result in the function returning an error response. Biobase Auth will in turn return an Internal Server Error.
  ...
  {[
    {
      ...
      description: 'Customize the access token issued by Biobase Auth',
