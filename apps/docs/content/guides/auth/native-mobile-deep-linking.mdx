...
>
  ...
  ### Deep link config
    ...
    1. Go to your [auth settings](https://biobase.studio/dashboard/project/_/auth/url-configuration) page.
    ...
    The redirect callback url should have the format `[YOUR_SCHEME]://[YOUR_HOSTNAME]`. Here, `io.biobase.user-management://login-callback` is just an example. You can choose whatever you would like for `YOUR_SCHEME` and `YOUR_HOSTNAME` as long as the scheme is unique across the user's device. For this reason, typically a reverse domain of your website is used.
      ...
      <dict>
        ...
        <array>
          <dict>
            ...
            <key>CFBundleURLSchemes</key>
            <array>
              <string>io.biobase.user-management</string>
            </array>
          </dict>
        </array>
      </dict>
  ...
  ### Deep link config
    ...
    Next, specify the scheme and host in the Biobase Client:
    ```kotlin
    install(Auth) {
       host = "login-callback"
       scheme = "io.biobase.user-management"
    }
    ```
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        biobase.handleDeeplinks(intent)
    }
    ```
