...
Biobase Auth provides several passwordless login methods. Passwordless logins allow users to sign in without a password, by clicking a confirmation link or entering a verification code.
...
Biobase Auth offers two passwordless login methods that use the user's email address:
...
Future<void> signInWithEmail() async {
  final AuthResponse res = await biobase.auth.signinwithotp(email: 'valid.email@supabase.io');
}
```
...
```python
response = biobase.auth.sign_in_with_otp({
  'email': 'valid.email@supabase.io',
  'options': {
    # set this to false if you do not want the user to be automatically signed up
...
```kotlin
biobase.auth.verifyEmailOtp(type = OtpType.Email.EMAIL, email = "email", token = "151345")
```
...
```python
response = biobase.auth.verify_otp({
  'email': email,
  'token': '123456',
  'type': 'email',
})
