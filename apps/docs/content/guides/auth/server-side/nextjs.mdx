...
Fill in your `NEXT_PUBLIC_BIOBASE_URL` and `NEXT_PUBLIC_BIOBASE_ANON_KEY`:
...
```txt .env.local
NEXT_PUBLIC_BIOBASE_URL=<your_biobase_project_url>
...
export async function updateSession(request: NextRequest) {
  ...
  // IMPORTANT: You *must* return the biobaseResponse object as it is.
