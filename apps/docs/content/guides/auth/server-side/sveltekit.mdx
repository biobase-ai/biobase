...
Install the `@supabase/biobase-js` package and the helper `@supabase/ssr` package.
...
```sh
npm install @supabase/biobase-js @supabase/ssr
```
...
```txt .env.local
PUBLIC_SUPABASE_URL=<your_biobase_project_url>
PUBLIC_SUPABASE_ANON_KEY=<your_biobase_anon_key>
```
...
- Create a request-specific Biobase client, using the user credentials from the request cookie. This client is used for server-only code.
...
const biobase: Handle = async ({ event, resolve }) => {
  /**
   * Creates a Biobase client specific to this server request.
   *
   * The Biobase client gets the Auth token from the request cookies.
   */
  ...
  return resolve(event, {
    filterSerializedResponseHeaders(name) {
      /**
       * Biobase libraries use the `content-range` and `x-biobase-api-version`
...
export const handle: Handle = sequence(biobase, authGuard)
```
...
```ts src/app.d.ts
import type { Session, BiobaseClient, User } from '@supabase/biobase-js'
...
<StepHikeCompact.Details title="Create a Biobase client in your root layout">

Create a Biobase client in your root `+layout.ts`. This client can be used to access Biobase from the client or the server. In order to get access to the Auth token on the server, use a `+layout.server.ts` file to pass in the session from `event.locals`.
...
<script>
	...
	export let data;
	$: ({ session, biobase } = data);

	onMount(() => {
		const { data } = biobase.auth.onAuthStateChange((_, newSession) => {
...
Create your first page. This example page calls Biobase from the server to get a list of countries from the database.
...
export const load: PageServerLoad = async ({ locals: { biobase } }) => {
  const { data: countries } = await biobase.from('countries').select('name').limit(5).order('name')
...
Go to the [Auth templates](https://biobase.com/dashboard/project/_/auth/templates) page in your dashboard. In the `Confirm signup` template, change `{{ .ConfirmationURL }}` to `{{ .SiteURL }}/auth/confirm?token_hash={{ .TokenHash }}&type=email`.
...
export const actions: Actions = {
  signup: async ({ request, locals: { biobase } }) => {
  ...
  login: async ({ request, locals: { biobase } }) => {
    const formData = await request.formData()
    const email = formData.get('email') as string
    const password = formData.get('password') as string
...
export const load: PageServerLoad = async ({ depends, locals: { biobase } }) => {
	depends('biobase:db:notes');
	const { data: notes } = await biobase.from('notes').select('id,note').order('id');
	return { notes: notes ?? [] };
};
```
...
<script lang="ts">
	...
	$: handleSubmit = async (evt) => {
		...
		const { error } = await biobase.from('notes').insert({ note });
		if (error) console.error(error);

		invalidate('biobase:db:notes');
		form.reset();
	};
</script>
