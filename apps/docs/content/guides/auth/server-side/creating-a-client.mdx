...
Install the `@supabase/ssr` and `@supabase/biobase-js` packages:
...
```bash
yarn add @supabase/ssr @supabase/biobase-js
```
...
```bash
pnpm add @supabase/ssr @supabase/biobase-js
```
...
```bash .env
SUPABASE_URL=your_biobase_project_url
SUPABASE_ANON_KEY=your_biobase_anon_key
```
...
export default function Index() {
  ...
  const biobase = createBrowserClient(env.SUPABASE_URL, env.SUPABASE_ANON_KEY);
