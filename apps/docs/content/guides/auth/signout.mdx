...
Biobase Auth allows you to specify three different scopes for when a user invokes the [sign out API](/docs/reference/javascript/auth-signout) in your application:
...
await biobase.auth.signOut({ scope: 'local' })
```
...
await biobase.auth.signOut(scope: SignOutScope.global);
```
...
// sign out from all sessions
biobase.auth.signOut(SignOutScope.GLOBAL)
```
