...
subtitle: 'Customize the access token issued by Biobase Auth'
---
...
Claims returned must conform to our specification. Biobase Auth will check for these claims after the hook is run and return an error if they are not present.
...
begin
    ...
    if email_claim ilike '%@biobase.io' or authentication_method = 'sso/saml' or email_claim = any(allowed_emails) then
        return event;
    end if;
