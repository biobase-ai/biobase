...
You can add additional checks to the [Biobase MFA implementation](/docs/guides/auth/auth-mfa) with hooks. For example, you can:
...
Biobase Auth will send a payload containing these fields to your hook:
...
| Field      | Type     | Description                                                                                                                                                                                                           |
| ---------- | -------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `decision` | `string` | The decision on whether to allow authentication to move forward. Use `reject` to deny the verification attempt and log the user out of all active sessions. Use `continue` to use the default Biobase Auth behavior. |
...
as $$
  ...
  begin
    ...
    -- finally let Biobase Auth do the default behavior for a failed attempt
    return jsonb_build_object('decision', 'continue');
  end;
$$;
