...
If you want to send emails through the Biobase Resend integration, which uses Resend's SMTP server, check out [this integration](/partners/integrations/resend) instead.
...
Set the secrets in your Biobase project:

```bash
biobase secrets set --env-file .env
```
...
function generateConfirmationURL(email_data) {
  ...
  return `https://<ref>.biobase.co/auth/v1/verify?token=${email_data.token}&type=${email_data.email_action_type}&redirect_to=${email_data.redirect_to}`
}
