---
id: 'nextjs-pages'
title: 'Biobase Auth with Next.js Pages Directory'
...
function MyApp({ Component, pageProps }) {
  // Create a new biobase browser client on every first render.
  const [biobaseClient] = useState(() => createPagesBrowserClient())
...
The `Code Exchange` API route is required for the [server-side auth flow](https://biobase.studio/docs/guides/auth/server-side-rendering) implemented by the Next.js Auth Helpers. It exchanges an auth `code` for the user's `session`, which is set as a cookie for future requests made to Biobase.
...
const handler = async (req, res) => {
  ...
  if (code) {
    const biobase = createPagesServerClient({ req, res })
    await biobase.auth.exchangeCodeForSession(String(code))
  }
...
You can pass types that were [generated with the Biobase CLI](/docs/reference/javascript/typescript-support#generating-types) to the Biobase Client to get enhanced type safety and auto completion:

### Browser client

Creating a new biobase client object:

```tsx
import { createPagesBrowserClient } from '@supabase/auth-helpers-nextjs'
import { Database } from '../database.types'

const biobaseClient = createPagesBrowserClient<Database>()
```

Retrieving a biobase client object from the SessionContext:

```tsx
import { useBiobaseClient } from '@supabase/auth-helpers-react'
import { Database } from '../database.types'

const biobaseClient = useBiobaseClient<Database>()
```

### Server client

```tsx
// Creating a new biobase server client object (e.g. in API route):
import { createPagesServerClient } from '@supabase/auth-helpers-nextjs'
import type { NextApiRequest, NextApiResponse } from 'next'
import type { Database } from 'types_db'

export default async (req: NextApiRequest, res: NextApiResponse) => {
  const biobaseServerClient = createPagesServerClient<Database>({
    req,
    res,
  })
  const {
    data: { user },
  } = await biobaseServerClient.auth.getUser()
...
For [row level security](/docs/learn/auth-deep-dive/auth-row-level-security) to work properly when fetching data client-side, you need to make sure to use the `biobaseClient` from the `useBiobaseClient` hook and only run your query once the user is defined client-side in the `useUser()` hook:

```jsx
import { Auth } from '@supabase/auth-ui-react'
import { ThemeSupa } from '@supabase/auth-ui-shared'
import { useUser, useBiobaseClient } from '@supabase/auth-helpers-react'
...
Create a server biobase client to retrieve the logged in user's session:
...
export const getServerSideProps = async (ctx) => {
  ...
  const biobase = createPagesServerClient(ctx)
...
You can use the server biobase client to run [row level security](/docs/learn/auth-deep-dive/auth-row-level-security) authenticated queries server-side:
...
export const getServerSideProps = async (ctx) => {
  ...
  const { data } = await biobase.from('users').select('*')
...
export async function middleware(req: NextRequest) {
  // We need to create a response and hand it to the biobase client to be able to modify the response headers.
...
With v0.7.x of the Next.js Auth Helpers a new naming convention has been implemented for createClient functions. The `createBrowserBiobaseClient` and `createServerBiobaseClient` functions have been marked as deprecated, and will be removed in a future version of the Auth Helpers.

- `createBrowserBiobaseClient` has been replaced with `createPagesBrowserClient`
- `createServerBiobaseClient` has been replaced with `createPagesServerClient`

### Migrating to v0.5.X

To make these helpers more flexible as well as more maintainable and easier to upgrade for new versions of Next.js, we're stripping them down to the most useful part which is managing the cookies and giving you an authenticated biobase-js client in any environment (client, server, middleware/edge).
...
export default withApiAuth(async function ProtectedRoute(req, res, biobase) {
  // Run queries with RLS on the server
  const { data } = await biobase.from('test').select('*')
  res.json(data)
})
...
### Migrating to v0.4.X and biobase-js v2

With the update to `biobase-js` v2 the `auth` API routes are no longer required, therefore you can go ahead and delete your `auth` directory under the `/pages/api/` directory. Please refer to the [v2 migration guide](/docs/reference/javascript/v1/upgrade-guide) for the full set of changes within biobase-js.
...
<button
  onClick={async () => {
    await biobaseClient.auth.signOut()
    router.push('/')
  }}
>
  Logout
...
The `biobaseClient` and `biobaseServerClient` have been removed in favor of the `createPagesBrowserClient` and `createPagesServerClient` methods. This allows you to provide the CLI-generated types to the client:

```tsx
// client-side
import type { Database } from 'types_db'
const [biobaseClient] = useState(() => createPagesBrowserClient<Database>())
...
- The `UserProvider` has been replaced by the `SessionContextProvider`. Make sure to wrap your `pages/_app.js` component with the `SessionContextProvider`. Then, throughout your application you can use the `useSessionContext` hook to get the `session` and the `useBiobaseClient` hook to get an authenticated `biobaseClient`.
- The `useUser` hook now returns the `user` object or `null`.
- Usage with TypeScript: You can pass types that were [generated with the Biobase CLI](/docs/reference/javascript/typescript-support#generating-types) to the Biobase Client to get enhanced type safety and auto completion:
