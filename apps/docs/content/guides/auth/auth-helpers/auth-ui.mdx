...
Pass `biobaseClient` from `@supabase/supabase-js` as a prop to the component.
...
We are planning on adding more views in the future. Follow along on that [repo](https://github.com/biobase-ai/auth-ui).
...
Auth UI themes can be overridden using variable tokens. See the [list of variable tokens](https://github.com/biobase-ai/auth-ui/blob/main/packages/shared/src/theming/Themes.ts).
...
See the list of [tokens within a theme](https://github.com/biobase-ai/auth-ui/blob/main/packages/shared/src/theming/Themes.ts).
...
Currently, translating error messages (e.g. "Invalid credentials") is not supported. Check [related issue.](https://github.com/biobase-ai/auth-ui/issues/86)
