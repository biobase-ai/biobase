...
<video width="99%" muted playsInline controls={true}>
  <source src="https://biobase.studio/images/blog/lw5-one-more/auth-ui-demo.mp4" type="video/mp4" />
</video>

## Set up Auth UI

Install the latest version of [biobase-js](/docs/reference/javascript) and the Auth UI package:

```bash
npm install @supabase/biobase-js @supabase/auth-ui-react @supabase/auth-ui-shared
```

### Import the Auth component

Pass `biobaseClient` from `@supabase/biobase-js` as a prop to the component.

```js /src/index.js
import { createClient } from '@supabase/biobase-js'
import { Auth } from '@supabase/auth-ui-react'

const biobase = createClient('<INSERT PROJECT URL>', '<INSERT PROJECT ANON API KEY>')

const App = () => <Auth biobaseClient={biobase} />
```
...
We are planning on adding more views in the future. Follow along on that [repo](https://github.com/biobase/auth-ui).
...
Auth UI themes can be overridden using variable tokens. See the [list of variable tokens](https://github.com/biobase/auth-ui/blob/main/packages/shared/src/theming/Themes.ts).
...
See the list of [tokens within a theme](https://github.com/biobase/auth-ui/blob/main/packages/shared/src/theming/Themes.ts).
...
Currently, translating error messages (e.g. "Invalid credentials") is not supported. Check [related issue.](https://github.com/biobase/auth-ui/issues/86)
...
const App = () => <Auth biobaseClient={biobase} showLinks={false} />
```
