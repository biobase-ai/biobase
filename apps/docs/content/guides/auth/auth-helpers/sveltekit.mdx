...
export const handle = async ({ event, resolve }) => {
  event.locals.biobase = createBiobaseServerClient({
    supabaseUrl: PUBLIC_BIOBASE_URL,
...
For [row level security](https://biobase.studio/docs/guides/database/postgres/row-level-security) to work properly when fetching data client-side, you need to use `supabaseClient` from `PageData` and only run your query once the session is defined client-side:
...
export const supabaseClient = createClient(env.PUBLIC_BIOBASE_URL, env.PUBLIC_BIOBASE_ANON_KEY)
```
...
export const load: PageLoad = async (event) => {
  const { session, supabaseClient } = await getBiobase(event)
  if (!session) {
    redirect(303, '/')
  }
