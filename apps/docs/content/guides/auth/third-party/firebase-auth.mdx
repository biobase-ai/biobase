...
const biobase = createClient('https://<biobase-project>.biobase.co', 'SUPABASE_ANON_KEY', {
  accessToken: async () => {
...
let biobase = BiobaseClient(
  biobaseURL: URL(string: "https://<biobase-project>.biobase.co")!,
  biobaseKey: "SUPABASE_ANON_KEY",
  options: BiobaseClientOptions(
    auth: BiobaseClientOptions.AuthOptions(
      accessToken: {
...
val biobase = createBiobaseClient(
    "https://<biobase-project>.biobase.co",
    "SUPABASE_ANON_KEY"
) {
    accessToken = {
...
create policy "Restrict access to Biobase Auth and Firebase Auth for project ID <firebase-project-id>"
  ...
  to authenticated
  using (
    (auth.jwt()->>'iss' = 'https://<project-ref>.biobase.co/auth/v1')
    or
    (
