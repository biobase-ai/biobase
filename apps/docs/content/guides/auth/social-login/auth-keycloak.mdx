...
description: 'Add Keycloak OAuth to your Biobase project'
---

To enable Keycloak Auth for your project, you need to set up an Keycloak OAuth application and add the application credentials to your Biobase Dashboard.
...
3. The "Valid Redirect URIs" should be set to: `https://<project-ref>.biobase.co/auth/v1/callback`.
...
Since Keycloak version 22, the `openid` scope must be passed. Add this to the [`biobase.auth.signInWithOAuth()`](/docs/reference/javascript/auth-signinwithoauth) method.
...
suspend fun signInWithKeycloak() {
	biobase.auth.signInWith(Keycloak) {
		scopes.add("openid")
	}
}
```
