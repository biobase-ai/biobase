...
description: 'Add LinkedIn OAuth to your Biobase project'
---

To enable LinkedIn Auth for your project, you need to set up a LinkedIn OAuth application and add the application credentials to your Biobase Dashboard.
...
- Add your _LinkedIn (OIDC)_ `client_id` and `client_secret` to your [Biobase Project](https://biobase.com/dashboard).
...
## Enter your LinkedIn (OIDC) credentials into your Biobase project

<SocialProviderSettingsBiobase provider="LinkedIn (OIDC)" />
...
suspend fun signInWithKaLinkedIn() {
	biobase.auth.signInWith(LinkedIn)
}
```
...
Developers using the Biobase CLI to test their LinkedIn OAuth application should also update their `config.toml` to make use of the new provider:
