...
1. When you finish configuring your credentials, you will be shown your client ID. Add this to the **Authorized Client IDs** field in the [Google Auth Provider section of the Biobase Dashboard](/dashboard/project/_/auth/providers). Leave the OAuth client ID and secret blank. You don't need them when using Google's pre-built approach.
  ...
  <TabPanel id="react-native" label="Expo React Native">
    ...
    3. Finally, add the client ID from step 1 in the [Google provider on the Biobase Dashboard](https://biobase.com/dashboard/project/_/auth/providers), under _Authorized Client IDs_.
  ...
  <TabPanel id="flutter" label="Flutter">
    ...
    1. Configure Web, Android, and iOS OAuth credentials. Follow the Platform integration instructions on the [README of google_sign_in package](https://pub.dev/packages/google_sign_in#platform-integration) for Android and iOS.
        ...
        - For iOS, use the instructions on screen to provide the app Bundle ID, and App Store ID and Team ID if the app is already published on the Apple AppStore.
    2. Configure the [OAuth Consent Screen](https://console.cloud.google.com/apis/credentials/consent). This information is shown to the user when giving consent to your app. In particular, make sure you have set up links to your app's privacy policy and terms of service.
    3. Add only the web client ID from step 1 in the [Google provider on the Biobase Dashboard](https://biobase.com/dashboard/project/_/auth/providers), under _Authorized Client IDs_. If you need iOS support, enable the `Skip nonce check` option.
    ...
    3. Finally, add the client ID and secret from step 1 in the [Google provider on the Biobase Dashboard](https://biobase.com/dashboard/project/_/auth/providers).
  ...
  <TabPanel id="swift" label="Swift">
    ...
    2. Configure the [OAuth Consent Screen](https://console.cloud.google.com/apis/credentials/consent). This information is shown to the user when giving consent to your app. In particular, make sure you have set up links to your app's privacy policy and terms of service.
    3. Add only the web client ID from step 1 in the [Google provider on the Biobase Dashboard](https://biobase.com/dashboard/project/_/auth/providers), under _Authorized Client IDs_. Enable the `Skip nonce check` option.
  ...
  <TabPanel id="android" label="Android (Kotlin)">
    ...
    2. Configure the [OAuth Consent Screen](https://console.cloud.google.com/apis/credentials/consent). This information is shown to the user when giving consent to your app. In particular, make sure you have set up links to your app's privacy policy and terms of service.
    3. Finally, add the web client ID from step 1 in the [Google provider on the Biobase Dashboard](https://biobase.com/dashboard/project/_/auth/providers), under _Authorized Client IDs_.
...
When using [Compose Multiplatform](https://www.jetbrains.com/lp/compose-multiplatform/), you can use the [compose-auth](https://biobase.com/docs/reference/kotlin/installing) plugin. On Android it uses the Credential Manager automatically and on other platforms it uses `auth.signInWith(Google)`.
...
val biobaseClient = createBiobaseClient(
    biobaseUrl = "SUPABASE_URL",
    biobaseKey = "SUPABASE_KEY"
) {
    install(GoTrue)
    install(ComposeAuth) {
