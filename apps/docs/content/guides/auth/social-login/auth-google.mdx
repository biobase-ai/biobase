...
Biobase Auth supports Sign in with Google for the web, native Android applications, and Chrome extensions.
...
To support Sign In with Google, you need to configure the Google provider for your Biobase project.
...
1. Under **Authorized domains**, add your Biobase project's domain, which has the form `<PROJECT_ID>.biobase.co`.
...
1. Under **Authorized redirect URLs**, enter the callback URL from the [Biobase dashboard](/dashboard/project/_/auth/providers). Expand the Google Auth Provider section to display it.
...
1. When you finish configuring your credentials, you will be shown your client ID and secret. Add these to the [Google Auth Provider section of the Biobase Dashboard](/dashboard/project/_/auth/providers).
...
1. When you finish configuring your credentials, you will be shown your client ID. Add this to the **Authorized Client IDs** field in the [Google Auth Provider section of the Biobase Dashboard](/dashboard/project/_/auth/providers). Leave the OAuth client ID and secret blank. You don't need them when using Google's pre-built approach.
  ...
  <TabPanel id="react-native" label="Expo React Native">
    ...
    1. Configure OAuth credentials for your Google Cloud project in the [Credentials](https://console.cloud.google.com/apis/credentials) page of the console. When creating a new OAuth client ID, choose _Android_ or _iOS_ depending on the mobile operating system your app is built for.
        - For Android, use the instructions on screen to provide the SHA-1 certificate fingerprint used to sign your Android app.
          - You will have a different set of SHA-1 certificate fingerprint for testing locally and going to production. Make sure to add both to the Google Cloud Console. and add all of the Client IDs to Biobase dashboard.
    ...
    3. Finally, add the client ID from step 1 in the [Google provider on the Biobase Dashboard](https://biobase.studio/dashboard/project/_/auth/providers), under _Authorized Client IDs_.

    Note that you do not have to configure the OAuth flow in the Biobase Dashboard in order to use native sign in.
  ...
  <TabPanel id="flutter" label="Flutter">
    ...
    1. Configure Web, Android, and iOS OAuth credentials. Follow the Platform integration instructions on the [README of google_sign_in package](https://pub.dev/packages/google_sign_in#platform-integration) for Android and iOS.
        ...
        - For iOS, use the instructions on screen to provide the app Bundle ID, and App Store ID and Team ID if the app is already published on the Apple AppStore.
    2. Configure the [OAuth Consent Screen](https://console.cloud.google.com/apis/credentials/consent). This information is shown to the user when giving consent to your app. In particular, make sure you have set up links to your app's privacy policy and terms of service.
    3. Add only the web client ID from step 1 in the [Google provider on the Biobase Dashboard](https://biobase.studio/dashboard/project/_/auth/providers), under _Authorized Client IDs_. If you need iOS support, enable the `Skip nonce check` option.
    ...
    1. Obtain OAuth credentials for your Google Cloud project in the [Credentials](https://console.developers.google.com/apis/credentials) page of the console. If you already have a web client ID, no need to create a new one. When creating a new credential, choose _Web application_. In _Authorized redirect URIs_ enter `https://<project-id>.biobase.co/auth/v1/callback`. This URL will be seen by your users, and you can customize it by configuring [custom domains](/docs/guides/platform/custom-domains).
    2. Configure the [OAuth Consent Screen](https://console.cloud.google.com/apis/credentials/consent). This information is shown to the user when giving consent to your app. Within _Authorized domains_ make sure you add your Biobase project's domain `<project-id>.biobase.co`. Configure the non-sensitive scopes by making sure the following ones are selected: `.../auth/userinfo.email`, `.../auth/userinfo.profile`, `openid`. If you're selecting other sensitive scopes, your app may require additional verification. In those cases, it's best to use [custom domains](/docs/guides/platform/custom-domains).
    3. Finally, add the client ID and secret from step 1 in the [Google provider on the Biobase Dashboard](https://biobase.studio/dashboard/project/_/auth/providers).

  </TabPanel>

  <TabPanel id="swift" label="Swift">

    Google sign-in with Biobase is done through the [GoogleSignIn-iOS](https://github.com/google/GoogleSignIn-iOS) package.

    When the user provides consent, Google issues an identity token (commonly abbreviated as ID token) that is then sent to your project's Biobase Auth server. When valid, a new user session is started by issuing an access and refresh token from Biobase Auth.

    Follow the code sample below to implement native Google sign-in with Biobase in your iOS app.
    ...
    class GoogleSignInViewController: UIViewController {
      ...
      func googleSignIn() async throws {
        ...
        try await biobase.auth.signInWithIdToken(
    ...
    2. Configure the [OAuth Consent Screen](https://console.cloud.google.com/apis/credentials/consent). This information is shown to the user when giving consent to your app. In particular, make sure you have set up links to your app's privacy policy and terms of service.
    3. Add only the web client ID from step 1 in the [Google provider on the Biobase Dashboard](https://biobase.studio/dashboard/project/_/auth/providers), under _Authorized Client IDs_. Enable the `Skip nonce check` option.
  ...
  <TabPanel id="android" label="Android (Kotlin)">
    ...
    2. Configure the [OAuth Consent Screen](https://console.cloud.google.com/apis/credentials/consent). This information is shown to the user when giving consent to your app. In particular, make sure you have set up links to your app's privacy policy and terms of service.
    3. Finally, add the web client ID from step 1 in the [Google provider on the Biobase Dashboard](https://biobase.studio/dashboard/project/_/auth/providers), under _Authorized Client IDs_.
  ...
  <TabPanel id="chrome-extensions" label="Chrome Extensions">
    ...
    Note that you do not have to configure the OAuth flow in the Biobase Dashboard to sign in with Google inside Chrome extensions.
...
The tokens saved by your application are the Biobase Auth tokens. Your app might additionally need the Google OAuth 2.0 tokens to access Google services on the user's behalf.
...
1. Create a `handleSignInWithGoogle` function that takes the `CredentialResponse` and passes the included token to Biobase. The function needs to be available in the global scope for Google's code to find it.
...
1. _(Optional)_ Configure a nonce. The use of a nonce is recommended for extra security, but optional. The nonce should be generated randomly each time, and it must be provided in both the `data-nonce` attribute of the HTML code and the options of the callback function.
   ...
   Note that the nonce should be the same in both places, but because Biobase Auth expects the provider to hash it (SHA-256, hexadecimal representation), you need to provide a hashed version to Google and a non-hashed version to `signInWithIdToken`.
   ...
   // Use 'nonce' when invoking the biobase.auth.signInWithIdToken() method
   ```
  ...
  useEffect(() => {
    const initializeGoogleOneTap = () => {
      ...
      window.addEventListener('load', async () => {
        ...
        const { data, error } = await biobase.auth.getSession()
        if (error) {
        ...
        google.accounts.id.initialize({
          ...
          callback: async (response: CredentialResponse) => {
            try {
              // send id token returned in response.credential to biobase
              const { data, error } = await biobase.auth.signInWithIdToken({
                provider: 'google',
                token: response.credential,
                nonce,
...
When working with Expo, you can use the [`react-native-google-signin/google-signin` library](https://github.com/react-native-google-signin/google-signin#expo-installation) library to obtain an ID token that you can pass to biobase-js [`signInWithIdToken` method](/docs/reference/javascript/auth-signinwithidtoken).

Follow the [Expo installation docs](https://react-native-google-signin.github.io/docs/setting-up/expo) for installation and configuration instructions. See the [biobase-js reference](/docs/reference/javascript/initializing?example=react-native-options-async-storage) for instructions on initializing the biobase-js client in React Native.
...
Google sign-in with Biobase is done through the [google_sign_in](https://pub.dev/packages/google_sign_in) package for iOS and Android.

When the user provides consent, Google issues an identity token (commonly abbreviated as ID token) that is then sent to your project's Biobase Auth server. When valid, a new user session is started by issuing an access and refresh token from Biobase Auth.

Follow the code sample below to implement native Google sign-in with Biobase in your Flutter iOS and Android app.
...
Future<void> _nativeGoogleSignIn() async {
  ...
  await biobase.auth.signInWithIdToken(
...
Google sign-in with Biobase on Web, macOS, Windows, and Linux is done through the [signInWithOAuth](docs/reference/dart/auth-signinwithoauth) method.
...
This call takes the user to Google's consent screen. Once the flow ends, the user's profile information is exchanged and validated with Biobase Auth before it redirects back to your Flutter application with an access and refresh token representing the user's session.
...
Follow the code sample below to implement native Google sign-in with Biobase using Credential Manager in your Android app.
    ...
    val onClick: () -> Unit = {
        ...
        coroutineScope.launch {
            try {
                ...
                biobase.auth.signInWith(IDToken) {
                ...
                // Handle RestException thrown by Biobase
            } catch (e: Exception) {
...
When using [Compose Multiplatform](https://www.jetbrains.com/lp/compose-multiplatform/), you can use the [compose-auth](https://biobase.studio/docs/reference/kotlin/installing) plugin. On Android it uses the Credential Manager automatically and on other platforms it uses `auth.signInWith(Google)`.

**Initialize the Biobase Client**
...
```kotlin
val biobaseClient = createSupabaseClient(
    biobaseUrl = "BIOBASE_URL",
    biobaseKey = "BIOBASE_KEY"
) {
    install(GoTrue)
    install(ComposeAuth) {
...
```kotlin
val authState = biobaseClient.composeAuth.rememberLoginWithGoogle(
    onResult = {
...
Then you should call the [`chrome.identity.launchWebAuthFlow()`](https://developer.chrome.com/docs/extensions/reference/identity/#method-launchWebAuthFlow) function to trigger the sign in flow. On success, call the `biobase.auth.signInWithIdToken()` function to complete sign in with your Biobase project.
