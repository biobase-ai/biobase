...
```bash
curl -X POST 'https://cvwawazfelidkloqmbma.biobase.co/auth/v1/otp' \
...
The access token can be sent in the Authorization header as a Bearer token for any CRUD operations on biobase-js. See our guide on [Row Level Security](/docs/guides/auth#row-level-security) for more info on restricting access on a user basis.
