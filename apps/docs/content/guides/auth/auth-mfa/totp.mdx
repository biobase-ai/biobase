...
[TOTP MFA API](/docs/reference/javascript/auth-mfa-api) is free to use and is enabled on all Biobase projects by default.
...
1. Call `biobase.auth.mfa.enroll()`.
...
2. Calling the `biobase.auth.mfa.challenge()` API.
   This prepares Biobase Auth to accept a verification code from the user
   and returns a challenge ID. In the case of Phone MFA this step also sends the verification code to the user.
3. Calling the `biobase.auth.mfa.verify()` API.
...
- When the component appears on screen, the `biobase.auth.mfa.enroll()` API is
...
- Once the user has scanned the QR code with their authenticator app, they
  ...
  click on `Enable`.
- A challenge is created using the `biobase.auth.mfa.challenge()` API and the
  code from the user is submitted for verification using the
  `biobase.auth.mfa.verify()` challenge.
...
export function EnrollMFA({
  ...
  const onEnableClicked = () => {
    setError('')
    ;(async () => {
      const challenge = await biobase.auth.mfa.challenge({ factorId })
      if (challenge.error) {
      ...
      const verify = await biobase.auth.mfa.verify({
        factorId,
        challengeId,
        code: verifyCode,
      })
  ...
  useEffect(() => {
    ;(async () => {
      const { data, error } = await biobase.auth.mfa.enroll({
        factorType: 'totp',
      })
      if (error) {
        throw error
      ...
      // Biobase Auth returns an SVG QR code which you can convert into a data
...
This can be done by using the `biobase.auth.mfa.getAuthenticatorAssuranceLevel()` API. When the user signs in and is redirected back to your app, you should call this method to extract the user's current and next authenticator assurance level (AAL).
...
function AppWithMFA() {
  ...
  useEffect(() => {
    ;(async () => {
      try {
        const { data, error } = await biobase.auth.mfa.getAuthenticatorAssuranceLevel()
        if (error) {
          throw error
        }
...
- `biobase.auth.mfa.getAuthenticatorAssuranceLevel()` does return a promise.
...
function AuthMFA() {
  ...
  const onSubmitClicked = () => {
    setError('')
    ;(async () => {
      const factors = await biobase.auth.mfa.listFactors()
      if (factors.error) {
        throw factors.error
      }
...
- You can extract the available MFA factors for the user by calling
  `biobase.auth.mfa.listFactors()`. Don't worry this method is also very quick
