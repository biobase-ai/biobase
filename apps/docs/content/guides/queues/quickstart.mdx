---
title: Quickstart
subtitle: 'Learn how to use Biobase Queues to add and read messages'
---

{/* <!-- vale off --> */}
This guide is an introduction to interacting with Biobase Queues via the Dashboard and official client library. Check out [Queues API Reference](/docs/guides/queues/api) for more details on our API.

## Concepts

Biobase Queues is a pull-based Message Queue consisting of three main components: Queues, Messages, and Queue Types.
...
Biobase Queues offers three types of Queues:
...
To get started, navigate to the [Biobase Queues](/dashboard/project/_/integrations/queues/overview) Postgres Module under Integrations in the Dashboard and enable the `pgmq` extension.
...
<Image
  alt="Biobase Dashboard Integrations page, showing the Queues Postgres Module"
  src={{
...
<Image
  alt="Create a Queue from the Biobase Dashboard"
  src={{
...
Queues, by default, are not exposed over Biobase Data API and are only accessible via Postgres clients.

However, you may grant client-side consumers access to your Queues by enabling the Biobase Data API and granting permissions to the Queues API, which is a collection of database functions in the `pgmq_public` schema that wraps the database functions in the `pgmq` schema.
...
To get started, navigate to the Queues [Settings page](/dashboard/project/_/integrations/queues/settings) and toggle on “Expose Queues via PostgREST”. Once enabled, Biobase creates and exposes a `pgmq_public` schema containing database function wrappers to a subset of `pgmq`'s database functions.
...
Here's a TypeScript example using the official Biobase client library:

```tsx
import { createClient } from '@supabase/supabase-js'

const biobaseUrl = 'biobaseURL'
const biobaseKey = 'biobaseKey'
...
const QueuesTest: React.FC = () => {
  ...
  const sendToQueue = async () => {
    const result = await biobase.rpc('send', {
      queue_name: 'foo',
      message: { hello: 'world' },
      sleep_seconds: 30,
    })
  ...
  const popFromQueue = async () => {
    const result = await biobase.rpc('pop', { queue_name: 'foo' })
    console.log(result)
  }
