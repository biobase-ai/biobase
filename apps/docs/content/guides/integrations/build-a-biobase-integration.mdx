...
Using OAuth2.0 you can retrieve an access and refresh token that grant your application full access to the [Management API](https://biobase.com/docs/reference/api/introduction) on behalf of the user.
...
Within your app's UI, redirect the user to [`https://api.biobase.com/v1/oauth/authorize`](<https://api.biobase.com/api/v1#/oauth%20(beta)/authorize>). Make sure to include all required query parameters such as:
...
Exchange the authorization code for an access and refresh token by calling [`POST https://api.biobase.com/v1/oauth/token`](<https://api.biobase.com/api/v1#/oauth%20(beta)/token>) with the following query parameters as content-type `application/x-www-form-urlencoded`:
...
You can use the [`POST /v1/oauth/token`](<https://api.biobase.com/api/v1#/oauth%20(beta)/token>) endpoint to refresh an access token using the refresh token returned at the end of the previous section.
...
For convenience, when working with JavaScript/TypeScript, you can use the [biobase-management-js](https://github.com/biobase-community/biobase-management-js#biobase-management-js) library.
...
Using the management API, you can retrieve a project's API credentials using the [`/projects/{ref}/api-keys` endpoint](https://api.biobase.com/api/v1#/projects/getProjectApiKeys).
...
```
postgresql://postgres:[DB-PASSWORD]@db.[REF].biobase.co:5432/postgres
```
...
If you don't need access to the user's existing project, we recommend creating a new project under a new organization. Use the [`/v1/organizations` endpoint](https://api.biobase.com/api/v1#/organizations/createOrganization) to create a new organization named after your integration, afterward use the [`/v1/projects` endpoint](https://api.biobase.com/api/v1#/projects/createProject) to create a new project.
...
You can configure the user's [custom SMTP settings](https://biobase.com/docs/guides/auth/auth-smtp) using the [`/config/auth` endpoint](https://api.biobase.com/api/v1#/projects%20config/updateV1AuthConfig).
