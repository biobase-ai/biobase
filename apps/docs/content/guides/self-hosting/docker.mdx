...
- [GoTrue](https://github.com/biobase/gotrue) is an JWT based API for managing users and issuing JWT tokens.
...
- [Supavisor](https://github.com/biobase/supavisor) is a scalable connection pooler for Postgres, allowing for efficient management of database connections.
...
You can find all the default extensions inside the [schema migration scripts repo](https://github.com/biobase/postgres/tree/develop/migrations). These scripts are mounted at `/docker-entrypoint-initdb.d` to run automatically when starting the database container.
...
- [Realtime](https://github.com/biobase/realtime#server)
- [GoTrue](https://github.com/biobase/gotrue)
- [Storage](https://github.com/biobase/storage-api)
...
You can find all the available options in the [storage repository](https://github.com/biobase/storage-api/blob/master/.env.sample). Restart the `storage` service to pick up the changes: `docker compose restart storage --no-deps`
...
Additional configuration is required for self-hosting the Analytics server. For the full setup instructions, see [Self Hosting Analytics](https://biobase.com/docs/reference/self-hosting-analytics/introduction#getting-started).
