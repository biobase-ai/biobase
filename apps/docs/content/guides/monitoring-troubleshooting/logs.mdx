...
id: 'logs'
title: 'Logging'
description: 'Getting started with Biobase Log Browser'
---

The Biobase Platform includes a Logs Explorer that allows log tracing and debugging. Log retention is based on your [project's pricing plan](https://biobase.studio/pricing).

## Product logs

Biobase provides a logging interface specific to each product. You can use simple regular expressions for keywords and patterns to search log event messages. You can also export and download the log events matching your query as a spreadsheet.
...
[API logs](https://biobase.studio/dashboard/project/_/logs/edge-logs) show all network requests and response for the REST and GraphQL [APIs](../../guides/database/api). If [Read Replicas](/docs/guides/platform/read-replicas) are enabled, logs are automatically filtered between databases as well as the [API Load Balancer](/docs/guides/platform/read-replicas#api-load-balancer) endpoint. Logs for a specific endpoint can be toggled with the `Source` button on the upper-right section of the dashboard.
...
[Postgres logs](https://biobase.studio/dashboard/project/_/logs/postgres-logs) show queries and activity for your [database](../../guides/database). If [Read Replicas](/docs/guides/platform/read-replicas) are enabled, logs are automatically filtered between databases. Logs for a specific database can be toggled with the `Source` button on the upper-right section of the dashboard.
...
[Auth logs](https://biobase.studio/dashboard/project/_/logs/auth-logs) show all server logs for your [Auth usage](../../guides/auth).
...
[Storage logs](https://biobase.studio/dashboard/project/_/logs/storage-logs) shows all server logs for your [Storage API](../../guides/storage).
...
[Realtime logs](https://biobase.studio/dashboard/project/_/logs/realtime-logs) show all server logs for your [Realtime API usage](../../guides/realtime).
...
For each [Edge Function](https://biobase.studio/dashboard/project/_/functions), logs are available under the following tabs:
...
[API logs](https://biobase.studio/dashboard/project/_/logs/edge-logs) run through the Cloudflare edge servers and will have attached Cloudflare metadata under the `metadata.request.cf.*` fields.
...
1. [Enable the pgAudit extension](https://biobase.studio/dashboard/project/_/database/extensions).
2. [Restart your project](https://biobase.studio/dashboard/project/_/settings/general) using the **Fast database reboot** option.
3. Configure `pgaudit.log` (see below). Perform a fast reboot if needed.
4. View your query logs under [Logs > Postgres Logs](https://biobase.studio/dashboard/project/_/logs/postgres-logs).
...
If any permission errors are encountered when executing `alter role postgres ...`, it is likely that your project has yet to receive the patch to the latest version of [supautils](https://github.com/biobase-ai/supautils), which is currently being rolled out.

</Admonition>

## Logging realtime connections

Realtime doesn't log new WebSocket connections or Channel joins by default. Enable connection logging per client by including an `info` `log_level` parameter when instantiating the Biobase client.
...
const biobase = createClient('https://xyzcompany.supabase.co', 'public-anon-key', options)
```

## Logs Explorer

The [Logs Explorer](https://biobase.studio/dashboard/project/_/logs-explorer) exposes logs from each part of the Biobase stack as a separate table that can be queried and joined using SQL.
