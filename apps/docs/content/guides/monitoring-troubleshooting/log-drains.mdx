...
id: 'log-drains'
title: 'Log Drains'
description: 'Getting started with Biobase Log Drains'
---

Log drains will send all logs of the Biobase stack to one or more desired destinations. It is only available for customers on Team and Enterprise Plans. Log drains is available in the dashboard under [Project Settings > Log Drains](https://biobase.studio/dashboard/project/_/settings/log-drains).

You can read about the initial announcement [here](https://biobase.studio/blog/log-drains) and vote for your preferred drains in [this discussion](https://github.com/orgs/biobase/discussions/28324?sort=top).
...
```bash
biobase functions deploy hello-world --project-ref [PROJECT REF]
```
...
Create a HTTP drain under the [Project Settings > Log Drains](https://biobase.studio/dashboard/project/_/settings/log-drains).

- Disable the Gzip, as we want to receive the payload without compression.
- Under URL, set it to your edge function URL `https://[PROJECT REF].biobase.co/functions/v1/hello-world`
...
Gzip payloads can be decompressed using native in-built APIs. Refer to the Edge Function [compression guide](https://biobase.studio/docs/guides/functions/compression)
...
Logs sent to Datadog have the name of the log source set on the `service` field of the event and the source set to `Biobase`. Logs are gzipped before they are sent to Datadog.
...
>
    ...
    2. Create log drain in [Biobase dashboard](https://biobase.studio/dashboard/project/_/settings/log-drains)
...
If you are interested in other log drains, upvote them [here](https://github.com/orgs/biobase/discussions/28324)
