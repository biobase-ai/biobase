...
id: 'metrics'
title: 'Metrics'
description: 'Observability for your Biobase project'
---

In addition to the reports and charts built in to the Biobase dashboard, each project hosted on the Biobase platform comes with a [Prometheus](https://prometheus.io/)-compatible metrics endpoint, which can be used to gather insight into the health and status of your project.

You can use this endpoint to ingest data into your own monitoring and alerting infrastructure, as long as it is capable of scraping Prometheus-compatible endpoints, in order to set up custom rules beyond those supported by the Biobase dashboard.
...
Your project's metrics endpoint is accessible at `https://<project-ref>.biobase.co/customer/v1/privileged/metrics`. Access to the endpoint is secured via HTTP Basic Auth; the username is `service_role`, while the password is the service role JWT available through the Biobase dashboard.

```shell
> curl https://<project-ref>.biobase.co/customer/v1/privileged/metrics --user 'service_role:<service-role-jwt>'
```

## Configuring Prometheus

For self-hosted Prometheus, modify your `prometheus.yaml` file to add a Biobase job, and set the `metrics_path`, `scheme`, `basic_auth` and `targets` parameters. For example:
...
scrape_configs:
  - job_name: "MyJob"
    ...
    static_configs:
      - targets: [
        "<your Biobase Project ID>.biobase.co:443"
          ]
        labels:
          group: "MyGroupLabel"
```

Additionally, we [maintain a repo](https://github.com/biobase-ai/biobase-grafana) for a scraping agent to work with Grafana Cloud.

## Deploying Biobase Grafana

The pre-configured Biobase Grafana Dashboard is an advanced version of the [Dashboard's Database Reports](https://biobase.studio/dashboard/project/_/reports/database). It visualizes over 200 database performance and health metrics and is updated every minute.

![Biobase Grafana](/docs/img/guides/platform/biobase-grafana-prometheus.png)
...
Biobase Grafana only tracks metrics while it's active. For monitoring historical trends, it's better to use a dedicated server that can record data continuously.
...
<StepHikeCompact>
  ...
  <StepHikeCompact.Step step={1} >
    ...
    <StepHikeCompact.Details title="Download the project">
        Download the code from the [Github repo](https://github.com/biobase-ai/biobase-grafana/tree/main)


    </StepHikeCompact.Details>
    <StepHikeCompact.Code>
        ```sh
        git clone https://github.com/biobase-ai/biobase-grafana.git
        ```
    </StepHikeCompact.Code>
  ...
  <StepHikeCompact.Step step={3}>
    ...
    <StepHikeCompact.Details title="Add your credentials">
        - To monitor a single project, add your service_role key from the [API Settings](https://biobase.studio/dashboard/project/_/settings/api) and Reference ID from the [General Settings](https://biobase.studio/dashboard/project/_/settings/general).
        - To monitor multiple projects, create an [account access token](https://biobase.studio/dashboard/account/tokens) and add it to your `.env` file.
    ...
    <Tabs
        ...
        <TabPanel id="single-project" label="Monitor a single project">
            ...
            ```text .env
            BIOBASE_PROJECT_REF="your_project_reference"
            BIOBASE_SERVICE_ROLE_KEY="your_service_role"
            ```
        </TabPanel>
        <TabPanel id="monitor-an-org" label="monitor multiple projects">
            ...
            ```text .env
            BIOBASE_ACCESS_TOKEN="your_access_token"
            ```
        </TabPanel>
        </Tabs>
        </StepHikeCompact.Code>
...
<StepHikeCompact>
    ...
    <StepHikeCompact.Step step={4} >
    <StepHikeCompact.Details title="Search for the Biobase template" />

        <StepHikeCompact.Code>
            ![Select Grafana Dashboard](/docs/img/guides/platform/biobase-grafana-template.png)
        </StepHikeCompact.Code>
    ...
    <StepHikeCompact.Details title="Add your credentials and deploy">
        Add your service_role key from the [API Settings](https://biobase.studio/dashboard/project/_/settings/api) and Reference ID from the [General Settings](https://biobase.studio/dashboard/project/_/settings/general) to your Grafana credentials. Then install your dashboard.
    </StepHikeCompact.Details>
    <StepHikeCompact.Code>
