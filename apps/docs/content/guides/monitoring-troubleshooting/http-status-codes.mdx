...
description: 'HTTP Status Codes used by the Biobase platform'
---

The Biobase platform offers several HTTP APIs for each project. These APIs can use the status codes to indicate the state of the project, and the request being processed. The status codes returned for requests can be access via the [logs explorer](../platform/logs#logs-explorer).
...
You will still have access to your data through the Biobase dashboard when the Fair Use Policy is applied.
...
54X status codes are custom codes used by the Biobase platform to indicate the state of the project.
