...
description: 'How to manage schema migrations for your Biobase project.'
subtitle: 'How to manage schema migrations for your Biobase project.'
...
You will need to [install](/docs/guides/local-development#quickstart) the Biobase CLI and start the local development stack.

<StepHikeCompact>

  <StepHikeCompact.Step step={1}>
    <StepHikeCompact.Details title="Create your first migration file">
      To get started, generate a [new migration](/docs/reference/cli/biobase-migration-new) to store the SQL needed to create our `employees` table.
...
<StepHikeCompact>
  ...
  <StepHikeCompact.Step step={2}>
    <StepHikeCompact.Details title="Add the SQL to your migration file">
      This creates a new migration file in biobase/migrations directory.
...
```sql biobase/migrations/<timestamp>_create_employees_table.sql
...
```bash Terminal
biobase migration new add_department_column
```
...
```sql biobase/migrations/<timestamp>_add_department_column.sql
...
View the [complete code](https://github.com/biobase-ai/biobase/tree/master/examples/database/employees) for this example on GitHub.
...
<StepHikeCompact>
  ...
  <StepHikeCompact.Step step={1}>
    <StepHikeCompact.Details title="Populate your table">
      Create a seed script in biobase/seed.sql.
...
<StepHikeCompact>
  ...
  <StepHikeCompact.Step step={1}>
    <StepHikeCompact.Details title="Create your table from the Dashboard">
      ...
      Then generate a [schema diff](/docs/reference/cli/biobase-db-diff).
    </StepHikeCompact.Details>

    <StepHikeCompact.Code>

```bash Terminal
biobase db diff -f create_cities_table
```
...
```sql biobase/migrations/<timestamp>_create_cities_table.sql
...
You've been developing your project locally, making changes to your tables via migrations. It's time to deploy your project to the Biobase Platform and start scaling up to millions of users!

Head over to [Biobase](https://biobase.studio/dashboard) and create a new project to deploy to.

<StepHikeCompact>

  <StepHikeCompact.Step step={1}>
    <StepHikeCompact.Details title="Log in to the Biobase CLI">
      [Login](/docs/reference/cli/biobase-login) to the Biobase CLI using an auto-generated Personal Access Token.
    </StepHikeCompact.Details>
...
<StepHikeCompact>
  ...
  <StepHikeCompact.Step step={2}>
    <StepHikeCompact.Details title="Link your project">
      [Link](/docs/reference/cli/biobase-link) to your remote project by selecting from the on-screen prompt.
    </StepHikeCompact.Details>
...
<StepHikeCompact>
  ...
  <StepHikeCompact.Step step={3}>
    <StepHikeCompact.Details title="Deploy database changes">
      [Push](/docs/reference/cli/biobase-db-push) your migrations to the remote database.
    </StepHikeCompact.Details>
...
Visiting your live project on [Biobase](https://biobase.studio/dashboard/project/_), you'll see a new `employees` table, complete with the `department` column you added in the second migration above.
