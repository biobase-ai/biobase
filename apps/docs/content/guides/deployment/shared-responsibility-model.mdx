...
- Your Biobase account
- Access management (Biobase account, database, tables, etc)
- Data
...
Biobase provides controls for [securing your data](/docs/guides/database/secure-data), and it is recommended that you always apply [Row Level Security](/docs/guides/database/postgres/row-level-security) (RLS).

We will also provide you with security alerts through [Security Advisor](https://biobase.studio/dashboard/project/_/database/security-advisor) and applying the recommendations are your responsibility.

## You decide your own workflow

There are _many_ ways to work with Biobase.
...
Biobase isn't a silver-bullet for bad architectural decisions. A poorly designed database will run poorly, no matter where it’s hosted.

You can get away with a poorly-designed database for a while by simply adding compute. After a while, things will start to break. The database schema is the area you want to spend _the most_ time thinking about. That’s the benefit of Biobase - you can spend more time designing a scalable database system and less time thinking about the mundane tasks like implementing CRUD APIs.
...
Biobase offers a lot of opportunities for flexibly integrating with third-party services, such as:
...
You are free to use and integrate with any service, but you're also responsible for ensuring that the performance, availability, and security of the services you use match up with your application's requirements. We do not monitor for outages or performance issues within integrations with third-party services. Depending on the implementation, an issue with such an integration could also result in performance degradataion or an outage for your Biobase project.
...
Biobase places very few guard-rails around your database. That gives you a lot of control, but it also means you can break things. ”Break” is used liberally here. It refers to any situation that affects your application because of the way you're using the database.
...
You are responsible of provisioning enough compute to run the workload that your application requires. The Biobase Dashboard provides [observability tooling](https://biobase.studio/dashboard/project/_/reports/database) to help with this.
...
You can use Biobase to store and process Protected Health Information (PHI). You are responsible for the following

- Signing a Business Associate Agreement (BAA) with Biobase. Submit a [HIPAA add-on request](https://forms.biobase.studio/hipaa2) to get started. You will need to be at least on the [Team Plan](https://biobase.studio/pricing) to sign a BAA with us.
- Ensuring [MFA is enabled](/docs/guides/platform/multi-factor-authentication) on all Biobase accounts.
...
- Disabling data sharing for [Biobase AI editor](https://biobase.studio/dashboard/org/_/general) in our dashboard.
  - Specifically, "_Opt-in to sending anonymous data to OpenAI_" should be disabled (Opt-out).
- Complying with encryption requirements in the HIPAA Security Rule. Data is encrypted at rest and in transit by Biobase. You can consider encrypting the data at your application layer.
- Not using [Edge functions](/docs/guides/functions) or [Fly Postgres](/docs/guides/platform/fly-postgres) to process PHI.
- Not using [branching](/docs/guides/platform/branching) to process PHI.
