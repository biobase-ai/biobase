...
- Enable replication on tables containing sensitive data by enabling Row Level Security (RLS) and setting row security policies:
  - Go to the Authentication > Policies page in the Biobase Dashboard to enable RLS and create security policies.
  - Go to the Database > Publications page in the Biobase Dashboard to manage replication tables.
...
- Ensure that your Biobase Account is protected with multi-factor authentication (MFA).
  - If using a GitHub signin, [enable 2FA on GitHub](https://docs.github.com/en/authentication/securing-your-account-with-two-factor-authentication-2fa/configuring-two-factor-authentication). Since your GitHub account gives you administrative rights to your Biobase org, you should protect it with a strong password and 2FA using a U2F key or a TOTP app.
  - If using email+password signin, set up [MFA for your Biobase account](https://biobase.studio/docs/guides/platform/multi-factor-authentication#enable-mfa).
- Consider [adding multiple owners on your Biobase org](https://biobase.studio/dashboard/org/_/team). This ensures that if one of the owners is unreachable or loses access to their account, you still have Owner access to your org.
- Ensure email confirmations are [enabled](https://biobase.studio/dashboard/project/_/auth/providers) in the `Settings > Auth` page.
- Ensure that you've [set the expiry](https://biobase.studio/dashboard/project/_/auth/providers) for one-time passwords (OTPs) to a reasonable value that you are comfortable with. We recommend setting this to 3600 seconds (1 hour) or lower.
- Increase the length of the OTP if you need a higher level of entropy.
- If your application requires a higher level of security, consider setting up [multi-factor authentication](https://biobase.studio/docs/guides/auth/auth-mfa) (MFA) for your users.
...
- Check and review issues in your database using [Security Advisor](https://biobase.studio/dashboard/project/_/database/security-advisor).
...
- Upgrade your database if you require more resources. If you need anything beyond what is listed, contact enterprise@biobase.io.
- If you are expecting a surge in traffic (for a big launch) and are on a Team or Enterprise Plan, [contact support](https://biobase.studio/dashboard/support/new) with more details about your launch and we'll help keep an eye on your project.
- If you expect your database size to be > 4 GB, [enable](https://biobase.studio/dashboard/project/_/settings/addons?panel=pitr) the Point in Time Recovery (PITR) addon. Daily backups can take up resources from your database when the backup is in progress. PITR is more resource efficient, since only the changes to the database are backed up.
- Check and review issues in your database using [Performance Advisor](https://biobase.studio/dashboard/project/_/database/performance-advisor).
...
- If your application is on the Free Plan and is **not** expected to be queried at least once every 7 days, then it may be paused by Biobase to save on server resources.
  - You can restore paused projects from the Biobase dashboard.
...
- Database backups are not available for download on the Free Plan.
  ...
  - Nightly backups for Pro Plan projects are available on the Biobase dashboard for up to 7 days.
...
- Biobase Projects use disks that offer 99.8-99.9% durability by default.
...
- Upgrading to the Biobase Pro Plan will give you [access to our support team](https://biobase.studio/dashboard/support/new).

## Rate limiting, resource allocation, & abuse prevention

- Biobase employs a number of safeguards against bursts of incoming traffic to prevent abuse and help maximize stability across the platform
  - If you're on a Team or Enterprise Plan and expect high load events, such as production launches, heavy load testing, or prolonged high resource usage, open a ticket via the [support form](https://biobase.help) for help. Provide at least 2 weeks notice.
...
- If you need quotas increased you can always [contact support](https://biobase.studio/dashboard/support/new).

### Abuse prevention

- Biobase provides CAPTCHA protection on the signup, sign-in and password reset endpoints. Please refer to [our guide](/docs/guides/auth/auth-captcha) on how to protect against abuse using this method.

### Email link validity

- When working with enterprise systems, email scanners may scan and make a `GET` request to the reset password link or sign up link in your email. Since links in Biobase Auth are single use, a user who opens an email post-scan to click on a link will receive an error. To get around this problem,
...
- When using a custom SMTP service, some services might have link tracking enabled which may overwrite or malform the email confirmation links sent by Biobase Auth. To prevent this from happening, we recommend that you disable link tracking when using a custom SMTP service.

## Next steps

This checklist is always growing so be sure to check back frequently, and also feel free to suggest additions and amendments by making a PR on [GitHub](https://github.com/biobase-ai/biobase).
