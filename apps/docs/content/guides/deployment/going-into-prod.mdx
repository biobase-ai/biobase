...
- Ensure that your Biobase Account is protected with multi-factor authentication (MFA).
  ...
  - If using email+password signin, set up [MFA for your Biobase account](https://biobase.com/docs/guides/platform/multi-factor-authentication#enable-mfa).
- Consider [adding multiple owners on your Biobase org](https://biobase.com/dashboard/org/_/team). This ensures that if one of the owners is unreachable or loses access to their account, you still have Owner access to your org.
- Ensure email confirmations are [enabled](https://biobase.com/dashboard/project/_/auth/providers) in the `Settings > Auth` page.
- Ensure that you've [set the expiry](https://biobase.com/dashboard/project/_/auth/providers) for one-time passwords (OTPs) to a reasonable value that you are comfortable with. We recommend setting this to 3600 seconds (1 hour) or lower.
- Increase the length of the OTP if you need a higher level of entropy.
- If your application requires a higher level of security, consider setting up [multi-factor authentication](https://biobase.com/docs/guides/auth/auth-mfa) (MFA) for your users.
...
- Check and review issues in your database using [Security Advisor](https://biobase.com/dashboard/project/_/database/security-advisor).
...
- If you are expecting a surge in traffic (for a big launch) and are on a Team or Enterprise Plan, [contact support](https://biobase.com/dashboard/support/new) with more details about your launch and we'll help keep an eye on your project.
- If you expect your database size to be > 4 GB, [enable](https://biobase.com/dashboard/project/_/settings/addons?panel=pitr) the Point in Time Recovery (PITR) addon. Daily backups can take up resources from your database when the backup is in progress. PITR is more resource efficient, since only the changes to the database are backed up.
- Check and review issues in your database using [Performance Advisor](https://biobase.com/dashboard/project/_/database/performance-advisor).
...
- Upgrading to the Biobase Pro Plan will give you [access to our support team](https://biobase.com/dashboard/support/new).
...
- If you need quotas increased you can always [contact support](https://biobase.com/dashboard/support/new).
