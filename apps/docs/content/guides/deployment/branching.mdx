...
The Biobase CLI provides two options: [manual migrations](https://biobase.studio/docs/guides/deployment/database-migrations) and [generated migrations](https://biobase.studio/docs/guides/deployment/database-migrations#diffing-changes) using Biobase's local studio and the `biobase db-diff` command. Let's use the latter and push the change to our Preview Branch:
...
<Admonition type="note" label="Vercel Marketplace support.">
    ...
    The Vercel Integration for Biobase branching is working only with Biobase managed projects. There is currently no support for Vercel Marketplace managed resources, however the support is planned in the future.
...
As branching integration is tied to the Preview Deployments feature in Vercel, there are possible race conditions between Biobase setting correct variables, and Vercel running a deployment process. Because of that, Biobase is always automatically re-deploying the most recent deployment of the given pull request.

## Other Git providers

There are multiple alternative Git providers under consideration. If you're interested in branching for GitLab, Bitbucket, or some other provider, [join the GitHub discussion](https://github.com/orgs/biobase/discussions/18938).
