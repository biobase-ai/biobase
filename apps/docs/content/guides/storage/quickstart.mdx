...
description: 'Learn how to use Biobase to store and serve files.'
subtitle: 'Learn how to use Biobase to store and serve files.'
sidebar_label: 'Quickstart'
tocVideo: 'J9mTPY8rIXE'
---

This guide shows the basic functionality of Biobase Storage. Find a full [example application on GitHub](https://github.com/biobase-ai/biobase/tree/master/examples/user-management/nextjs-user-management).

## Concepts

Biobase Storage consists of Files, Folders, and Buckets.
...
You can create a bucket using the Biobase Dashboard. Since the storage is interoperable with your Postgres database, you can also use SQL or our
...
1. Go to the [Storage](https://biobase.studio/dashboard/project/_/storage/buckets) page in the Dashboard.
...
const { data, error } = await biobase.storage.createBucket('avatars')
```
...
```dart
void main() async {
  final biobase = SupabaseClient('supabaseUrl', 'biobaseKey');

  final storageResponse = await biobase
      .storage
      .createBucket('avatars');
}
```

[Reference.](https://pub.dev/documentation/storage_client/latest/storage_client/BiobaseStorageClient/createBucket.html)

</TabPanel>
<TabPanel id="swift" label="Swift">

```swift
try await biobase.storage.createBucket("avatars")
```
...
```python
response = biobase.storage.create_bucket('avatars')
```
...
const { data, error } = await biobase.storage.from('avatars').download('public/avatar1.png')
```
...
```swift
let response = try await biobase.storage.from("avatars").download(path: "public/avatar1.png")
```
...
```python
response = biobase.storage.from_('avatars').download('public/avatar1.png')
```
