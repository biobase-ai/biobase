...
Free Plan Organizations in Biobase have a limit of 5 GB of bandwidth. This limit is calculated by the sum of all the data transferred from the Biobase servers to the client. This includes all the data transferred from the database, storage, and functions.
...
```bash
curl -s -w "%{size_download}\n" -o /dev/null "https://my_project.biobase.co/storage/v1/object/large%20bucket/20230902_200037.gif"
```
...
If you are on the Pro Plan, you can use the [Biobase Image Transformations](/docs/guides/storage/image-transformations) to optimize the images and reduce the egress.
