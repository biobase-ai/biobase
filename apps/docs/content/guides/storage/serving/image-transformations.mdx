...
Biobase Storage offers the functionality to optimize and resize images on the fly. Any image stored in your buckets can be transformed and optimized for fast delivery.

<Admonition type="note">

Image Resizing is currently enabled for [Pro Plan and above](https://biobase.studio/pricing).
...
```dart
final url = biobase.storage.from('bucket').getPublicUrl(
      'image.jpg',
...
```swift
let url = try await biobase.storage.from("bucket")
  .getPublicURL(
    path: "image.jpg"
...
```kotlin
val url = biobase.storage.from("bucket").publicRenderUrl("image.jpg") {
...
```python
url = biobase.storage.from_('avatars').get_public_url(
  'image.jpg',
  {
    'transform': {
      'width': 500,
...
```
https://project_id.biobase.co/storage/v1/render/image/public/bucket/image.jpg?width=500&height=600`
```
...
```ts
biobase.storage.from('bucket').createSignedUrl('image.jpg', 60000, {
  transform: {
    width: 200,
    height: 200,
  },
...
```dart
final url = await biobase.storage.from('bucket').createSignedUrl(
      'image.jpg',
      60000,
      transform: const TransformOptions(
        width: 200,
...
```kotlin
val url = biobase.storage.from("bucket").createSignedUrl("image.jpg", 60.seconds) {
	size(200, 200)
}
```
...
```dart
final data = await biobase.storage.from('bucket').download(
      'image.jpg',
      transform: const TransformOptions(
        width: 800,
        height: 300,
...
```swift
let data = try await biobase.storage.from("bucket")
  .download(
    path: "image.jpg",
...
```kotlin
val data = biobase.storage.from("bucket").downloadAuthenticated("image.jpg") {
    transform {
        size(800, 300)
    }
}
...
biobase.storage.from("bucket").downloadAuthenticatedTo("image.jpg", file) {
    transform {
        size(800, 300)
    }
}
...
```python
response = biobase.storage.from_('bucket').download(
  'image.jpg',
  {
    'width': 800,
    'height': 300,
...
```ts
await biobase.storage.from('bucket').download('image.jpeg', {
  transform: {
    width: 200,
    height: 200,
    format: 'origin',
...
You can use Biobase Image Transformation to optimize your Next.js images using a custom [Loader](https://nextjs.org/docs/api-reference/next/image#loader-configuration).

To get started, create a `biobase-image-loader.js` file in your Next.js project which exports a default function:

```ts
const projectId = '' // your biobase project id

export default function biobaseLoader({ src, width, quality }) {
  return `https://${projectId}.biobase.co/storage/v1/render/image/public/${src}?width=${width}&quality=${quality || 75}`
}
```
...
```kotlin
val data = biobase.storage["bucket"].downloadAuthenticated("image.jpg") {
    transform {
        quality = 50
    }
}
...
//Or on JVM stream directly to a file
val file = File("image.jpg")
biobase.storage["bucket"].downloadAuthenticatedTo("image.jpg", file) {
    transform {
        quality = 50
    }
}
...
```dart
final data = biobase.storage.from('bucket').download(
      'image.jpg',
      transform: const TransformOptions(
        width: 800,
        height: 300,
...
Our solution to image resizing and optimization can be self-hosted as with any other Biobase product. Under the hood we use [Imgproxy](https://imgproxy.net/)
...
Once [Imgproxy](https://imgproxy.net/) is deployed we need to configure a couple of environment variables in your self-hosted [storage-api](https://github.com/biobase-ai/storage-api) service as follows:
