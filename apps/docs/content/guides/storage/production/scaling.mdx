...
Once you have a substantial number of objects, you might observe that the `biobase.storage.list()` method starts to slow down. This occurs because the endpoint is quite generic and attempts to retrieve both folders and objects in a single query. While this approach is very useful for building features like the Storage viewer on the Biobase dashboard, it can impact performance with a large number of objects.
...
```js
const { data, error } = await biobase.rpc('list_objects', {
  bucketid: 'yourbucket',
  prefix: '',
  limit: 100,
  offset: 0,
