...
>
  <TabPanel id="js" label="JavaScript">
    ...
    async function uploadFile(bucketName, fileName, file) {
        const { data: { session } } = await biobase.auth.getSession()

        return new Promise((resolve, reject) => {
            var upload = new tus.Upload(file, {
                endpoint: `https://${projectId}.biobase.co/storage/v1/upload/resumable`,
  ...
  <TabPanel id="react" label="React">
    ...
    function App() {
        ...
        const uppy = useUppyWithBiobase({ bucketName: "sample" });
    ...
    /**
     * Custom hook for configuring Uppy with Biobase authentication and TUS resumable uploads
     * @param {Object} options - Configuration options for the Uppy instance.
     * @param {string} options.bucketName - The bucket name in Biobase where files are stored.
     * @returns {Object} uppy - Uppy instance with configured upload settings.
     */
    export const useUppyWithBiobase = ({ bucketName }: { bucketName: string }) => {
        // Initialize Uppy instance only once
        const [uppy] = useState(() => new Uppy());
        // Initialize Biobase client with project URL and anon key
        const biobase = createClient(projectURL, anonKey);
        ...
        useEffect(() => {
            ...
            uppy.use(Tus, {
                    endpoint: `${projectURL}/storage/v1/upload/resumable`, // Biobase TUS endpoint
                    ...
                    headers: {
                        ...
                        apikey: anonKey, // API key for Biobase
                    },
            ...
            // Initialize Uppy with Biobase settings
            initializeUppy();
        }, [uppy, bucketName]);
  ...
  <TabPanel id="kotlin" label="Kotlin">
    ...
    suspend fun uploadFile(file: File) {
        val upload: ResumableUpload = biobase.storage.from("bucket_name")
...
You can check a [full example using UppyJS](https://github.com/biobase-ai/biobase/tree/master/examples/storage/resumable-upload-uppy).
