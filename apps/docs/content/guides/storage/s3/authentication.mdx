...
subtitle: 'Learn about authenticating with Biobase Storage S3.'
sidebar_label: 'S3'
---

You have two options to authenticate with Biobase Storage S3:
...
This is all the information you need to connect to Biobase Storage using any S3-compatible service.
...
>
    <TabPanel id="javascript" label="aws-sdk-js">
        ...
        const client = new S3Client({
          ...
          endpoint: 'https://project_ref.biobase.co/storage/v1/s3',
          credentials: {
    ...
    <TabPanel id="credentials" label="AWS Credentials">
        ...
        endpoint_url = https://project_ref.biobase.co/storage/v1/s3
        region = project_region
        ```
    </TabPanel>
...
You can authenticate to Biobase S3 with a user JWT token to provide limited access via RLS to all S3 operations. This is useful when you want initialize the S3 client on the server scoped to a specific user, or use the S3 client directly from the client side.
