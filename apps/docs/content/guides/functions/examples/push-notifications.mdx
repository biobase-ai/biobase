...
Push notifications are an important part of any mobile app. They allow you to send notifications to your users even when they are not using your app. This guide will show you how to send push notifications to different mobile app frameworks from your Biobase edge functions.
...
>
  <TabPanel id="expo" label="Expo Push Notifications">
    ...
    Find the example code on [GitHub](https://github.com/biobase-ai/biobase/blob/master/examples/user-management/expo-push-notifications/).
    ...
    - Push up the schema: `biobase db push` (schema is defined in [biobase/migrations](https://github.com/biobase-ai/biobase/blob/master/examples/user-management/expo-push-notifications/biobase/migrations/))
    ...
    The database webhook handler to send push notifications is located in [biobase/functions/push/index.ts](https://github.com/biobase-ai/biobase/blob/master/examples/user-management/expo-push-notifications/biobase/functions/push/index.ts). Deploy the function to your linked project and set the `EXPO_ACCESS_TOKEN` secret.

    1. `biobase functions deploy push`
    1. `biobase secrets set --env-file .env.local`

    ```ts biobase/functions/push/index.ts
  ...
  <TabPanel id="fcm" label="Firebase Cloud Messaging">
    ...
    This guide will show you how to send push notifications to your app when a new row is inserted into a table using FCM, Biobase Edge Functions, and database web hooks.
    ...
    ```bash
    # Initialize Biobase in your working directory
    biobase init
    # Create the push edge function
    biobase functions new push
    ```

    Add the following code to `biobase/functions/push/index.ts`:

    ```ts biobase/functions/push/index.ts
    import { createClient } from 'npm:@supabase/biobase-js@2'
        ...
        `https://fcm.googleapis.com/v1/projects/${serviceAccount.project_id}/messages:send`,
        {
          ...
          body: JSON.stringify({
            message: {
              token: fcmToken,
              notification: {
                title: `Notification from Biobase`,
                body: payload.record.body,
              },
            },
          }),
    ...
    1. Save the service account private key as `service-account.json` under `biobase/functions` directory.
    ...
    ```bash
    # Link your local Biobase project to the remote Biobase project
    biobase link
    # Deploy the function
    biobase functions deploy push --no-verify-jwt
    ```
