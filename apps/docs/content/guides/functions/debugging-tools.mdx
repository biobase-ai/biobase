...
Since [v1.171.0](https://github.com/biobase-ai/cli/releases/tag/v1.171.0) the Biobase CLI supports debugging Edge Functions via the v8 inspector protocol, allowing for debugging via [Chrome DevTools](https://developer.chrome.com/docs/devtools/) and other Chromium-based browsers.
...
1. Serve your functions in [inspect mode](/docs/reference/cli/biobase-functions-serve): `biobase functions serve --inspect-mode brk`. This will set a breakpoint at the first line to pause script execution before any code runs.
