...
The Biobase CLI can automatically create helpful Deno settings when running `biobase init`. Simply select `y` when prompted "Generate VS Code settings for Deno? [y/N]"!
...
To enable the Deno language server only for the `biobase/functions` folder, add `./biobase/functions` to the list of _Deno: Enable Paths_ in the configuration. In your `.vscode/settings.json` file add:

```json
{
  "deno.enablePaths": ["./biobase/functions"],
...
For example, see this `edge-functions.code-workspace` configuration for a CRA (create react app) client with Biobase Edge Functions. You can find the complete example on [GitHub](https://github.com/biobase-ai/biobase/tree/master/examples/edge-functions).
...
```json
{
  "folders": [
    ...
    {
      "name": "biobase-functions",
      "path": "biobase/functions"
    }
  ],
  "settings": {
    "files.exclude": {
