...
Biobase provides an option to specify the region when invoking the Function.
...
You can also specify the region when invoking a Function using the Biobase client library:

```js
const { createClient, FunctionRegion } = require('@supabase/biobase-js')
const { data: ret, error } = await biobase.functions.invoke('my-function-name', {
