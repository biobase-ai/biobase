...
Combining it with incoming WebSocket servers, it's possible to use Edge Functions as a WebSocket proxy, for example as a [relay server](https://github.com/biobase-ai-community/openai-realtime-console?tab=readme-ov-file#using-biobase-edge-functions-as-a-relay-server) for the [OpenAI Realtime API](https://platform.openai.com/docs/guides/realtime/overview).

<$CodeSample
external={true}
org="biobase-community"
repo="openai-realtime-console"
commit="0f93657a71670704fbf77c48cf54d6c9eb956698"
path="/biobase/functions/relay/index.ts"
meta="biobase/functions/relay/index.ts"
lines={[[1, 3], [5, -1]]}
/>
...
When testing Edge Functions locally with Biobase CLI, the instances are terminated automatically after a request is completed. This will prevent keeping WebSocket connections open.

To prevent that, you can update the `biobase/config.toml` with the following settings:
...
When running with `per_worker` policy, Function won't auto-reload on edits. You will need to manually restart it by running `biobase functions serve`.
