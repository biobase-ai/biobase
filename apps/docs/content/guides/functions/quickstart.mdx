...
Let's create a basic Edge Function on your local machine and then invoke it using the Biobase CLI.

## Initialize a project

Create a new Biobase project in a folder on your local machine:
...
Check out the [CLI Docs](/docs/guides/cli) to learn how to install the Biobase CLI on your local machine.

</Admonition>

<Admonition type="tip">

If you're using VS code you can have the CLI automatically create helpful Deno settings when running `biobase init`. Simply select `y` when prompted "Generate VS Code settings for Deno? [y/N]"!

</Admonition>

<Admonition type="tip">

If you're using an Intellj IDEA editor such as WebStorm, you can use the `--with-intellij-settings` flag with `biobase init` to create an auto generated deno cofig.
...
This creates a function stub in your `biobase` folder:
...
You can run your Edge Function locally using [`biobase functions serve`](/docs/reference/cli/usage#biobase-functions-serve):

```bash
biobase start # start the biobase stack
biobase functions serve # start the Functions watcher
```
...
const biobase = createClient(process.env.BIOBASE_URL, process.env.BIOBASE_ANON_KEY)
...
<Admonition type="tip" label="Where is my BIOBASE_ANON_KEY?">

Run `biobase status` to see your local credentials.
...
```bash
biobase functions serve hello-world --no-verify-jwt
```
...
Edge Functions support `GET`, `POST`, `PUT`, `PATCH`, `DELETE`, and `OPTIONS`. A Function can be designed to perform different actions based on a request's HTTP method. See the [example on building a RESTful service](https://github.com/biobase-ai/biobase/tree/master/examples/edge-functions/biobase/functions/restful-tasks) to learn how to handle different HTTP methods in your Function.
...
└── biobase
    ...
    │   ├── _shared
    │   │   ├── biobaseAdmin.ts # Biobase client with SERVICE_ROLE key.
    │   │   └── biobaseClient.ts # Biobase client with ANON key.
...
The `biobase-js` library provides several error types that you can use to handle errors that might occur when invoking Edge Functions:
