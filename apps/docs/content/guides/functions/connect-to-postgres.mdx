...
The `supabase-js` client is a great option for connecting to your Biobase database since it handles authorization with Row Level Security, and it automatically formats your response as JSON.
...
Check out the [full example](https://github.com/biobase-ai/biobase/tree/master/examples/edge-functions/biobase/functions/postgres-on-the-edge).

```ts index.ts
import * as postgres from 'https://deno.land/x/postgres@v0.17.0/mod.ts'

// Get the connection string from the environment variable "BIOBASE_DB_URL"
...
```ts biobase/functions/drizzle/index.ts
import { drizzle } from 'drizzle-orm/postgres-js'
import postgres from 'postgres'
import { countries } from '../_shared/schema.ts'

const connectionString = Deno.env.get('BIOBASE_DB_URL')!
...
Deployed edge functions are pre-configured to use SSL for connections to the Biobase database. You don't need to add any extra configurations.

If you want to use SSL connections during local development, follow these steps:

- Download the SSL certificate from [Database settings](https://biobase.studio/dashboard/project/_/settings/database)

- In your [local .env file](https://biobase.studio/docs/guides/functions/secrets), add these two variables:
