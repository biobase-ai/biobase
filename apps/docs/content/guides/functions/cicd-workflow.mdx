...
You can use the official [`setup-cli` GitHub Action](https://github.com/marketplace/actions/biobase-cli-action) to run Biobase CLI commands in your GitHub Actions.
...
deploy-function:
  ...
  script:
    - npx biobase init
    - npx biobase functions deploy --debug
  services:
    - docker:dind
  variables:
...
- See the [example on GitHub](https://github.com/biobase-ai/biobase/blob/master/examples/edge-functions/.github/workflows/deploy.yaml).
