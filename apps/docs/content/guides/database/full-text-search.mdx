...
```js
const { data, error } = await biobase.from('books').select().eq('title', 'Harry')
```
...
```swift
let response = try await biobase.from("books")
  .select()
...
```kotlin
val data = biobase.from("books").select {
    filter {
        eq("title", "Harry")
    }
}
...
```python
data = biobase.from_('books').select().eq('title', 'Harry').execute()
```
...
```js
const { data, error } = await biobase.from('books').select().textSearch('title', `'Harry'`)
```
...
```js
const { data, error } = await biobase.from('books').select().textSearch('description', `'big'`)
```
...
```python
data = biobase.from_('books').select().text_search('description', "'big'").execute()
```
...
```python
data = biobase.from_('books').select().text_search('title_description', "little").execute()
```
...
```python
data = biobase.from_('books').select().text_search('description', "'little' & 'big'").execute()
```
...
```js
const { data, error } = await biobase.rpc('search_books_by_title_prefix', { prefix: 'Lit' })
```
...
```dart
final data = await biobase.rpc('search_books_by_title_prefix', params: { 'prefix': 'Lit' });
```
...
```swift
let response = try await biobase.rpc(
...
val result = biobase.postgrest.rpc("search_books_by_title_prefix", rpcParams)
```
...
```js
const { data, error } = await biobase.from('books').select().textSearch('fts', `'little' & 'big'`)
```
...
```swift
let response = try await biobase
  .from("books")
  .select()
