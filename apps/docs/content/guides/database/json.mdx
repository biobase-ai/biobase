...
1. Go to the [Table Editor](https://biobase.com/dashboard/project/_/editor) page in the Dashboard.
...
```js
const { data, error } = await biobase.from('books').insert([
  {
...
```dart
await biobase.from('books').insert([
  {
    'title': 'The Poky Little Puppy',
    'author': 'Janette Sebring Lowrey',
    'metadata': {
...
Biobase Swift provides a convenience `AnyJSON` type.
...
```kotlin
val data = biobase.from("books").insert(listOf(
...
```js
const { data, error } = await biobase.from('books').select(`
    title,
...
```kotlin
val data = biobase.from("books").select(Columns.raw("""
    title,
    description: metadata->description,
    price: metadata->price,
    low_age: metadata->ages->0,
...
```python
data = biobase.from_('books').select("""
  title,
  description: metadata->description,
  price: metadata->price,
  low_age: metadata->ages->0,
...
Biobase provides the [`pg_jsonschema` extension](/docs/guides/database/extensions/pg_jsonschema) that adds the ability to validate `json` and `jsonb` data types against [JSON Schema](https://json-schema.org/) documents.
