...
Requests made through the Biobase API and Dashboard have a timeout of 60 seconds. The database also has a global default timeout of 2 minutes.
...
Because it applies to sessions only, it can only be used with connections through Supavisor in session mode (port 5432) or a direct connection. It cannot be used in the Dashboard, with the Biobase Client API, nor with Supavisor in Transaction mode (port 6543).
...
See the full guide on [changing session timeouts](https://github.com/orgs/biobase/discussions/21133).

### Function level

This works with the Database REST API when called from the Biobase client libraries:
...
<Admonition type="tip">
  If you are changing the timeout for the Biobase Client API calls, you will need to reload
...
The Biobase Dashboard contains tools to help you identify timed-out and long-running queries.
...
| Role                       | API/Tool                                                                  |
| -------------------------- | ------------------------------------------------------------------------- |
| biobase_admin             | Used by Biobase to configure projects and for monitoring                 |
| authenticator              | PostgREST                                                                 |
| biobase_auth_admin        | Auth                                                                      |
| biobase_storage_admin     | Storage                                                                   |
| biobase_realtime_admin    | Realtime                                                                  |
| biobase_replication_admin | Synchronizes Read Replicas                                                |
| postgres                   | Biobase Dashboard and External Tools (e.g., Prisma, SQLAlchemy, PSQL...) |
