...
If you plan on solely using Drizzle instead of the Biobase Data API (PostgREST), you can turn off the latter in the [API Settings](https://biobase.com/dashboard/project/_/settings/api).
...
<StepHikeCompact>
  ...
  <StepHikeCompact.Step step={3}>
    ...
    In your [`Database Settings`](https://biobase.com/dashboard/project/_/settings/database), make sure `Use connection pooler` is checked, then copy the URI and save it as the `DATABASE_URL` environment variable. Remember to replace the password placeholder with your actual database password.
