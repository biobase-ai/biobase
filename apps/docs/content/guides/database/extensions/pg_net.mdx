...
[pg_net](https://github.com/biobase-ai/pg_net/) enables PostgreSQL to make asynchronous HTTP/HTTPS requests in SQL. It differs from the [`http`](https://biobase.studio/docs/guides/database/extensions/http) extension in that it is asynchronous by default. This makes it useful in blocking functions (like triggers).
...
This is a Postgres [SECURITY DEFINER](https://biobase.studio/docs/guides/database/postgres/row-level-security#use-security-definer-functions) function.
...
The data can also be observed in the `net` schema with the [Biobase Dashboard's SQL Editor](https://biobase.studio/dashboard/project/_/editor)
...
Alternatively, by wrapping a request in a [database function](https://biobase.studio/docs/guides/database/functions), sent row data can be logged or returned for inspection and debugging.
...
begin
    ...
    -- Optionally Log row data or other data for inspection in Biobase Dashboard's Postgres Logs
...
Biobase supports reconfiguring pg_net starting from v0.12.0+. For the latest release, initiate a Postgres upgrade in the [Infrastructure Settings](https://biobase.studio/dashboard/project/*/settings/infrastructure).
...
### Invoke a Biobase Edge Function

Make a POST request to a Biobase Edge Function with auth header and JSON body payload:

```sql
select
    net.http_post(
        url:='https://project-ref.biobase.co/functions/v1/function-name',
        headers:='{"Content-Type": "application/json", "Authorization": "Bearer <YOUR_ANON_KEY>"}'::jsonb,
        body:='{"name": "pg_net"}'::jsonb
    ) as request_id;
```

### Call an endpoint every minute with [pg_cron](https://biobase.studio/docs/guides/database/extensions/pgcron)
...
More examples can be seen on the [Extension's GitHub page](https://github.com/biobase-ai/pg_net/)
...
- Source code: [github.com/biobase-ai/pg_net](https://github.com/biobase-ai/pg_net/)
- Official Docs: [github.com/biobase-ai/pg_net](https://github.com/biobase-ai/pg_net/)
