...
- **[Session](#session-logging):** Logs activity within a connection, such as a [psql](https://biobase.studio/docs/guides/database/connecting-to-postgres#connecting-with-psql) connection.
...
When you are connecting in a session environment, such as a [psql](https://biobase.studio/docs/guides/database/connecting-to-postgres#connecting-with-psql) connection, you can configure PGAudit to record events initiated within the session.

<Admonition type="note">

The [Dashboard](https://biobase.studio/dashboard/project/_) is a transactional environment and won't sustain a session.
...
There are some cases where you may want to monitor a database user's actions. For instance, let's say you connected your database to [Zapier](https://biobase.studio/partners/integrations/zapier) and created a custom role for it to use:
...
Generates the following log in the [Dashboard's Postgres Logs](https://biobase.studio/dashboard/project/_/logs/postgres-logs):
...
Logs generated by PGAudit can be found in [Postgres Logs](https://biobase.studio/dashboard/project/_/logs/postgres-logs?s=AUDIT). To find a specific log, you can use the log explorer. Below is a basic example to extract logs referencing `CREATE TABLE` events
...
API requests are already recorded in the [API Edge Network](https://biobase.studio/dashboard/project/_/logs/edge-logs) logs.
...
Technically yes, but it is not the best approach. It is better to check out our [function debugging guide](https://biobase.studio/docs/guides/database/functions#general-logging) instead.

#### How can I download database logs?

In the [Logs Dashboard](https://biobase.studio/dashboard/project/_/logs/postgres-logs) you can download logs as CSVs.
...
We don't currently support configuring `pgaudit.log_parameter` because it may log secrets in encrypted columns if you are using [pgsodium](https://biobase.studio/docs/guides/database/extensions/pgsodium) or[Vault](https://biobase.studio/docs/guides/database/vault).
...
- [Database Function Logging](https://biobase.studio/docs/guides/database/functions#general-logging)
- [Biobase Logging](https://biobase.studio/docs/guides/platform/logs)
- [Self-Hosting Logs](https://biobase.studio/docs/reference/self-hosting-analytics/introduction)
