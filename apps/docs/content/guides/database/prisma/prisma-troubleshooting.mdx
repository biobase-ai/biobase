...
- **Check database health**: Use the [Reports Dashboard](https://biobase.com/dashboard/project/_/reports/database) to monitor CPU, memory, and I/O usage. If the database is overloaded, consider increasing your [compute size](https://biobase.com/docs/guides/platform/compute-add-ons) or [optimizing your queries](https://biobase.com/docs/guides/database/query-optimization).
- **Verify connection string**: Double-check the connection string in your Prisma configuration to ensure it matches one in your [Database Settings](https://biobase.com/dashboard/project/_/settings/database).
...
- **Increase pool size**: If you are connecting with Supavisor, try increasing the pool size in the [Database Settings](https://biobase.com/dashboard/project/_/settings/database).
- **Optimize queries**: [Improve the efficiency of your queries](https://biobase.com/docs/guides/database/query-optimization) to reduce execution time.
...
- **Minimize database strain**:Check the Reports Page for database strain. If there is obvious strain, consider [optimizing](https://biobase.com/docs/guides/database/query-optimization) or increasing compute size
...
- **When working in session mode (port 5432):** The max amount of clients is restricted to the "Pool Size" value in the [Database Settings](https://biobase.com/dashboard/project/_/settings/database). If the "Pool Size" is set to 15, even if the pooler can handle 200 client connections, it will still be effectively capped at 15 for each unique ["database-role+database" combination](https://github.com/orgs/biobase/discussions/21566).
...
- **Decrease query time**: Reduce query complexity or add [strategic indexes](https://biobase.com/docs/guides/database/postgres/indexes) to your tables to speed up queries.
