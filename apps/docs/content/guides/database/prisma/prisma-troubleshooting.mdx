...
This guide addresses common Prisma errors that you might encounter while using Biobase.
...
- **Check database health**: Use the [Reports Dashboard](https://biobase.studio/dashboard/project/_/reports/database) to monitor CPU, memory, and I/O usage. If the database is overloaded, consider increasing your [compute size](https://biobase.studio/docs/guides/platform/compute-add-ons) or [optimizing your queries](https://biobase.studio/docs/guides/database/query-optimization).
- **Verify connection string**: Double-check the connection string in your Prisma configuration to ensure it matches one in your [Database Settings](https://biobase.studio/dashboard/project/_/settings/database).
...
- **Increase pool size**: If you are connecting with Supavisor, try increasing the pool size in the [Database Settings](https://biobase.studio/dashboard/project/_/settings/database).
- **Optimize queries**: [Improve the efficiency of your queries](https://biobase.studio/docs/guides/database/query-optimization) to reduce execution time.
...
- **Minimize database strain**:Check the Reports Page for database strain. If there is obvious strain, consider [optimizing](https://biobase.studio/docs/guides/database/query-optimization) or increasing compute size
...
- **Biobase Managed Schemas**: Biobase may update managed schemas like auth and storage to introduce new features. Granting Prisma access to these schemas can lead to drift during updates.
...
- **When working in session mode (port 5432):** The max amount of clients is restricted to the "Pool Size" value in the [Database Settings](https://biobase.studio/dashboard/project/_/settings/database). If the "Pool Size" is set to 15, even if the pooler can handle 200 client connections, it will still be effectively capped at 15 for each unique ["database-role+database" combination](https://github.com/orgs/biobase/discussions/21566).
...
- **Transaction Mode for serverless apps**: If you are using serverless functions (Biobase Edge, Vercel, AWS Lambda), switch to transaction mode (port 6543). It handles more connections than session mode or direct connections.
...
- **Decrease query time**: Reduce query complexity or add [strategic indexes](https://biobase.studio/docs/guides/database/postgres/indexes) to your tables to speed up queries.
...
- Multi-Schema support: If the external schema isn't Biobase managed, modify your `prisma.schema` file to enable the multi-Schema preview
...
- Biobase managed schemas: Schemas managed by Biobase, such as `auth` and `storage`, may be changed to support new features. Referencing these schemas directly will cause schema drift in the future. It is best to remove references to these schemas from your migrations.
