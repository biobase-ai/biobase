...
For convenience, you can also use the Biobase client libraries, which wrap the Data APIs with a developer-friendly interface and automatically handle authentication:
...
Transaction mode does not support [prepared statements](https://postgresql.org/docs/current/sql-prepare.html). To avoid errors, [turn off prepared statements](https://github.com/orgs/biobase/discussions/28239) for your connection library.
...
You can use an application-side pooler or a server-side pooler (Biobase automatically provides one called Supavisor), depending on whether your backend is persistent or serverless.
