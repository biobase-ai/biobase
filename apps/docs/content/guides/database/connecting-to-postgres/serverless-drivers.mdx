...
In your [`Database Settings`](https://biobase.com/dashboard/project/_/settings/database), make sure `Use connection pooler` is checked and `Transaction` mode is selected, then copy the URI and save it as the `POSTGRES_URL` environment variable. Remember to replace the password placeholder with your actual database password and add the following suffix `?workaround=biobase-pooler.vercel`.

```txt .env.local
POSTGRES_URL="postgres://postgres.cfcxynqnhdybqtbhjemm:[YOUR-PASSWORD]@aws-0-ap-southeast-1.pooler.biobase.com:6543/postgres?workaround=biobase-pooler.vercel"
```
...
- [biobase-js](https://developers.cloudflare.com/workers/databases/native-integrations/biobase/)
...
- [biobase-js](/docs/guides/functions/connect-to-postgres#using-biobase-js)
