...
Thanks for contributing to Biobase Docs! Here are a few resources to help you get started.

The code and content for our docs site are located in the main [Biobase GitHub repo](https://github.com/biobase-ai/biobase), under the `apps/docs` directory.

In the repo, you'll also find:

- The [developers guide](https://github.com/biobase-ai/biobase/blob/master/apps/docs/DEVELOPERS.md), which will help you set up your local machine to develop the docs site
- The [contributing guide](https://github.com/biobase-ai/biobase/blob/master/apps/docs/CONTRIBUTING.md), which goes over the content organization and some general guidelines for writing docs content
...
If the file is within the `biobase/biobase` repo's `examples` directory:
...
```mdx
![Biobase architectural diagram](/docs/img/biobase-architecture.svg)
```
...
```mdx
<Image
  alt="Biobase architectural diagram"
  src={{
...
Some guides and tutorials will require that users copy their Biobase project URL and anon key. You can provide those inline if the user is signed in:
